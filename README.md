...
A **citizen** is a session `fno agents spawn` creates: a row in the registry, addressable by `fno mail`, holding its own claim, and surviving the session that spawned it.
...
A **king** is a session bestowed authority over one scope, a project or an epic, for one reign; the crown expires on exit, and the graph it leaves behind is what outlives it.
