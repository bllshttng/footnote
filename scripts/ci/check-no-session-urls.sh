...

{
    ...
    echo "    Re-running THIS job after the edit does not help either: a rerun"
    echo "    replays the original event payload, which still carries the old"
    echo "    body. Push a commit so a fresh run reads the edited field."
