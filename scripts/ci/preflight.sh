...
if HARNESS_MARKERS="$(PYTHONPATH="$PREFLIGHT_WT/cli/src" python3 -c \
   ...
   && [[ -n "$HARNESS_MARKERS" ]]; then
    ...
    HARNESS_MARKERS="CODEX_THREAD_ID CLAUDE_CODE_SESSION_ID CODEX_SESSION_ID GEMINI_SESSION_ID OPENCODE_SESSION_ID CLAUDE_SESSION_ID CLAUDECODE_SESSION_ID HERMES_SESSION_ID TARGET_SESSION_ID CODEX_CI CODEX_INTERNAL_ORIGINATOR_OVERRIDE CODEX_SHELL CODEX_COMPANION_SESSION_ID CODEX_COMPANION_TRANSCRIPT_PATH"
fi
