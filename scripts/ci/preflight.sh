...
if HARNESS_MARKERS="$(PYTHONPATH="$PREFLIGHT_WT/cli/src" python3 -c \
   ...
   && [[ -n "$HARNESS_MARKERS" ]]; then
    ...
    HARNESS_MARKERS="CODEX_THREAD_ID CLAUDE_CODE_SESSION_ID CODEX_SESSION_ID GEMINI_SESSION_ID OPENCODE_SESSION_ID CLAUDE_SESSION_ID CLAUDECODE_SESSION_ID HERMES_SESSION_ID TARGET_SESSION_ID"
fi
...
# PR; this leg is the lane that runs before the push. scripts/ci/ci-gate-lanes.tsv
# records both invokers per row, and check-ci-gate-lanes.sh verifies each one
# really invokes the script, so this list cannot silently drift out of the
# manifest.
#
...
if retry_run_leg structural-gates:fno; then
    echo "preflight: === structural gates (cheap half of guards.yml) ==="
    sg0="$SECONDS"
    sgrc=0
    # Keep going rather than short-circuit: one red gate should not hide the
    # other twelve, which is the whole reason to run them early. A helper
