...
bash tests/hooks/test_agy_stop_hook.sh'
    step "in_review dispatch guard (x-2dc5)" "." 'bash tests/target-preflight/test-init-in-review-gate.sh'
...
bash skills/agent/tests/test_spawn_guard.sh'
    ...
    step "ruff + mypy for new path-config modules" "cli" 'uv run ruff check src/fno/paths.py src/fno/config/ src/fno/config_io.py
...
if need_prereq 'python3'; then
    ...
    python3 -c 'import yaml' 2>/dev/null || miss "python3-yaml" "pip install pyyaml (the bundler + marketplace lint import it)"
fi
