...
#   fno_python_init "$REPO_ROOT"      # sets FNO_PYTHON, exports PYTHONPATH
...
fno_python_init() {
    ...
    # Package source first, so a checkout works before `uv tool install`.
    ...
    # (`fno` is the Rust binary and has no shebang; LC_ALL=C keeps sed off a
    # locale decode error if something non-text lands on PATH under that name.)
    local shim interp
