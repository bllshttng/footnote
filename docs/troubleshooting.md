...
- **The `review-coverage-gate` workflow is green and the `fno/review-coverage` status is red.** Both readings are correct and the gate is healthy. The workflow computes coverage and publishes the verdict. A run that finds no covered review therefore SUCCEEDS at its job and publishes a FAILING status. Read the status. Never read the run that produced it as the verdict. Reproduced live: the workflow run reported success at the same head and the same minute that the status reported failure. This is the producer confused for its product, and the two disagree by design.
