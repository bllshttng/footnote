...
- **A shell command is blocked for its text, not its effect.** Guards block commands containing a dangerous phrase. A literal merge command or a force-push counts, even inside an echo or a commit message. Route merges through `fno do pr merge` and write PR and commit bodies with `--body-file` rather than inline. Beyond the guards, the merge ruleset makes GitHub refuse an uncovered head on every path. The release valves are the `coverage-override` label and the attended `coverage-waive` command, below. That text-matching classifier is the harness's own. It is not this repo's gates. `hooks/pre-push.sh` and `hooks/git-protection.py` read the DESTINATION only. They refuse a write to `main`, `master`, `develop`, or `dev`. **A feature-branch force-push after a rebase needs no approval.** Not from an operator, not from a king. Prefer `--force-with-lease` so a concurrent push refuses instead of clobbering. This never extends to the four protected branches.
- **The `review-coverage-gate` workflow is green and the `fno/review-coverage` status is red.** Both readings are correct and the gate is healthy. The workflow INVALIDATES and never certifies. It never posts success on its own judgment, and on a new head it posts failure unconditionally. The green marker comes from a head-pinned local attestation in events.jsonl, which the workflow cannot see. Read the status. Never read the run that produced it as the verdict. Reproduced live: the workflow run reported success at the same head and the same minute that the status reported failure. This is the producer confused for its product, and the two disagree by design.
- **A merge is refused with `fno/review-coverage` not passing.** The ruleset requires a covered review at the exact head. At the review round cap the gate does not refuse and neither valve applies. The review phase is complete, and the PR merges on green CI. When the checkout is on another branch, run `fno-agents review-coverage --cwd <any-checkout> --pr <n>`. The numeric selector resolves and pins that PR's head. It publishes the status from the row it emits, and the refusal names both shas. If the merge must land regardless, there are two release valves. Apply the `coverage-override` label from a login that is not the PR author. The gate workflow stamps the pass naming you, and an author-applied label never opens the gate. Or run the attended command from an operator terminal: `fno do pr coverage-waive <n> --reason "why this head is waived"`. It records an operator decision pinned to the live 40-character head and prints `coverage waiver recorded: <owner/repo>#<n>@<short head>`. It is void the moment the head moves. A standing waiver covers every PR once recorded under the `review-coverage-waiver` subject. It never clears an unresolved confirmed correctness or security finding. `fno do pr merge` reads both valves and prints `coverage waived:` on its receipt, so a waived merge never reads as a reviewed one. The post-merge audit counts it by name. If GitHub still cites this context, retry the merge. The ruleset can lag behind the new success status. Full contract: [review-coverage-termination](architecture/review-coverage-termination.md).
- **CI fails on the LOC gate.** A positive executable-LOC delta on a control-plane path needs a `loc-exception: <reason>` line in the PR body. The failure output prints the exact delta and per-file breakdown. Prefer deleting before adding.
- **"Text file busy (os error 26)" in an fno-agents lib test.** A test wrote its exec stub from inside the test process, and a sibling test's fork held the write fd. Write the stub with `write_exec_stub` in `crates/fno-agents/src/lib.rs`; the guard test `every_exec_stub_goes_through_write_exec_stub` names the site. This cause is fixed at the source, not rerun.
- **Any other intermittent test failure.** These are rerun-able flakes, not real failures; re-run the job before investigating.
...
## GraphQL quota reserve proof

Run `python3 scripts/diagnostics/graphql-quota-soak.py --duration 3600 --pr <reviewed-pr> --receipt .fno/artifacts/graphql-quota-soak/<UTC>.json` while at least 15 workers are live. The verifier samples quota and live-worker count once per minute, then resolves the PR head over REST and runs one priority coverage recompute. It writes no receipt unless `min_remaining > 200`, `min_live_workers >= 15`, and head-matched `coverage=covered` with `reviewed_count > 0` all hold. Validate the latest receipt with `python3 scripts/diagnostics/graphql-quota-soak.py --check-latest --min-seconds 3600 --max-age-hours 24`. A missing or stale receipt fails.
