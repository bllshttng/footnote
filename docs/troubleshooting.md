...
- **A merge is refused with `fno/review-coverage` not passing.** The ruleset requires a covered review at the exact head. When the checkout is on another branch, run `fno-agents review-coverage --cwd <any-checkout> --pr <n>`. The numeric selector resolves and pins that PR's head. It publishes the status from the row it emits, and the refusal names both shas. If the merge must land regardless, apply the `coverage-override` label. The gate workflow stamps the pass naming you. `fno do pr merge` reads the same label. It prints `coverage waived:` on its receipt, so a waived merge never reads as a reviewed one. The post-merge audit counts it by name. If GitHub still cites this context, retry the merge. The ruleset can lag behind the new success status.
