...
- **A shell command is blocked for its text, not its effect.** Guards block any command whose text contains a dangerous phrase (a literal merge command, a force-push), even inside an echo or a commit message. Route merges through `fno pr merge` and write PR/commit bodies with `--body-file` rather than inline. Beyond the guards, the merge ruleset makes GitHub refuse a merge of an uncovered head on every path; the `coverage-override` label is the only release valve.
- **A merge is refused with `fno/review-coverage` not passing.** The merge ruleset requires a covered review at the exact head being merged. Run the review verb at HEAD, which publishes the status from the row it emits; the refusal text names the sha it was computed at and the sha it needs. When the merge must land regardless, apply the `coverage-override` label: the gate workflow stamps the pass naming you, and the post-merge audit counts it by name.
