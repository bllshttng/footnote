...
- **A merge is refused with `fno/review-coverage` not passing.** The ruleset requires a covered review at the exact head. When the checkout is on another branch, run `fno-agents review-coverage --cwd <any-checkout> --pr <n>`. The numeric selector resolves and pins that PR's head. It publishes the status from the row it emits, and the refusal names both shas. If the merge must land regardless, there are two release valves. Apply the `coverage-override` label from a login that is not the PR author (the gate workflow stamps the pass naming you; an author-applied label never opens the gate). Or, from an attended operator terminal, run `fno do pr coverage-waive <n> --reason "why this head is waived"`: it records an operator decision pinned to the live 40-character head, prints `coverage waiver recorded: <owner/repo>#<n>@<short head>`, and is void the moment the head moves. A standing waiver, recorded once under the `review-coverage-waiver` subject, covers every PR but never an unresolved confirmed correctness or security finding. `fno do pr merge` reads both valves and prints `coverage waived:` on its receipt, so a waived merge never reads as a reviewed one. The post-merge audit counts it by name. If GitHub still cites this context, retry the merge. The ruleset can lag behind the new success status. Full contract: [review-coverage-termination](architecture/review-coverage-termination.md).
