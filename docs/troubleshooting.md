...
- **"Text file busy (os error 26)" in an fno-agents lib test.** A test wrote its exec stub in-process. A sibling test's fork held the write fd open. Write the stub with `write_exec_stub` in `crates/fno-agents/src/lib.rs`. The guard test `every_exec_stub_goes_through_write_exec_stub` names the site. The fix is at the source, not a rerun.
