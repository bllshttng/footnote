...
| `routing.models` | list[RoutingModelBlock] | `[]` | never | The declared routing inventory: a list of {name, harness, model, route, account, band, effort, cost_per_mtok_in, context} rows. Nothing built-in is authoritative, so a fresh install declares its own fleet and inherits no other machine's. Declare none and the grid records no-inventory-declared and injects nothing. `fno/routing_sample.toml` ships as a labelled sample no code path reads. |
