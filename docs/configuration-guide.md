...
| `agents.defaults.substrate` | str | `` | advanced | Default substrate for bare spawns (pane\|bg\|headless); an explicit substrate wins, empty = unset (per-provider default). Config-sourced value degrades open with a warning if incompatible with the resolved provider (e.g. bg is claude + opencode). |
...
| `dispatch.substrate` | str | `` | advanced | Default dispatch substrate (bg\|headless\|pane); empty = per-harness default (claude + opencode=bg, else headless). |
