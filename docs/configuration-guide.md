...
| `backlog.render_targets` | list[RenderTargetConfig] | `[]` | advanced | Auto-rendered projections (GLOBAL config file only): every graph mutation re-renders each `{path, scope, projection=local\|backlog\|roadmap}` target; public projections use the leak gate and local is full-detail/ungated. |
...
| `agents.defaults.substrate` | str | `` | advanced | Default substrate for bare spawns (pane\|thread\|headless); bg is a deprecated alias for thread for one release. An explicit substrate wins, empty = unset (per-provider default). Config-sourced value degrades open with a warning if incompatible with the resolved provider (e.g. thread is claude + opencode). |
...
| `dispatch.substrate` | str | `` | advanced | Default dispatch substrate (thread\|headless\|pane); bg is a deprecated alias for thread for one release. Empty = per-harness default (claude + opencode=thread, else headless). |
