...
| `backlog.render_targets` | list[RenderTargetConfig] | `[]` | advanced | Auto-rendered public projections (GLOBAL config file only): every graph mutation re-renders each {path, project, projection=backlog\|roadmap} target through the leak gate. |
