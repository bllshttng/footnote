...
| `mux.board_scope` | str | `repo` | advanced | Which projects the mux backlog board shows: repo (default, this checkout's project.id) \| all \| workspace:<name>, e.g. workspace:main for a workspace holding web, backend and marketing. A scope that cannot resolve shows the unscoped lane only - the board refuses to widen back to every project. Latched at mux server birth; `fno mux kill-server` re-reads. |
