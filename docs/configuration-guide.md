...
| `blueprint.max_prs_per_epic` | int | `4` | advanced | Default cap on group PRs per decomposed epic. An epic plan-doc's `max_children:` frontmatter overrides this per-epic (and `--max-prs` may then only tighten it). |
