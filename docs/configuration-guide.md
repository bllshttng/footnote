...
Keys live in a flat `config.toml` (`.fno/config.toml` project-local, `~/.fno/config.toml` global). A dotted key like `branch.prefix` is `prefix` under `[branch]`. For a copy-paste template with every key at its default, see [settings.example.toml](settings.example.toml); read or set individual keys with `fno config get <key>` / `fno config set <key> <value>`.
