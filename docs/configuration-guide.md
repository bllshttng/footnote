...
| `agents.defaults.effort` | str | `` | advanced | Default reasoning effort for bare spawns (minimal\|low\|medium\|high\|xhigh\|max); an explicit --effort wins, empty = unset. Config-sourced effort degrades open on providers with no effort surface (gemini). |
