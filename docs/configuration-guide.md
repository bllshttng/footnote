...
| `agents.defaults.provider` | str | `` | advanced | Default provider for bare `fno agents spawn` / `/agent spawn` (claude/codex/gemini/agy/opencode); an explicit -H flag wins, empty = unset (harness inference then claude). Validated at the spawn seam. |
...
| `agents.profiles` | dict[str, SpawnDefaultsBlock] | `{}` | advanced | Per-verb spawn-defaults overlay keyed by the seed's leading slash-verb (agents.profiles.<verb>.{provider,model,effort,substrate,permission_mode,route,account}), one rung above agents.defaults in precedence. The verb key is the seed's first token, /-stripped and fno:-namespace-stripped, lowercase (/fno:think and /think resolve alike). The overlay reaches every spawn carrying a slash-verb seed, including autonomous dispatch; an explicit flag always wins. No content-based inference; a table with no matching verb resolves exactly as agents.defaults. |
...
| `recovery.watchdog` | typing.Literal['off', 'report', 'wake', 'handoff'] | `off` | advanced | External fleet watchdog lane on the same tick: off (default) \| report (classify and emit) \| wake (also resume stalled sessions) \| handoff (also permit a proved cross-provider outage transaction). Reap and reroute never fire from a tick; they need a manual `fno agents watchdog --apply-all`. |
...
| `recovery.provider_outage_quorum` | int | `2` | never | Minimum distinct-session votes required to open a provider/account breaker; minimum and default 2. |
| `recovery.provider_outage_fup_window_seconds` | int | `300` | never | Fair Usage Policy cross-session quorum window in seconds; minimum and default 300. |
| `recovery.provider_outage_529_count` | int | `3` | never | Consecutive assistant 529 records required per session; minimum and default 3. |
