...
You create, update, rank, or close backlog cards from a terminal, or you wonder why a verb refused. This page owns the day-to-day verbs and the node lifecycle. Misreading it costs a lost edit: a field you change by hand does not move the card, because the board renders from node fields, never from the board file.
