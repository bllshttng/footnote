...
Pointing `state_dir` at a per-project root is how one checkout gets its own graph, ledger, briefs, agent registry and mail bus. `fno project init <id>` writes that key for you and prints which half of the environment it actually isolated: see [project environments](architecture/fno-project-environments.md). The mux socket directory (`<state_dir>/mux/`) follows the same key. The value must be one the mux can expand on its own: an absolute path, a `~` path, or a repo-relative path. A value carrying a template variable or `$VAR` is declined with a warning, and the mux falls back rather than follow it. `FNO_MUX_DIR` still overrides the dir outright. A pinned `FNO_CONFIG` can also carry no usable `state_dir`, because the file is unparseable or the key is absent. The socket dir then lands beside the pinned file, with a warning, never back on the real `~/.fno/mux`. The mux resolves per invocation like every other state path. Run a mux server from the project whose `state_dir` it must serve. When sessions sit at a root the current process no longer resolves, `fno mux doctor` warns.
