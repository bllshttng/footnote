...
Pointing `state_dir` at a per-project root is how one checkout gets its own graph, ledger, briefs, agent registry and mail bus. `fno project init <id>` writes that key for you and prints which half of the environment it actually isolated: see [project environments](architecture/fno-project-environments.md). The mux socket directory (`<state_dir>/mux/`) follows the same key, so `FNO_CONFIG` pointing at an isolated config isolates the mux too. `FNO_MUX_DIR` still overrides it outright. An `FNO_CONFIG` the mux cannot parse (a yaml-pinned path) lands the socket dir beside the pinned file rather than back on the real `~/.fno/mux`.
