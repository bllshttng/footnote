...
What each harness can DO, rendered from the capability table (`map_version 19`). One row per supported harness - including roster entries with no capability row, which read unmeasured everywhere - and one column per feature key.
