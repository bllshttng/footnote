...
Even without a plugin installed, the target loop skill itself includes an instruction for the LLM: "when emitting `<promise>X</promise>`, also write `.fno/target-promise.signal` with the content `X`." This is pure prompt-level enforcement - works on every driver that honors the skill markdown.
