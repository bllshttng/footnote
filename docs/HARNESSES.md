...
- **Background dispatch is substrate-specific, and the worker command is harness-native:** `/target bg` remains Claude-only (`claude --bg`); every other harness dispatches one-shot `headless`. The command a headless worker receives comes from the harness-capability map's per-harness `dispatch_command` (`fno dispatch resolve`), grounded in what each CLI actually supports, NOT a blanket "prose brief only" rule. Verified: **Claude** invokes `/target` (native slash command); **Codex** invokes `$fno:target` (its plugin exposes the footnote skill and `codex exec` expands `$plugin:skill`); **Antigravity (`agy`)**, gemini's successor, recognizes skills as active slash commands, so it invokes `/target`. **OpenCode** and legacy **Gemini** have no footnote slash surface (opencode is a plugin, gemini an extension), so they get a **prose brief** that names the node and instructs autonomous delivery. Never pass a Claude `/target` string to a harness that cannot expand it (opencode/gemini would run it verbatim and no-op). The build/route lane is Claude-only, so a non-Claude spawn drops `--role`/`--route`.
- **A hook that fires outside a session cannot live in the plugin manifest.** Claude sources plugin hooks from the running session's hook table, so a bare CLI subcommand with no agent session (`claude rm`) sees none of `hooks/hooks.json` - the command string is never even read, which is why an unset `${CLAUDE_PLUGIN_ROOT}` is never the explanation for such a failure. `WorktreeRemove` is the one event footnote needs there: Claude marks a hook-created worktree `hookBased` and will only remove it by running that hook, so without a settings-level copy every such worktree is stranded ("WorktreeRemove hook failed") forever. `fno setup cli-hooks` merges it into `~/.claude/settings.json`, and because that path is persisted outside the session it is canonicalized first - writing a worktree path there would strand every worktree on the machine the moment that worktree is archived. The plugin entry stays for in-session removal. Both then fire in-session, which is safe on the paths that matter (the script is idempotent on an already-gone path and still refuses a dirty one), with one narrow exception: fired concurrently on a live worktree, the loser can find the directory already removed by the winner and report a failure it did not cause.
