...
`/fno:law` is portable as a conversation skill, but enactment is currently Claude-only. Claude's `PreToolUse` hook recognizes one exact `fno law enact` command, binds a one-shot receipt into its updated input, and returns `ask` only in a documented prompting mode (`default`, `acceptEdits`, or `plan`). Bypass, yolo, `dontAsk`, headless, unknown permission modes, malformed commands, missing sessions, and failed proposal arms deny and keep the proposal staged. Codex, Gemini, OpenCode, and other non-Claude sessions can compose or resume a proposal but cannot enact it until their harness exposes an equivalent non-forgeable human approval event.
