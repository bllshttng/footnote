...
The table is measured at release time, and a harness newer than its fno drifts it. A row can be corrected by config alone, no code change (x-244c). Declare a `[harness.<name>]` block in `$PWD/.fno/config.toml` (or the global one; project-local wins per harness). The merge covers the whole row per field, config winning, and it runs through the same contract validation the bundled table ships under: a rejected override keeps the bundled row and names itself, so a typo in one harness's block never un-configures another. The shallow lane keys `[harness.<name>.attach]` and `[harness.<name>.resume]` keep working and land on the nested resume forms. The Rust reader (pane attach and resume) and the Python reader (dispatch) implement the same rules, and CI pins them together: a staged config is resolved through both and any difference fails naming the harness and field.
