...
Agent text is re-read by every recipient on every turn. It must read once. A machine checks seven rules at the tool boundary. Rules 1 to 6 run on PR bodies, comments, and changed markdown. Mail enforces the relay compression contract only: rule 7, the 80-word cap, which also runs on the evidence of a backlog encounter, its number config-driven per surface.
