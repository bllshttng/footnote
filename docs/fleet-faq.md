...
A stale file can sit at the canonical path. One session read that copy, saw an mtime five and a half hours old, declared the worker dead, and paid for a cold replacement spawn that had to relearn the task. The live transcript had been written twenty minutes earlier under the worktree path.
...
Prefer resume over spawn whenever the old worker holds context you would otherwise pay to rebuild. A worker five hours into a port is worth more than a fresh one, even a stronger fresh one.
...
## Should I spawn, reuse, or resume?
...
3. **Spawn** only when neither exists.
...
Escalate, never synthesize. A refused gate is a message to the king or the operator, and flipping config or an environment variable to get past it converts a safety property into a silent one.
...
- `fno inbox outstanding list` does not exist. The command printed a usage error, a grep filtered it to nothing, and the shell exited 0. The read looked like "no open questions" when 283 were open.
...
- **Fine:** the contract moves to a doc the file already points at from eight other functions, and a one-line pointer stays.

Also worth knowing, because it changes what you tell a worker: the same gate **excludes test paths** (`check-file-budget.sh`). Test coverage is free. Say so, or a worker will delete tests it should keep.

## Two live laws contradict each other

Surface and hold. Do not pick.

When two rulings point opposite ways at the same action and neither is marked as winning, choosing one is synthesizing a resolution the operator withheld. This is especially true when a law's own rationale says the tension was "flagged to the operator rather than resolved here". That sentence is a standing instruction, not a gap.
