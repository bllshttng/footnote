...
| `.fno/law-proposals/<proposal-id>.json` | `cli/src/fno/law.py` via `paths.law_proposals_dir()` | pending for 15 minutes, armed for 2 minutes, consumed or expired for at most 24 hours, then pruned by the proposal writer |
