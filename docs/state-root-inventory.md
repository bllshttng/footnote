...
| `provider-runtime-state.json`, `.update.lock` | `cli/src/fno/adapters/providers/runtime_state.py` via the `paths` accessor | permanent; the update lock lives for one write. The document's `reservations` block (the opt-in routing-admission owner, `cli/src/fno/adapters/providers/admission.py`) lives here too: reservation rows are written under the same lock and expire after `routing.admission.reservation_ttl_seconds` (default 900s), extended while a launch reconciliation keeps proving the worker live. |
