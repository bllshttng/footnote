...
| `operator-capture/` | `cli/src/fno/inbox/operator_turns.py` | per session, `<session-id>.jsonl` is the ack ledger and receipt. `<session-id>.scan.json` caches the scan cursor and pending turns. Safe to delete, the next read rescans from byte 0 |
