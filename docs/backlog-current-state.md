...
The canonical command vocabulary (11 groups; `annotate` retired behind a one-release forwarding shim) is defined once in `crates/fno-agents/src/backlog/commands.rs` and mirrored in `scripts/ci/verb-collapse-map.tsv`. Legacy spellings keep working during the compatibility window. The final 11-group-only catalog lands after the note storage merge and compatibility removal.
