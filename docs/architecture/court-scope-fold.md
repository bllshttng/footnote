...
the next fragment write closes the gap. Refresh cadence belongs to the
caller (hooks, or the operator).

## The fold lives in the native binary

`fno-agents court-fold` reads the graph, the claims dir, and the registry. It
...
`owned` says which live crown answers for a node. The candidates are the registry's live crowns, read the way the drain reads them (`territory::live_crowns`), never the crowns one caller happens to pass. The deepest crown level holds the node, then the lowest canonical scope on a tie. A crown with no live registry row (a manifest-only crown, or a king whose row went terminal while its session runs) owns nothing, and its own fold reads `owned_total` and `owned_counts` null with `owned_reason` rather than 0. A failed owner read makes every fold's `owned` fields null with `owned_reason`. Measured 2026-09-21: 11 of the L1 crown's 13 PR-bearing rows also sat in an L2 fold.

## Owned counts

`owned_total` and `owned_counts` count every status of the nodes this fold owns, from the same rule as the row `owned` mark: the deepest live crown whose scope holds a node, then the lowest canonical scope on a tie (`territory::node_owners`). For an L1 crown that is the work no L2 king holds, which is the work it can crown or dispatch. Measured 2026-09-22 on the live machine: the fno L1 fold reads a total of 2559 with 216 active rows, 76 of them owned; the four live L2 folds own 18, 52, 19 and 51, and 76 + 18 + 52 + 19 + 51 = 216. The king's scope line leads with this number.
