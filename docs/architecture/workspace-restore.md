...
Every member that cannot come back is named, with the reason. The reasons include: no registry row, no session id, a harness the table gives no resume form, an ambiguous name, a failed spawn, and a seat that is already live in this session, which names the pane. Silence is never an outcome. A run that resumes two and refuses one prints all three.

The verb answers for portals too (x-a6b9). After the members it walks every stored portal slot and reports one row per seat, classified by the same door the focus fill uses: a held seat whose claude Drive row answers resumes through the same off-loop re-entry plan batch the members use, a seat already showing its row reports `focused`, and a seat whose row answers nothing (or ambiguously) is refused with the key named. A filled Locate-tier portal carries a notice, because it shows where the thread lives, not the thread. `--json` splits the reply: `members` for squad members, `portals` for held seats, and the counts sum both. Under `--dry-run` every held portal reads `planned` and nothing spawns. The 2026-09-13 measurement that motivated this: the verb answered 7 members (6 refused, 1 focused) and zero portal rows while the store held 3 portal slots. The portals array is the fix; `tests/mux-portal-restore-server-kill.sh` proves it with a real kill and a real restore.
