...
The verb answers for portals too (x-a6b9). After the members it walks every stored portal slot and reports one row per seat, classified by the same door the focus fill uses. A held seat whose claude Drive row answers resumes through the same off-loop re-entry plan batch the members use. A seat already showing its row reports `focused`. A seat whose row answers nothing, or answers ambiguously, is refused with the key named. A filled Locate-tier portal carries a notice, because it shows where the thread lives, not the thread. `--json` splits the reply: `members` for squad members, `portals` for held seats, and the counts sum both. Under `--dry-run` every held portal reads `planned` and nothing spawns. The change answers a 2026-09-13 measurement. The verb then reported 7 members and 6 refusals with zero portal rows. The store held 3 portal slots. The portals array is the fix. `tests/mux-portal-restore-server-kill.sh` proves it with a real kill and a real restore.
