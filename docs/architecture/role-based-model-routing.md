...
`config.routing` declares the model inventory. One `[[routing.models]]` row per model carries `name`, `harness`, `model`, and optional `band`, `effort`, `cost_per_mtok_in`, `context`, `context_measured_at`, `context_source`, `route`, `account`, and `color` (the sideline lane color when the row matches an agent, parsed by the mux's Rust reader and declared in the schema so a typo surfaces at config validation). A row's `context` counts as a measured window only beside a `context_measured_at` date; `context-run --probe` and `fno config route inventory` read the dated row, and an unmeasured model falls back to the id-based default and says so.
