...
A routed codex worker reaches its endpoint through three inline `-c` config tokens (`model_providers.<p>`, `model_provider`, `model`) built by `codex_route.rs` in fno-agents; the API key rides the spawn env under the name the provider declares. The spawn records only the route's IDENTITY on the registry row (`route_provider_id`, `model_name`): never an endpoint, never a key. A relaunch re-resolves the route from TODAY's config through the same Rust builder, so the spawn and every relaunch door cannot disagree, and an edited `base_url` applies on the next relaunch.

Which door does what: `fno agents resume` and the headless followup (`fno agents ask`) restore the route, because they own the child env. The mux ResumeAgent gesture and `fno mux workspace restore` refuse a routed codex row and name `fno agents resume <row>`, because a pane spawn's only env channel is an argv prefix visible in `ps`. A refusal names the row and the provider; nothing launches. A loaded codex thread still wakes over the app-server first - that door needs no route.

A provider named `openai` is refused at resolve time: it is the sentinel every unrouted codex row already stamps, so a relaunch could never tell a restored route from a default-endpoint launch. When a routed row's provider is deleted from config, or its protocol stops being `openai` at relaunch time, or its key no longer resolves, every relaunch door refuses by name instead of relaunching on codex's default provider - a default-provider relaunch holding the route's identity would read as success while billing the wrong account.
