...
| `wake` (owed work) | the row holds its node's live claim, its open PR's gate reports a non-empty `ready_blockers`, and the transcript is silent past the stalled threshold. Checked ABOVE the plain wake so a gate-red row wakes with the obligation payload whatever its tail reads; the gate outranks the worker's self-report (x-c7fd: the summary said green while the gate said red). A row that already declared itself done with a closed promise retires first - that shape belongs to the loop gate, not this lane | `PR #<n> gate red (<blockers>); <n>m silent with the claim live, gate outranks the self-report` |
...
| `wake` (owed work) | `fno agents mail send <id> --raw "<obligation>"`: user-shaped text at the prompt line, the one shape a stopped codex worker's turn boundary picks up. The payload names the PR, the blockers, and the next verbs (`fno do pr status`, then `/fno:pr check`). Delivery truth is the mail receipt. Refire is digest-gated per row: an unchanged blocker set never mails twice, a changed set is a new debt and mails once more |
