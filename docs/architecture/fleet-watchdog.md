...
`fno agents reap` sweeps two surfaces: the registry rows, and the mux tab sideline through `fno mux workspace prune --tabs-only --include-used-shells`. The daemon does not run the mux half. A visible pane closes because a human asked, never on a timer.
