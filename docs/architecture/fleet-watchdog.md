...
`config.recovery.watchdog` rides the pr_watch tick: `off` (default), `report` (classify and emit only), `wake` (also apply the wake lane), or `handoff` (permit a positively proved cross-provider outage transaction). `config.autonomy.enabled` and `config.recovery.enabled` veto every mode. `wake` and manual `--apply-all` never imply handoff authority. No tick value reaps or performs the legacy reroute. Every completed sweep, tick or manual, writes `~/.fno/watchdog-sweep.json` with the exact provider-outage report object; an absent or unreadable provider instrument is stamped `unknown`, never a measured empty breaker set.

## Provider-wide outages and handoff

The provider supervisor runs once before GitHub PR polling on the existing tick, so a stalled GitHub request cannot starve outage detection and no second daemon, timer, or GitHub poll exists. Durable raw assistant transcript records vote; `session_truth.last_message`, registry timestamps/status, process liveness, free-text task messages, and quoted user text do not. Pane text votes only after an atomic snapshot and only while fresh. Every vote joins machine-recorded harness, model-vendor provider, and account-record identity. A missing axis is count-bearing UNKNOWN and cannot open a breaker or select a route.

A Fair Usage Policy 429 that says access requires a request is a manual-restore signal even when it names no reset. It is distinct from the older reset-bearing temporary limit. Two distinct sessions on the same provider/account within 300 seconds open the breaker. A 529 needs three consecutive assistant errors spanning at least 120 seconds on each session, then two persistent sessions within 600 seconds. Successful assistant content resets a session's 529 sequence. Transcript evidence remains eligible for 600 seconds; persisted pane and destination-health markers remain eligible for 120 seconds. Reset-bearing limits retain a 120-second close grace. UNKNOWN never means healthy.

In `handoff` mode, the destination must carry explicit provider/account/model identity, its provider must differ from the broken provider, and a neutral canary outside the node worktree must persist the exact `FNO_PROVIDER_HEALTH_OK` response and stop before ownership changes. Candidate probing may continue before source death; once the source is positively dead, the transaction never tries a second destination. The shared transaction serializes on `dispatch:<node>`, re-reads source registry/claim/manifest/worktree authority, positively stops the exact pane or PID incarnation, archives the immutable target manifest, releases only the exact node-claim holder, and spawns one pane successor in the same worktree and branch. It commits only after positive pane/PID executability, exact successor claim, fresh manifest, and unique executable ownership. Any post-death failure parks the node unclaimed and preserves the worktree and archive.

Breaker transitions, terminal handoff transitions, and count-bearing refusals are schema-backed daemon events. A daemon-authored node decision is recorded once only when a transaction newly reaches `committed` or `parked`; observations, canary proof, intermediate phases, refusals, contention, and terminal replay record no decision.

Manual inspection remains `fno agents watchdog --json`. `fno agents watchdog` is a dry run, `--apply` applies wake only, and `--apply-all` applies the existing manual wake/reap/reroute lanes; none can authorize a provider handoff. To enable installed-cadence migration, set `recovery.watchdog = "handoff"` in config and keep both master switches enabled. There is intentionally no manual handoff command and no migration test should run against a live node.
