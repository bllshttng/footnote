...
The watchdog is not the only quiet-row driver. The daemon's retire arm runs the nudge ladder (`pr_nudge.rs`), which mails or resumes any session the retirement sweep keeps on an open PR so the PR reaches a merge; after 3 unanswered nudges it escalates to the operator, and a recorded merge order (subject `merge-order:<node>:after:<lead>`) pauses it. The two lanes read each other's activity through the transcript, so a row the watchdog woke does not count toward the ladder's escalation. See [reaping-faq.md](../reaping-faq.md) for the ladder's rules.
