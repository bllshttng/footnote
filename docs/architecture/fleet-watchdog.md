...
The watchdog is not the only quiet-row driver. The daemon's retire arm runs the nudge ladder (`pr_nudge.rs`) over any session the retirement sweep keeps on an open PR. A live row gets mail, a dead one a resume. After 3 unanswered nudges it escalates to the operator. A recorded merge order (subject `merge-order:<node>:after:<lead>`) pauses it. The two lanes read each other's activity through the transcript, so a row the watchdog woke does not count toward the ladder's escalation. See [reaping-faq.md](../reaping-faq.md) for the ladder's rules.
...
1. **Job state present:** `claude respawn <short>` resumes the job in place under its own id. `fno agents resume <name>` resolves to the same thing.
2. **Job state gone, transcript alive:** `claude --bg --resume <full-session-id>` continues that session in the background under the SAME id. Measured 2026-09-15 on claude 2.1.272: with the session stopped and `jobs/<short>/` deleted, the command exited 0, printed `backgrounded · <short> (idle — send a prompt to start)`, `claude agents --json` listed the same full `sessionId`, and `jobs/<short>/` was recreated. Run it in the session's recorded cwd with the row's `CLAUDE_CONFIG_DIR` and `--settings` so the route rides along. A copy starts only when the session is ALREADY running: the binary prints `note: session <short> is already running in the background, so this started a copy as <new-short>.` and backgrounds the copy under a new id. Any driver that sees `started a copy as` must stop the copy and refuse; a copy is a fork by another name. The resumed serving process inherits none of the spawn's identity env, so a driver that relaunches this way must stamp `FNO_AGENT_SELF` and friends itself.

So the `stale` verdict's remedy splits on one fact. When `jobs/<short>/state.json` survives, the remedy is a respawn. When it is gone, the remedy is the same-id bg resume in route 2, and `fno agents resume <name>` runs it.
...
A merge is the second door into the same room. The daemon's merge reaper asks its own question: is every named node done and merged. It owns that answer and the worktree, which it takes under a rule the sweep does not have. It does not own the retirement. The stop, the receipt and the guarded registry write are the sweep's, and the reaper calls them. The sweep keeps the harness session. Only `fno agents rm` deletes session state. Retirement still runs one native effect: cursor-agent's worker-server reap, which ends leaked processes, not session state. So a row the reaper takes leaves the record a scheduled retirement leaves.
...
Row retirement is the daemon GC sweep, not the watchdog. The classifier is `gc.rs`: work-done through the reverse join, or the planning lane, plus quiet past grace. The lifecycle is `gc_sweep.rs`: stop, registry drop, tree prune. The native effects are `gc_native.rs`: retirement measures the active surface not-applicable and keeps the session. Cursor-agent's worker-server reap is the one cascade arm that stays. `fno agents rm` walks the full removal cascade, typed per effect.
