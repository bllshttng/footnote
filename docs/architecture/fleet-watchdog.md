...
A merge is the second door into the same room. The daemon's merge reaper asks its own question: is every named node done and merged. It owns that answer and the worktree, which it takes under a rule the sweep does not have. It does not own the retirement. The stop, the receipt and the guarded registry write are the sweep's, and the reaper calls them. The sweep keeps the harness session: only `fno agents rm` deletes session state, and the one native effect retirement still runs is cursor-agent's worker-server reap, which ends leaked processes, not session state. So a row the reaper takes leaves the record a scheduled retirement leaves.
