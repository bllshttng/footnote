...
Two reads keep the armed lane off a live session. A transcript whose last record is the assistant turn that ISSUED a tool call has a tool still running: a worker twenty minutes into a build or a `gh pr checks --watch` is silent to every other read, and if that turn mentions a promise the tail classifies `done`. Retire refuses there, the same reading `reap_decision` makes. And the question half asks in front of every terminal marker rather than deleting a span, because `re` has no right-most search: a pattern spanning an open tag to a close tag matched leftmost, so a turn that mentioned the tag in prose and then asked something lost the question to the strip.

`spawned` is read from `origin`, a field written once at row birth, and every path that creates a registry row states what it made: the spawn sites write `spawned`, `register_existing_session` takes the caller's word, and the harness-store healer writes `adopted`. A row with no marker answers unknown and no lane acts on it. Reading the absence instead was wrong twice: a row written before the field existed carries nothing, and an operator's own terminal adopted from the claude store is routinely one of those. Keying on `status == "orphaned"` did not cover them either, because `status` is a liveness stamp that one `fno mail send` flips to `live`.
