...
The unfinished-work report answers the operator question the fleet watchdog serves. The verdict classifier in `fno.agents.watchdog` stays the internal recovery engine (wake, reroute, reap, retire). This module answers "was work started and never finished?" It carries four dimensions. Each finding names the one verb that clears it. `started_free_claim`: an in_progress node whose claim is free, ranked by idle age. The basis adds the branch's commits ahead of `origin/main` where a worktree resolves. Clear with `/fno:target <node>`. `done_ahead_of_main`: a done node whose worktree branch carries commits ahead of a freshly fetched `origin/main`. Clear with the stranded recovery verb scoped to the repository. `dirty_ownerless_worktree`: a worktree with uncommitted paths and no authoritative live owner. Clear by adopting or finishing it. `open_pr_ownerless`: an ownerless open PR at any age. A PR whose owner reads unknown waits 24h before the dimension goes unknown. Clear with `/fno:pr check <number>`.
