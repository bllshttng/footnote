...
When a node is found, these **graph node** fields append: `graph_node_id`, `target_claim_key`, `target_claim_holder`, `target_claim_ttl`, `target_claim_blocked_reason`
...
- **The budget flows inward.** The Rust caller passes its own bound as `fno inbox board --budget-ms`. The board derives every per-source slice from that total, minus a serialization reserve. No independent per-source default exists anywhere. `HAND_RUN_BUDGET_MS` budgets the whole board for a hand run. The env override `FNO_KING_BOARD_TIMEOUT` keeps its seconds unit but now bounds the whole board.
- **The board always answers.** As its budget runs out it stops starting reads. It marks each unstarted source `not-read: board budget exhausted after <source>` and emits the payload it has. `unreadable` queues keep exit 1. The outer transport kill is now a backstop for a different failure: the board did not self-report. The Rust message names that as a board bug, never as a slow source.
- **No fresh interpreter of the running package.** The five fixed reads and the per-claim `backlog get` run in process now. Each one calls the function its verb wraps: `read_planned_unclaimed`, the claims root merge, `outstanding.collect`. The claimed-node reads share one graph read plus exact node resolution. `gh pr list` stays a subprocess because it is a different binary. `backlog ready` stays one because its filter chain lives inline in the typer command. A second implementation there drifts from `next`. `agents needs` stays one because the Rust runtime owns it.
- **A killed send leaves a row.** `fno agents mail send` writes the durable bus row before `resolve_or_suggest` runs. The row is addressed by the recipient name the caller typed. A live miss reuses it as the durable floor, so there is no second row. A confirmed hosted delivery stamps `delivered_at` on its audit record. Every unread reader uses that stamp to suppress the twin, and never advances the recipient's cursor. A refusal that queues nothing retracts its pre-row with the standard withdraw tombstone.
