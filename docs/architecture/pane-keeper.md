...
A worker pane's pty master does not live in the mux server. It lives in a keeper process: `fno-agents-worker --pane`, spawned per pane by the server. When the server dies, the keeper and its child keep running. The next server on the same session re-adopts the same child instead of spawning a replacement. Plain panes are unchanged. The server holds their master directly, and they die with it, exactly as they always have. Every pane the server spawns through the resume path is keeper-hosted too: `resume_worker_into` spawns through the same keeper, so no worker pane the server creates dies with it. Giving them a keeper means reusing the canonical mesh spawn wrapper, which lives in the Python launcher.
