...
It now reads `load_registry` in process, in well under a second. There is no budget left to miss and no cache to age. An incomplete or unreadable registry still degrades the threshold away. Its reason names the registry, not a timeout that no longer happens.

The count itself changed, and the change is deliberate. `fno agents list --status live` matches the status string `live` exactly, after its own dedup and liveness filtering. The in-process read uses `LIVE_STATUSES`, the six-status set the spawn gate itself calls live. On one machine at 2026-09-04 the first returned 1 row and the second returned 49, over a registry holding 47 rows at `live` and 2 at `idle`. So the leak threshold now counts what the gate counts. Read a new leak verdict as a new denominator, not as a fleet that changed overnight.
