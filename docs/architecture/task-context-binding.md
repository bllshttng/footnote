...
- **Native gate** (`fno-agents task-context-gate`, stdin JSON: `node`, `root`, optional `binding`/`expect`/`env`): the declared-gate verdict table. With no `binding` in the request it loads the declared env path (`FNO_TASK_CONTEXT_FILE`, or the request's `env` map); nothing declared is an absent gate. Refusals are named with the `context_` prefix and carry the full native answer as `detail`: `context_stale_source`, `context_missing_source`, `context_wrong_node`, `context_binding_unreadable`. Both Python doors - init and receipt validate - shell this one verb and map refusals to their own outputs, so the verdict has one implementation.
...
- **Prepare** (`fno-agents task-context-prepare`, stdin JSON: `binding`, optional `out`): mints and digests a binding; with `out` it writes the bound file straight to the artifact slot (parents created). This is the one prepare door; the former `fno do resume receipt context-prepare` leaf was retired when the transport moved into the verb itself.
