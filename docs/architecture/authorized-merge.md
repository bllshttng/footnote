...
Every merge to main restales every other open PR at once, so nothing orders which one merges next without a slot. The first PR that `decide` holds on `ci_base_stale` while the slot is free takes it, and the receipt names the PR and says `merge_slot_held`. While a PR holds the slot, every other PR is `Held`, including one with fresh CI, so a racer cannot restale the holder's rebased checks. When the holder merges, closes, goes red, takes a dispatch hold, or its 60-minute lease expires, the slot releases. A hold frees it because a held holder can satisfy none of the other three. When an admitted holder arms instead of merging, it keeps the slot until its merge lands. `Effect::Arm` never enters the slot block in `decide`. The TTL covers one rebase, this repo's measured 31-minute rust-ci max, and one 10-minute sweep tick, with margin. The lease lives in the existing claims store at `<canonical repo>/.fno/claims`, keyed `merge-slot:<base_ref>`.
