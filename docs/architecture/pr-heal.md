...
A red whose verdict a rerun can change is rerun once before anyone calls it real. `cancelled` gets a full `gh run rerun <run>`: it reached no verdict, so the rerun is how it reaches one. The test-shaped signatures (`pytest`, `cargo-test`, `shard-rollup`, `smoke-step`) and anything `unknown` get `gh run rerun <run> --failed`, because a failure that passes on a second run was never the PR's defect. The deterministic classes (`rustfmt-drift`, `ruff-lint`, `mypy`, `closure-trailer`, `review-gate`, `guard-script`) never rerun: they fail identically every time, and the rerun is pure spend. The dedup key is `(head sha, run id)`, not the check name, because nine failing checks can share one workflow run and one `--failed` rerun covers them all. A second red on the same pair is the verdict: it escalates saying it failed twice on the same sha. A rerun that comes back green lands one `pr_heal_flake` row in the journal, keyed on the failing test when the log named one and on the check name otherwise; a key's third row files one backlog node, so a flake that reruns green forever is visible by construction. The `--playbook` table carries a `rerunnable` column, and it is the single source for the classification.
...
Two rebase triggers run ahead of classification, because a push restarts CI and classifying the old sha's checks would classify a dead run. A PR whose `mergeable` is `false` is conflicting and stuck by definition. A PR holding the `merge-slot:<base>` claim (the one the merge sweep named as next) is behind by the slot's own verdict. Both run `fno do pr push` from the PR's own worktree: fetch, rebase onto `origin/main`, preflight, push once, one receipt. A conflicting PR whose node a live worker holds is never rebased; the claim refusal runs first. A run is capped at six rebases, so a leaked trigger stops the loop instead of restarting CI on the whole fleet. A conflict files one deduplicated inbox question against the branch's node, naming the conflicting files and `fno do pr rebase <n>`. Any other push failure falls through to the normal heal path, because a rebase that could not run is not a verdict about the PR.

One invocation emits one `pr_heal_tick` row into the global `~/.fno/events.jsonl`. The row carries the counts: PRs seen, healed, `rebased`, `reran` with the `rerun_keys` the once-per-(sha, run id) guard reads back, skipped by reason, unknown signatures, escalations, the PRs acted on, and `duration_s`. Every PR also gets one `pr_heal_pr` row per run carrying the action taken and the blocking reason, so no PR sits for more than one tick without a receipt saying why. `fno doctor event find --field type=pr_heal_tick --since 24h` reads it. `--all --apply --dry-run` rehearses every refusal and prints the plan without touching a worktree or the inbox.
