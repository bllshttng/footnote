...
The mux sideline delete, `fno agents rm`, and `claude rm` are one chain. The cascade is Claude-only: Claude's UUIDv4 short-id join is safe, while a Codex UUIDv7 head-8 is a roughly 65.5-second clock bucket and can collide. `claude rm` removes the session's worktree under Claude's guards, which is why it is the callee instead of the agent-view Ctrl+X path; agent view discards an uncommitted worktree without refusing.
