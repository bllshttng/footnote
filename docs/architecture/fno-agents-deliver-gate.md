...
## A busy claude session enqueues, it does not corrupt

This document's injection gate governs the PTY lane (codex/gemini). The claude lane never goes through it. Claude is always socket-delivered (Locked Decision 9). Both lanes historically shared one belief. A mid-turn paste corrupts a busy recipient's composer, so delivery had to wait for idle.

A dedicated node measured that belief false for the claude socket lane. The proof came directly off a live session's own transcript. A busy claude session does not corrupt its composer on an injected paste. It enqueues the paste and processes it at the next turn boundary. The evidence is a `queue-operation`/`enqueue` record. It gets written to the recipient's transcript at submit time, not at turn end:

```json
{"type":"queue-operation","operation":"enqueue","content":"<the injected turn's content>"}
```

This differs from the `queued_command` shape a submitted, turn-processed message eventually lands as. The `enqueue` record confirms the paste arrived, before the recipient's own turn finishes.
