...
`inference_samples` closes that second gap. No stub write can forge a vendor turn. So a fresh age with zero model-bearing records stops certifying liveness, and the verdict reads `unknown` with basis `no-inference`. Only `observed_model.kind == "no-model-yet"` is that zero. `no-transcript`, `not-file-backed` and `unreadable` are absences and lower nothing. docs/architecture/fleet-watchdog.md carries the measurement.
