...
Liveness is pure pid-anchored with no TTL, the `reconcile_lane_slot` shape. The pid is the durable harness session pid from `resolve_session_pid`. A dead worker's task frees the instant its process is gone. `acquire_claim`'s stale-recovery step archives the corpse and retries. No heartbeat is needed. The holder is the FULL harness session id (`resolve_self_session_id`), never a head-8 handle. A codex UUIDv7 head-8 is a ~65.5s clock bucket. Two codex workers spawned in one minute then share a handle. The second re-acquires the first's task as idempotent.
...
`fno agents history <node|pr|session>` resolves one argument against three sources in one grouped answer: live registry rows, reap receipts, and the ledger. It answers the operator's question - what did this session work on, and how do I get back to it - as a lookup rather than a content grep across transcript stores. `fno whoami ledger` stays as a hidden alias over the same shared matcher, so nothing in flight breaks; the store keeps its file name, the verb names the question. The receipt's `resume` string prints verbatim: it was rendered from the capability table at reap time, and re-deriving it would answer a different question if the table moved since. A live row suppresses its own receipt, because a receipt is written as the row drops and the two never both describe the present.

Three facts worth recording, because an earlier dispatch brief planned against all three. First, the retained-row store already ships: the GC sweep persists one `ReapReceipt` per reaped row at `~/.fno/reap-receipts/<harness>-<session id>.json` (PR 1326), carrying the row's own fields, a resume command rendered from the capability table, and ledger enrichment when one exists. The brief's premise that reaping without a new store destroys the resume handle is false: `build_reap_receipt` refuses a row with no session identity or no declared resume form, and a refused row is HELD in the registry (`kept_no_receipt`) instead of reaped. Unknown never reaps, so a reap cannot destroy a resume handle. Second, the identity gate is closed: PR 1278 put a resolvable `harness_session_id` on every row at spawn, whatever the harness, so the receipt gate has something to build from on every lane. Third, what was actually missing was a reader and an expiry, and both are the same verb: `fno agents history` for the reading, `config.agents.reap_receipts.retain_days` (default 7) for the pruning, expired by the same GC sweep that writes new receipts. A receipt whose `reaped_at` is missing or unparseable is kept and named in the sweep summary - a failed read is not evidence of age.

Every miss names its source. Ledger coverage measured 2026-09-01 over 3647 rows: `session_id` on 3526, `model` on 800, `provider` on about 130, `harness` on none, and an explicit `node_id_unrecoverable: true` on 11. The uuid write path never backfilled, so a lookup against an old row legitimately finds nothing; the verb prints `not recorded` with the reason instead of an empty answer that a stranger would read as an absence of work.
