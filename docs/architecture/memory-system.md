...
**The corpus predicate.** The pointer writer `append_corrections_pointer` refuses a postmortem path that does not lie under the postmortems root of the same home the corrections log resolved through (`real_session.rs::is_real_run`; FNO_HOME replaces `~/.fno` wholesale, mirroring `corrections_log_path`). Containment, never a `/tmp` name match: a run whose whole FNO_HOME sits in a temp dir under test is real relative to that home. Before this guard, a unit test's temp-dir postmortem fell through the log ladder to the real `~/.fno/corrections.log`, and 360 of 418 live rows (86 percent) were such fixtures whose files no longer existed. The legacy rows are not deleted - the log is append-only - so the read side skips them too: `autocorrect-pack.sh` counts a vanished path outside the postmortems root into `skipped_fixture_rows` and never renders it into `implicated_rules`.

**The skill-diff leg.** The packet now carries the verb's skill: a row's SOURCE `target-postmortem` strips to `target`, resolves `skills/<verb>/SKILL.md` in the repo, and lands in `implicated_rules` with its full text, so the review can propose a diff to the skill itself. The commit-time leg closes the loop: the corrections post-commit hook resolves its SOURCE from the committing repo (`git-rule-edit` in `~/.claude`, `skill-commit` anywhere else, installed by the shared post-commit dispatcher `scripts/setup/setup-worktree.sh` writes), so a hand-applied skill correction in this repo produces the applied row `corrections-verify` scores against. The distress vocabulary gained the other word: a `RESULT: BLOCKED` return (line or JSON form) emits the same deduped `blocked` event row the `<help>` tag emits, discriminated by `data.kind` (`help` | `result_blocked`), so both run outcomes are countable in the same fold.
