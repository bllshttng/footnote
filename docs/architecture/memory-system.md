...
Two retirement rules close the pipeline, because landed artifacts had no exit and piled up. `classify_postmortem` consumes every success terminal (`DonePRGreen` and every other `Done*` reason) instead of filing it: a clean session's eval is a receipt, the file on disk stays the monthly review's input, and the wedge regex must never see these gists - the pasted assistant prose matches it constantly, which is how 35 of 62 landed receipts were `DonePRGreen`. And the daily archive sweep (`retire_stale_postmortems` in `graph/archive.py`) closes any landed receipt still sitting as an open `idea` after 30 days untouched, marked `retired: stale-postmortem-receipt`, so the sweep's normal age gate removes it from the working graph like any terminal row. Rows a human or the ack flow acted on (queued, claimed, deferred) are never touched.
...
**The corpus predicate.** The pointer writer `append_corrections_pointer` refuses a postmortem path outside the postmortems root of the same home the log resolved through. The root lives in `real_session.rs::is_real_run`. FNO_HOME replaces `~/.fno` wholesale there, mirroring `corrections_log_path`. The test is containment, never a `/tmp` name match. A run whose whole FNO_HOME sits in a temp dir under test is real relative to that home. Before this guard, a unit test's temp-dir postmortem fell through the log ladder to the real `~/.fno/corrections.log`. That leak wrote 360 of 418 live rows, all pointing at files that no longer exist. The legacy rows stay in the log. The log is append-only. The read side skips them instead. `autocorrect-pack.sh` counts a vanished postmortem-sourced path outside the root into `skipped_fixture_rows` and never renders it into `implicated_rules`. Dead paths from other sources land in `skipped_dead_rows`, so corpus poisoning stays readable beside ordinary file aging.

**The skill-diff leg.** The packet now carries the verb's skill. A row's SOURCE `target-postmortem` strips to `target` and resolves `skills/<verb>/SKILL.md` in the repo. The skill lands in `implicated_rules` with its full text, so the review can propose a diff to the skill itself. The commit-time leg closes the loop. The corrections post-commit hook resolves its SOURCE from the committing repo. `git-rule-edit` stays for `~/.claude`. `skill-commit` names an edit in any other repo. The dispatcher `scripts/setup/setup-worktree.sh` writes installs the hook. A hand-applied skill correction now produces the applied row `corrections-verify` scores against. The distress vocabulary gained the other word. A `RESULT: BLOCKED` return, in line or JSON form, emits the same deduped `blocked` event row the `<help>` tag emits. `data.kind` carries `help` or `result_blocked`, so both run outcomes are countable in the same fold.

## The S2 leg: intel feeds the corrections log

The autocorrect loop's corrections log had a starving leg. The S2 path (`scripts/corrections-insights-tag.sh`) ingests `#agent-correction` lines from an insights report, and no writer produced one until `fno-agents intel` landed. The fold (`crates/fno-agents/src/intel.rs`, report skill `/fno:intel`) classifies every user-shaped transcript turn by provenance first: operator, relay, harness, keepalive. The corrections the skill quotes are therefore the operator's own words, not the mail bus the agents write to each other in. Each correction line carries `signal=<friction category>`, which is the field the verifyDecisions lane scores. The full loop map, including the config keys for the evals and heal legs, is [self-improvement-loops](self-improvement-loops.md).
