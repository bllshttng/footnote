...
`runtime.list_worktrees` accepts worktrees under either the canonical base or the legacy base through the transition window. `fno agents workspace worktree status` keeps surfacing legacy entries until they are torn down.
