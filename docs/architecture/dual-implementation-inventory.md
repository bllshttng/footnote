...
Six live duals below, plus one retirement recorded where it landed (the watchdog reap and retire lanes, 2026-09-04). The ask adapters, the claim classifier and the graph store are retired (2026-09-02), the review-freshness mirror followed (2026-09-03), and the ready-selection dual is retired (2026-09-08). The native graph reader remains the confirmed dual closest to a port, with seven port-owed duals named below.
...
**Process-liveness readers.** Dual, disposition dual-logic, port owed, not in this PR. Python leg `census` in `cli/src/fno/agents/spawn_gate.py`: the `fno agents top` audit surface and the Python admission gate's own reading. It reads psutil pid plus start time, the claude roster pid, a live worker slot claim. Rust twin `slot_count` at `crates/fno-agents/src/spawn_gate.rs`, pinned to the Python census by the shared fixture in `cli/tests/agents/test_spawn_gate_agreement.py`. The sweep leg is `plan_reconcile` in `crates/fno-agents/src/liveness_sweep.rs`. Its served pair reads the freshness rule in `crates/fno/src/served_liveness.rs`. The rule is vendored into fno-agents and pinned by a contract test. A real dependency waits on a synchronized 0.3.x release: `cargo publish` resolves the dependency against the registry copy, which predates the module. Both gates answer "is there fleet headroom" from their own process reading. The sweep measures the same rows, so a saturated machine can make the two disagree by construction. The port moves both admission gates onto the served measurement, and that is gate semantics: who gets admitted on a saturated machine changes. It is recorded as owed, not taken here. After the x-1b75 port, `fno agents registry-json` is also a consumer of the served measurement: the Rust client derives the two served fields from the same freshness rule, and both counting hooks read that verdict instead of running a status-word filter of their own.
