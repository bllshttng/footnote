...
**Process-liveness readers.** Dual, disposition dual-logic, port owed, not in this PR. Python leg `census` in `cli/src/fno/agents/spawn_gate.py`: the `fno agents top` audit surface and the Python admission gate's own reading. It reads psutil pid plus start time, the claude roster pid, a live worker slot claim. Rust twin `slot_count` at `crates/fno-agents/src/spawn_gate.rs`, pinned to the Python census by the shared fixture in `cli/tests/agents/test_spawn_gate_agreement.py`. The sweep leg is `plan_reconcile` in `crates/fno-agents/src/liveness_sweep.rs`, whose served pair `fno agents list --json` serves through the freshness rule in `crates/fno/src/served_liveness.rs`. Both gates answer "is there fleet headroom" from their own process reading. The sweep measures the same rows, so a saturated machine can make the two disagree by construction. The port moves both admission gates onto the served measurement, and that is gate semantics: who gets admitted on a saturated machine changes. It is recorded as owed, not taken here.
