...
**`pending_supersession_reason`.** Dual, disposition dual-logic, port owed, not in this PR. Python leg `cli/src/fno/graph/statuses.py:220`, Rust leg `crates/fno-agents/src/graph_store.rs:770`. Both answer one question - does this supersession lack merged-PR proof - and both are correct today; recorded when the x-e451 edge settlement landed beside it. A port deletes the Python leg and converts the guard to characterization; the readiness predicate it sits beside is already Rust-only.
