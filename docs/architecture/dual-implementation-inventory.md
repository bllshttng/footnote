...
CLOSED (2026-09-14, x-3873, law d-496680aa). The pair is now single: `fno agents dispatch next` - the keystroke's leg, selection plus resolver plus one shellout - is deleted, and the mux keystroke resolves the board itself and shells `fno agents spawn` directly. `dispatch resolve`, `capabilities` and `family` went with it; the two read leaves live on as hidden Rust verbs (`fno agents capabilities`, `fno agents target-family`). The shell dispatcher (`dispatch-node.sh`) passes the node and only what the human typed. One launch verb, and it is spawn.
