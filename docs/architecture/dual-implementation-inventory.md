...
**The spawn gate.** Dual, disposition dual-logic, RETIRED (2026-09-13, x-6089). The legs: `cli/src/fno/agents/spawn_gate.py` `run_gate` (pane, `--route`, account and wake revival) and `crates/fno-agents/src/spawn_gate.rs` `run_gate` (the unrouted bg/headless arms, called from `bin/client.rs`), plus SIX Python re-readers of the same answer: `probe_capacity`, `explain.gates_for`, `top.lane_rows`, `route_resolve._vendor_tables`, advance's dispatch width, and doctor_lanes' share reading. The Rust gate was a strict subset - no provider cap, no account quota lock, no king share, no registry schema - so any new conjunct had to be threaded through eight Python files. The port made the Rust gate the ONE gate: it owns the axes (fleet incident, schema, quota lock, provider cap, CPU, slots, RAM, king share) in the exact order the Python gate evaluated them, and the `spawn-gate` verb answers `gate` and `probe` modes over one JSON round trip. The Python `run_gate` kept its signature and became the transport, so the twenty-plus test files that monkeypatch it stayed valid; `probe_capacity` is the verb's probe mode. The provider counter (`provider_live_count` and its two helpers), `share_reading`, `_king_share` and the refusal helpers are DELETED; `census` survives for the top, orphans and unfinished-work display surfaces. Surviving Python names and their callers: `census`/`LiveCensus` (`fno agents top`, orphans, unfinished work), `_pid_alive` and `_roster_path` (census), `available_ram_gb` (census callers and tests), `_cpu_axis`/`_prefetch_fleet_reading`/`_load_cpus`/`_load_snapshot` (doctor_lanes' advisor arm and footprint tests), `provider_lanes_cap` (scoreboard/fold, advance), `GateGuard`/`consume_provider_admission` (the spawn seam and the wake revival's `retain_revived_worker`), `_refuse`/`_emit_gate_event` (the one refusal-emitting seam), `provider_lanes_cap` again for the config vocabulary, and the QoS helpers. The axis tests moved to Rust beside the code (`spawn_gate.rs`, `spawn_gate_lanes.rs`, `spawn_gate_verb.rs`); the Python suites stub the verb and assert the transport contract (`test_spawn_gate.py::TestTransport`) and the probe answer (`test_gate_status.py`, `test_agents_top.py`, `test_spawn_headroom.py`). The exit-code table gained one number, 86 `gate_unavailable`, byte-parity in both trees; 82/83 fleet, 84 state root and 85 sandbox kept their merged meanings (x-0460).
