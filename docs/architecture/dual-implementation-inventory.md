...
**`claims_root_for`.** Dual, disposition dual-logic, port owed, not in this PR. Python leg `cli/src/fno/claims/io.py` (`claims_root_for`, the canonical prefix list `_GLOBAL_ID_PREFIXES`), Rust leg `crates/fno-agents/src/claims.rs` (`claims_root_for`, the hand-copied `GLOBAL_ID_PREFIXES`). Both answer which claims root a claim key coordinates on, and the answer is a routing decision, not shared vocabulary: a prefix present in one list and not the other sends the writer and the reader to two different files while each believes it won. That divergence shipped for real: the spawn-gate mutex key was colon-less (unrouted, same answer on both sides, `None`), and `groom`/`update` sat in the Python list alone, so the Rust verbs read `<space>/claims` where Python read the global root. The parity guard `cli/tests/unit/test_claims_root_routing.py` holds the lists equal today. Writing that guard is itself the port trigger: a port resolves the key's root where the claim is minted and deletes one list.
