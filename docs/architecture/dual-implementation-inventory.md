...
**The spawn gate.** Dual, disposition dual-logic, RETIRED (2026-09-13, x-6089). The legs were `cli/src/fno/agents/spawn_gate.py` `run_gate` and `crates/fno-agents/src/spawn_gate.rs` `run_gate`. The Python leg covered pane, `--route`, account and wake revival. The Rust leg covered the unrouted bg/headless arms from `bin/client.rs`. SIX Python re-readers recomputed pieces of the same answer: `probe_capacity`, `explain.gates_for`, `top.lane_rows`, `route_resolve._vendor_tables`, advance's dispatch width, and doctor_lanes' share reading. The Rust gate was a strict subset. It had no provider cap, no account quota lock, no king share, and no registry schema check. Any new conjunct had to be threaded through eight Python files.
