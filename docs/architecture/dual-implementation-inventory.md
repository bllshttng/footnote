...
**The node-dispatch launchers.** Dual, disposition dual-logic, RETIRED (2026-09-12). This pair is Python/Python - the register's first - and the disposition still decides it: two hand-written implementations of one behavior (launch a worker for a backlog node), and "two languages" was incidental to the discriminator all along. The legs: `cli/src/fno/dispatch.py` `_dispatch_one` (the mux keystroke's leg: a hardcoded claude harness, a hardcoded pane substrate, a direct `dispatch_spawn_bounded_pane` call, and an inline copy of the family-2 guard, the spawn gate and the release choreography) and `cli/src/fno/backlog/advance.py` `_spawn_worker` (resolve preferences, then shell `fno agents spawn`). They had already been caught diverging twice: the launch-time account picking incident recorded at `autonomous_route.py` (a hoisted shared function, then a parity test to hold the legs equal), and the harness answer - the same node dispatched by the keystroke and by `fno backlog advance` landed on different harnesses and models. The port (x-e53e) makes `agents.node_dispatch.resolve_node_spawn` + `node_spawn_argv` the ONE preference path and `fno agents spawn --node <id>` the ONE launcher; `fno agents dispatch next` (renamed from `one`, hidden alias kept) is selection + the resolver + one shellout, and advance keeps the resolver plus its own shellout. Deleted with the leg: the hardcoded harness, `_cutover_command`, the inline guard/gate/release pair, and the parity test `test_route_tuple_identical_across_launchers` (`cli/tests/unit/test_advance.py`), which existed only to hold the two legs equal - leaving it would have been the second-tenure move working principle 9 forbids. Surviving callers, so no predicate is deleted quietly: `select_autonomous_route` (advance's route read and `dispatch next`'s quota route), `_next_node` (`dispatch next`'s selection; the same board order `fno backlog next` uses), `_worktree_ensure_for_launch` (`dispatch next`'s launch cwd), `_grid_lane_for` (the capacity read inside `resolve_node_spawn`), and `resolve_dispatch_brief` (advance's brief chain and the spawn door's `--node` seed).
