...
The spawn seam runs after defaults resolve and before either the Python dispatch or Rust client can mint a worker. When the harness is Codex, the payload is a bounded code workflow (`/target`, `/execute`, `/tdd`, `/fix`, or `/pr`), and the launch is not full-access, the seam resolves `git rev-parse --path-format=absolute --git-common-dir` for the launch cwd. An empty result exits 2 with the cwd and query named, and no registry row is created. A resolved path launches normally; full-access launches do not need this bounded-grant assertion. The predicate is the effective grant, not a required posture flag.

The watchdog consumes the distress that target initialization already writes. A last message naming a path under `.git` together with `Operation not permitted` becomes `sandbox-blocked` only when the node claim reads free and `origin/main..HEAD` contains zero commits. Its action is `reap`, which frees the parked worker slot. An unreadable claim or branch, a held claim, or any branch commit holds the row and the report names that guard instead of reaping on a stray string.

The measured failure had fourteen workers with logs carrying one `.git` lock signature and the correct distress explanation; nobody read the distress. The row's posture alone could not distinguish a usable sandbox from the one that failed. The grant path and watchdog verdict make both facts durable and machine-readable.
