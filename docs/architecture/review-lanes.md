...
One boundary on where the refused state surfaces. The row is scoped by the REVIEWER's own checkout branch and head, so it mints a refused verdict on a PR's coverage row only when the attempt ran in that PR's own branch checkout. A mis-aimed attempt (the incident shape: a session on the base branch) journals its terminal where the attempt ran, and no PR reads it as its own state, because nothing in the raw lane carries the PR's identity: the fire-side guard is what stops that shape, before the RPC. Attributing a foreign-checkout refusal to the PR it was aimed at needs the invocation-id join to the sent row, which is future work, deliberately not guessed here: attributing it wrongly marks every PR against that base as refused.
