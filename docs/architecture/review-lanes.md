...
- `carried_subset` - the code delta only shrank. Every raw line still shipping is byte-identical to one the reviewer read. The vanished lines are paths the base absorbed on the rebase.
...
A subset is decided one level down, on the sorted raw lines kept beside the hash. The HEAD set contained in the reviewed set is a shrink. A line the reviewer never saw is new unreviewed code.
