...
The fix is the `branch` field on `review_attestation` plus one predicate, `attestation_in_scope`, applied by both scans (`local_latest_passes` and `unattested_reviewers_scan`) before any freshness call. An attestation naming the PR's head branch is in scope. An event predating the field counts only on exact head equality. A foreign branch cannot share this head sha without being this commit. The legacy line deliberately does not inherit the carry: an attestation on a moved head cannot be scoped to any PR.
