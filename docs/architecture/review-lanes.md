...
One boundary on where the refused state surfaces. The row is scoped by the REVIEWER's own checkout branch and head. When the attempt ran in the PR's own branch checkout, the PR's coverage row mints the refused verdict. When the attempt ran in a foreign checkout (the incident shape: a session on the base branch), the terminal lands in the journal where the attempt ran. No PR reads it as its own state, because nothing in the raw lane carries the PR's identity. The fire-side guard is what stops that shape, before the RPC. Attributing a foreign-checkout refusal to the PR it was aimed at needs the invocation-id join to the sent row. That join is future work, deliberately not guessed here. Attributing it wrongly marks every PR against that base as refused.
