...
A rebase replaces every sha on the branch, so a tiling keyed by sha alone would reset coverage on every rebase the merge gate forces. The producer carries a tile across a head move when `review_freshness` proves the attestation's pinned head still IS the current head by content identity: git content on both sides, base divided out. What grants it is a whole-range verdict only, `fresh`, `carried_base_sync`, `carried_docs_only`, `carried_subset`, or `carried_interdiff` at zero lines. `carried_interdiff` above zero still counts as a review under law d-608344c1 but grants no tile, because a tile asserts every shipping line was read. Each carry is named on the row as `{head, freshness}` in `range_tiling.carried`.
