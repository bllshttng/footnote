...
How a diff gets reviewed: the fno-owned review lane by default, and the
harness's native review verbs (Claude `/code-review`, codex `/review`) as
operator-chosen alternates, with the constraints each trigger lane carries.
...
(which covers the cross-model peer lane, a different surface).
...
The retirement measurement, recorded so nobody rebuilds the panel from
first principles: PR 1170 walked six rounds and produced 84 findings, of
which 8 were real - six readers of one diff yield six sets of objections,
not six times the defects, and one inline adversarial reviewer caught all
8 real defects in three rounds. The six specialist hunters remain
individually invocable as agents; only the six-at-once panel is gone.

The load-bearing correction this doc carried before the lane existed: the
widespread belief that `/code-review` "cannot be self-invoked by the
session that wrote the diff" was too strong then, and the owned lane
makes it moot now - the session reviews its own diff inline, dispatches
zero review subagents, and emits the same head-pinned attestation a
native pass produces.
...
`fno.config.assert_subagent_budget` is the fan-out-neutral guard: any skill that DECLARES a dispatch width in its text calls it before dispatching (`fno config assert-subagent-budget --width <n>`), and a refusal names the provider, the width and the budget. The default review lane needs no such check - it runs inline and dispatches zero review subagents, so a budget of 1 costs it nothing. The retired panel's route-substitution branch (`sigma` unavailable under a low budget, resolving to `code-review`) is gone with the panel: the lane IS the default on every harness, and `sigma` as a configured name is refused at init with the lane named as the replacement, so the gate never again consumes a name the route cannot produce.

One boundary stays, stated rather than papered over. A native subagent is a harness-internal Task call; fno never observes it, so the guard is a DECLARATION check plus the skill text that carries it, not a hard cap on a native dispatch. Total prevention needs a harness hook that does not exist. Where fno routes the dispatch (spawn, the owned lane), the budget is a real cap.
...
The fno review lane calls the script itself, from its emit step, on a clean pass (`pass` only when the classifier yields zero blocking findings). `hooks/code-review-attest.sh` calls it for native code review on Claude and Codex too.
...
The round budget is `config.review.max_rounds` (default 2). A round is a review VERDICT on the branch since the last pass. CI failures, lint failures and rebases are not rounds. A pass resets the counter. A PR merges after one to three reviews and never waits for a clean round. The full statement, with the honest-limits contract, is [review-coverage-termination.md](review-coverage-termination.md). That contract covers what class-gating does and does not close, the CONFIRMED axis, and GitHub's per-identity limit.
