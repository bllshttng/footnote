...
One of these lanes is obligatory on a code payload. The stop gate (`crates/fno-agents/src/loopcheck.rs`) and `fno do pr merge` enforce that obligation, not just this prose. A code PR that reaches the gate with no head-pinned `review_attestation` is held. The held reason names this harness's verb. This doc is the lane menu, and the gate is the authority. Opt out with `config.review.self_review_required = false` only while the global claim-backed opt-out lease is live.

## Merge-gating opt-out leases

The merge-gating opt-outs are `review.self_review_required = false`, an explicit empty `review.optional_apps = []`, and `auto_merge.require_checks_pass = false`. Each successful `fno config set` acquires a global `config-optout:<key>` claim for `review.optout_ttl_minutes` (default 60, maximum 24 hours), and the command prints the holder, expiry, and release command. A second holder cannot overwrite the live lease.

Readers enforce the lease at read time. A missing, expired, stale, suspect, corrupted, or unreadable claim makes the opt-out inert and restores the gate's default direction. The reaper restores the file value when it can, and `fno doctor` reports residue when it cannot. This read-time rule is the guarantee because the session that set an opt-out can die before cleanup runs.

While any opt-out claim is live, unattended auto-merge is disarmed. The deliberate path remains available to the session holding the lease through `fno do pr merge`; ambient `DonePRGreen` finalization does not arm GitHub's native auto-merge. Release the setting or wait for the lease to lapse before allowing unattended merge.
...
Both now call one predicate, `review_freshness(reviewed_sha, head_sha)`. It lives in `crates/fno-agents/src/review_freshness.rs` (moved there from `loopcheck.rs`, which is over the file budget and shrink-only; the module is named by its question) and returns one of six states:
...
- `carried_interdiff(n=<lines>, cap=<bound>)` - the code delta changed, but by fewer than `review.carry_interdiff_lines` lines (default 100, the attestation law d-608344c1's threshold). The measure is the multiset symmetric difference of the two PR-code patches against the base, never a direct head-to-head diff, so base movement contributes nothing. This is the small-change arm: a rebase whose conflict resolution touched three lines carries instead of costing a full round. `None` never carries - an unreadable patch read on either side is `stale` whatever the identities say - and `0` in config disables the arm.
...
`fno do pr coverage-check` answers one of four states. COVERED is exit 0. REFUSED is exit 3, and the refusal names the failing conjunct and the rounds remaining. UNANSWERED is exit 4, a named instrument failure, never a verdict. IMPOSSIBLE is exit 5. The round budget is spent and a HARD finding is still non-terminal: a CONFIRMED correctness or security finding, or a truncated remainder. Re-reviewing cannot clear it. Every other finding open at the cap is filed as a node and the PR merges. The sentence names the two acts that can: a non-author GitHub approval, or the coverage-override label. Every merge surface honors exit 5. The merge verb refuses with its own receipt name. `fno do pr status` renders `review_coverage_impossible`. The git-protection hook's refusal set is `{3, 5}`.
