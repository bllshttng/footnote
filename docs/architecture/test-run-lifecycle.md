...
Each run does the same five things in order. First, a whole-suite run acquires the suite claim, and a nested run never does (see below). Second, spawn the argv as the leader of a fresh session via `setsid()`. It and everything it forks then share a pgid the owner never touches. Third, wait for the argv, bounded by the timeout. The timeout counts from admission, so a run that queued keeps its whole budget. Fourth, unconditionally `killpg` the group: SIGTERM, then SIGKILL after a grace window, confirmed empty by `killpg(pgid, 0)` rather than inferred from the leader's own exit. Fifth, release the claim.

The suite claim is whole-suite-only (user ruling 2026-09-25: a worker runs only the tests covering the files it changed, and CI catches the downstream breaks). `selects_direct_set` in `crates/fno-agents/src/test_run.rs` reads a DIRECT set from two shapes. One is a cargo test narrower than a whole crate. The other is a pytest run naming a test file or a `path::nodeid`. A directory target, no target at all, a flag value, or any opaque program reads whole. A direct-set run skips the claim entirely and never queues behind a suite holder. Everything else queues as before: the default suites, a bare `cargo build` handed to the owner, anything unclassifiable. A whole-suite local run belongs in a background task, so its queue wait never blocks the worker's turn.
