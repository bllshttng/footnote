...
Each run does the same five things in order. First, acquire the suite claim - only when the run selects a whole suite, and never when this run is itself nested inside another (see below). Second, spawn the argv as the leader of a fresh session via `setsid()`. It and everything it forks then share a pgid the owner never touches. Third, wait for the argv, bounded by the timeout. The timeout counts from admission, so a run that queued keeps its whole budget. Fourth, unconditionally `killpg` the group: SIGTERM, then SIGKILL after a grace window, confirmed empty by `killpg(pgid, 0)` rather than inferred from the leader's own exit. Fifth, release the claim.

The suite claim is whole-suite-only (user ruling 2026-09-25: a worker runs only the tests covering the files it changed, and CI catches the downstream breaks). `selects_direct_set` in `crates/fno-agents/src/test_run.rs` reads a DIRECT set from a cargo test narrower than a whole crate, or a pytest run naming a test file or a `path::nodeid`; a directory target, no target at all, a flag value, or any opaque program reads whole. A direct-set run skips the claim entirely and never queues behind a suite holder. Everything else - the default suites, a bare `cargo build` handed to the owner, anything unclassifiable - queues as before, and a whole-suite local run belongs in a background task so its queue wait never blocks the worker's turn.
...
- A rustc started by a build script never asks. When cargo runs a build script, it sets `CARGO_CFG_*` for that run only. The wrapper sees the variable there and nowhere else. That cargo is already admitted. Asking again is how the 2026-09-23 deadlock waited 1h49m on its own ancestor's claim.
...
- Admission fails open, named out loud. With no `fno-agents` on PATH the wrapper builds silently. With an older binary that lacks the verb, the line names the remedy: `Run: fno doctor update`. The wrapper also writes a `build_admission_unavailable` event with `reason: verb_missing`. A verb that fails for any other reason keeps the plain `unavailable (exit N)` line and writes `reason: error`. The runner door writes `run_admission_unavailable` the same way. `fno doctor event find build_admission_unavailable` reads them.
...
- Admission fails open at the runner door, exactly as at the compile door. No `fno-agents` on PATH runs the binary silently. A binary without the verb names `fno doctor update`, and the wrapper writes a `run_admission_unavailable` event with `reason: verb_missing`. One that exits below 128 keeps the `run admission unavailable (exit N)` line with `reason: error`. The binary runs anyway.
...
An fno-agents cargo test can create the same shape through `guard_birth`. The pytest reaper cannot see Rust tempfile roots. The Rust side closes this at birth. `guard_birth` refuses a real supervisor for a config dir under the OS temp dir. A lib unit-test build never births one.
