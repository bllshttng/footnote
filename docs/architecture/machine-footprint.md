...
The process census is compared only with `process_admission.max_processes`, carried by Python as `FNO_PROCESS_ADMISSION_MAX` and defaulting to 400 processes in both runtimes. `agents.max_live` remains an agent count and never enters this channel. Python separately carries the pane-valued `FNO_MUX_PANE_GROUP_MAX`; every `PanePlacement` carries `max_panes`, and a legacy client that omits it receives the conservative configured/default pane cap. Process and pane ceilings remain separate by config key, wire, Rust type and constant.
