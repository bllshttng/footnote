...
- **Mission drain tick** (`crates/fno-agents/src/active_backlog.rs`, `mission_drain_tick` / `dispatch_mission`). Reconciles prior fire-and-forget dispatches from events (feeding the breaker) and then dispatches by shelling `fno backlog advance --epic <id> --continuation --json`. The converge core owns all dispatch policy, so this tick never forks it. Policy covers cross-project fan-out, per-root `walker:` respect, spawn-gate headroom width, and claim dedup. The walker probe runs per child, against that child's own root (x-7f1f). One live walker in the epic repo refuses only its own repo's children. The rest of the pass still dispatches. `--continuation` means the daemon never reactivates a mission and retires an already-inactive one. An operator `--stop` between ticks sticks. A `deactivated`/`all_done` receipt retires the loop (`active_backlog_mission_retired`).
