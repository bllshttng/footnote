...
4. Fire-and-forget dispatch: `fno agents spawn` -> `/target [--no-merge] <id>`, the `--no-merge` flag gated on `config.auto_merge.grant`.
...
6. advance never merges the PR itself; it dispatches a worker whose merge posture comes from `config.auto_merge.grant` (default `none`). An actual merge, when enabled, is still gated by the worker's own `config.auto_merge.*` review layer (revisits epic LD#4).
