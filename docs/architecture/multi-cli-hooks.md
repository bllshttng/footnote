...
**One runner for the fno producers (x-21e1).** Claude and Codex register `hooks/context-run.sh <group>` instead of one entry per producer. The groups live in `hooks/context-hooks.json`, and `fno-agents context-run` starts every producer of the group at once, joins their directive text into the output contract above, and appends one `context_snapshot` per invocation with per-producer byte accounting. A producer that fails or hangs past its bound is recorded `unreadable` without changing the others' output.
