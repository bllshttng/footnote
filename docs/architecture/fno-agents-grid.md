...
- a 3s `ping` tick that, under `--all`, runs `poll_live_discover`: it re-reads the registry and live-adds any externally-spawned child (`fno agents spawn --cwd`, fresh `claude --bg`) without a restart. A claude row is admitted to a PTY pane only if it answers `worker.ping` - the authoritative split between a real PTY-hosted interactive claude and an adopted stream-json lane, which also sets `host_mode=interactive` but serves only `stream.*` and so fails the probe. **The probe runs on this same `select!` arm, so a slow/serial probe starves key input.** A stable stream-json lane fails the gate forever and was re-probed every tick at a 500ms timeout, freezing input on a busy fleet; a later fix addresses this with a `FAILED_PROBE_TTL` negative-cache (a failed name is skipped for 60s) plus probing the candidates concurrently via `join_all`, so steady state probes nothing and a burst caps at one ~500ms window.
...
## Squads (manual cross-repo teams)
...
- **"agent X not found"**: no registry row named X (often a name-vs-short_id mix-up: `fno agents spawn --name codex-bot` registers the *name* `codex-bot`; `codexbot` is its short_id, and the grid resolves by name), or an `ask`-driven subprocess with no PTY worker.
...
fno agents spawn "explore the repo and write a detailed review" --name pr-review --harness codex
