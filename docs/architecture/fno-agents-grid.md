...
- a 3s `ping` tick that, under `--all`, runs `poll_live_discover` (x-45e6): it re-reads the registry and live-adds any externally-spawned child (`fno agents spawn --cwd`, fresh `claude --bg`) without a restart. A claude row is admitted to a PTY pane only if it answers `worker.ping` (x-57eb) - the authoritative split between a real PTY-hosted interactive claude and an adopted stream-json lane, which also sets `host_mode=interactive` but serves only `stream.*` and so fails the probe. **The probe runs on this same `select!` arm, so a slow/serial probe starves key input.** A stable stream-json lane fails the gate forever and was re-probed every tick at a 500ms timeout, freezing input on a busy fleet; x-c226 fixes this with a `FAILED_PROBE_TTL` negative-cache (a failed name is skipped for 60s) plus probing the candidates concurrently via `join_all`, so steady state probes nothing and a burst caps at one ~500ms window.
