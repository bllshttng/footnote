...
- `gate` runs the full admission gate. The payload is `{mode, name, substrate, force, no_wait, route_provider, account, caller_session, succession_scope, holder_pid}`. An admitted answer is `{status: "admitted", gate_key, gate_holder, worker_key, worker_holder}`. When a claim is not held, its key is null. A refused answer is `{status: "refused", exit_code, receipt, event}`.
