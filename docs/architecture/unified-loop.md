...
`stalled_holder`, `unheld_progress`, and `undriven_pr` are three queues over ONE predicate, `_node_driver`, which answers who is driving a node: `active`, `stalled`, `crowned`, or `none`. `crowned` is a live crown driving the epic it reigns over, which never holds a claim or a PR of its own; without it the reigning epic would read unheld and hold the king open on a row no verb can clear. They cannot disagree, because none computes its own answer. The reason there are three queues at all is sourcing. `stalled_holder` starts from the claim list, so it can only reach a worker still holding its lock. `undriven_pr` starts from open PRs bound back to their node, so it catches the worker that exited cleanly and released. Measured 2026-09-03: 7 of 12 open PRs sat undriven that way while the board named none of them. `unheld_progress` starts from the graph, so it catches the node the status stamp left `in_progress` after its claim went free or stale and no PR exists; the stamp is never revoked when a worker dies, so status alone can never carry it. Measured 2026-09-05: one such row sat invisible to every queue for 25 minutes while a king watched a clean board. `undriven_pr` and `unheld_progress` are report-only toward the node. They never close or defer anything, and a node the operator already deferred or blocked is skipped, not nagged back up.
...
**One arm was cut and rebuilt. The second is the walk.**
...
- *Cross-session:* `fno-agents loop run --driver king --scope <scope>` respawns a king while the board is non-empty. When the board is empty, it terminates `NoWork`. The first cut of this arm was built and cut before it shipped. Its unit key collided with the king's own termination id. Its continue prompt was a target resume. The rebuild keys the unit per invocation. It names the reign in the prompt. It bounds dispatch-bearing invocations with the manifest `respawn_count`. See `crates/fno-agents/src/loop_king.rs`.
...
**What a clean `NoWork` terminal means, and the edge it leaves open.** It means the king EXITS. The walk arm respawns a king only while the walk itself runs and the board is non-empty. A king that exited on a clean board and whose board then REFILLED was, for a long time, nobody's edge. The `loop_king.rs` module docstring once handed that edge to the fleet watchdog. That was a mis-attribution. The watchdog wakes on `classify_tail == "stalled"`. That means a session gone silent while still owing its next move. A cleanly-exited king is neither. Neither component was buggy in isolation. The belief was the defect.

**The waker: the `king_wake` phase of `fno pr-watch tick`.** A loop that terminated is not running. Nothing inside it can observe a refill or an arriving message. The waker is therefore a separate, always-present observer. The pr-watch tick already is one: a global LaunchAgent on a 600s `StartInterval`, already phased, already hosting the fleet watchdog lane. Per crowned scope from the court, gated by `config.king.wake_enabled` (default false):

1. *Liveness, fail closed.* Transcript truth via `resolve_session_truth`, never a registry state word. Only `done` and `unknown`/`not-found` count as absence. `stalled` belongs to the watchdog. `unknown`/`no-records` and `resolver-error` are instrument failures, not absences.
2. *Triggers, best first.* An answered escalation: the question journal (not the mail bus - the answer's delivery mail addresses the holder's full session id, which no mailbox scan covers) holds a closed-with-answer question whose asker is the holder, closed after the sidecar's `answered_cursor`. The answer is the wake prompt, the cursor seeds at the reign's birth (`created_at`), one answer delivers per tick, and it skips only the debounce, never the ceiling. Mail: undrained bus rows addressed to the holder or to any project in the scope. Both holder spellings are scanned, the registry name and the reply-handle short id. Measured on the live bus, replies carry the short id. A name-only scan reads a permanent zero. Board change: a sha256 over the scope's `(id, status, _kanban_column, priority)` rows, compared against a `.fno/kings/<scope>.wake.json` sidecar; the sidecar stores the rows beside the hash so the wake prompt can carry the added/changed/removed diff (`--wake-detail`) instead of a whole-board re-read. A first observation stores and wakes nothing. Backstop: a timer re-check, kept so a missed event cannot strand a scope forever. It approximates the two event triggers. The 1800s interval is a policy choice, not a measurement.
3. *Gate, bill, dispatch.* Every trigger passes `admit_wake`. It decides and bills the rolling 24h ledger under one lock, so two overlapping ticks cannot both dispatch. Ceiling 32 wakes per scope per window, about 1.5x the worst measured day. Debounce 15m: a king woken inside it is still working under its own in-session arm. The bill lands BEFORE the dispatch. A crash between them costs one slot of 32. The reverse costs an unbounded respawn storm. The dispatch is `loop run --driver king --scope <scope> --wake --wake-reason <reason> --wake-holder <stale row>`, detached. A mail wake adds `--wake-address <matched inbox>`; a board or answer wake adds `--wake-detail <diff or answer>`, capped at 2000 chars. The woken session is fresh and cannot derive the dead holder's address itself. The holder names the registry row the wake already outranked by transcript. An ordinary wake neither spends nor is refused by the failure-retry `respawn_count` budget. A holder whose session is GONE (`unknown`/`not-found`, not parked `done`) is replaced, not woken: the dispatch adds `--wake-successor`, bills `respawn_count` like any walk respawn, refuses at the respawn ceiling before billing the wake ledger, and journals `king_spawned_successor`. A `ceiling` refusal raises one marker-deduped operator question. That refusal is exactly the silent stranding this phase exists to end.
4. *The wake drains what woke it.* The walk's continue prompt carries the reason. A mail-woken king is told to run `fno agents mail unread` BEFORE its first board read. The waking message is not a board row. No board read will surface it. A respawn that never reads the mail reproduces the original failure with more processes.

**The `RunAtLoad` caveat.** The pr-watcher LaunchAgent has `RunAtLoad: false` by design. The operator runs `launchctl load` themselves. A machine that never loaded it has no waker at all. Turning `king.wake_enabled` on does not change that. An operator who enables the key and sees no wakes must check `fno do pr watch status` first.
...
> **Removed 2026-08-03.** The `--driver megawalk` arm and `loop_megawalk.rs` are deleted ([path-census.md](path-census.md)), along with the `/megawalk` skill and the `fno megawalk` verbs. The backlog walk today runs through `/fno:target <node>` per node, `/fno:target bg --all-ready` for the ready board, merge-triggered `fno backlog advance`, and the active-backlog daemon ([active-backlog-dispatcher.md](active-backlog-dispatcher.md)). The sections below are the design record.
...
**Legacy event migration.** The Python walker emitted ~29 kinds into `megawalk-events.jsonl` (deleted in task 2.4). Representative mappings: `node_complete` -> `node_closed{close:closed}`, `walker_paused` -> `walk_paused`, `consecutive_failures_paused` -> `walk_paused{policy:consecutive_failures}`, `backlog_empty` -> `loop_terminated{reason:NoWork}`. The full prune ledger was the comment block in `loop_megawalk.rs` (removed with the driver; the removal trail is in [path-census.md](path-census.md)). `megawalk-events.jsonl` as a write target is dead; the prune ledger records each legacy kind's fate for auditors.
