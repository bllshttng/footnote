...
| `fleet_task` | `questions.jsonl` fleet_task rows, folded by `crates/fno-agents/src/fleet_task.rs` | yes, by running the row's command; the row closes when its condition clears or its node closes |
...
**The waker: the `king_wake` phase of `fno pr-watch tick`.** A loop that terminated is not running. Nothing inside it can observe a refill or an arriving message. The waker is therefore a separate, always-present observer. The pr-watch tick already is one: a global LaunchAgent on a 600s `StartInterval`, already phased, already hosting the fleet watchdog lane. The wake runs for each crowned scope from the court and is gated by `config.king.wake_enabled` (default false). If a crown has no registry row and its owner_cwd checkout contains the swept manifest, the wake roots it at that manifest's `owner_cwd`. The tick row's note names every dropped crown by scope. Its steps are:
