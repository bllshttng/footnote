...
3. *Gate, bill, dispatch.* Every trigger passes `should_wake`, the rolling 24h ledger on the manifest. Ceiling 32 wakes per scope per window, about 1.5x the worst measured day. Debounce 15m: a king woken inside it is still working under its own in-session arm. The bill lands BEFORE the dispatch. A crash between them costs one slot of 32. The reverse costs an unbounded respawn storm. The dispatch is `loop run --driver king --scope <scope> --wake --wake-reason <reason>`, detached. A wake neither spends nor is refused by the failure-retry `respawn_count` budget. A `ceiling` refusal raises one marker-deduped operator question. That refusal is exactly the silent stranding this phase exists to end.
