...
- *Cross-session:* `fno-agents loop run --driver king --scope <scope>` respawns a king while the board is non-empty and terminates `NoWork` when it is not. The first cut of this arm was built and cut before it shipped (its unit key collided with the king's own termination id, and its continue prompt was a target resume); the rebuild keys the unit per invocation, names the reign in the prompt, and bounds dispatch-bearing invocations with the manifest `respawn_count`. See `crates/fno-agents/src/loop_king.rs`.
...
**What a clean `NoWork` terminal means, and the edge it leaves open.** It means the king EXITS. The walk arm respawns it only while its own process is running and the board is non-empty; a king that exited on a clean board and whose board then REFILLED was, for a long time, nobody's edge. The `loop_king.rs` module docstring once handed that edge to the fleet watchdog, which was a mis-attribution: the watchdog wakes on `classify_tail == "stalled"`, a session silent while still owing its next move, and a cleanly-exited king is neither. Neither component was buggy in isolation; the belief was the defect.

**The waker: the `king_wake` phase of `fno pr-watch tick`.** A loop that terminated is not running, so nothing inside it can observe a refill or an arriving message; the waker is a separate, always-present observer, and the pr-watch tick already is one (a global LaunchAgent on a 600s `StartInterval`, already phased, already hosting the fleet watchdog lane). Per crowned scope from the court, gated by `config.king.wake_enabled` (default false):

1. *Liveness, fail closed.* Transcript truth via `resolve_session_truth`, never a registry state word. Only `done` and `unknown`/`not-found` are absence; `stalled` belongs to the watchdog; `unknown`/`no-records` and `resolver-error` are instrument failures, not absences.
2. *Triggers, best first.* Undrained bus mail addressed to the holder (both spellings: registry name and reply-handle short id - measured on the live bus, replies carry the short id, so a name-only scan reads a permanent zero) or to any project in the scope; then a board change, detected as a sha256 over the scope's `(id, status, _kanban_column, priority)` rows against a `.fno/kings/<scope>.wake.json` sidecar (a first observation stores and wakes nothing); then a timer backstop, an approximation of the two event triggers, kept so a missed event cannot strand a scope forever. The 1800s interval is a policy choice, not a measurement.
3. *Gate, bill, dispatch.* Every trigger passes `should_wake`, the rolling 24h ledger on the manifest: ceiling 32 wakes per scope per window (~1.5x the worst measured day), debounce 15m (a king woken inside it is still working under its own in-session arm). The bill lands BEFORE the dispatch - a crash between them costs one slot of 32, the reverse costs an unbounded respawn storm. The dispatch is `loop run --driver king --scope <scope> --wake --wake-reason <reason>`, detached: a wake neither spends nor is refused by the failure-retry `respawn_count` budget. A `ceiling` refusal raises one marker-deduped operator question, because it is exactly the silent stranding this phase exists to end.
4. *The wake drains what woke it.* The walk's continue prompt carries the reason; a mail-woken king is told to run `fno agents mail unread` BEFORE its first board read, because the waking message is not a board row and no board read will surface it. A respawn that never reads the mail reproduces the original failure with more processes.

**The `RunAtLoad` caveat.** The pr-watcher LaunchAgent has `RunAtLoad: false` by design (the operator runs `launchctl load` themselves), so a machine that never loaded it has no waker at all. Turning `king.wake_enabled` on does not change that; an operator who enables the key and sees no wakes should check `fno do pr watch status` first.
