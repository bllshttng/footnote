...
The owner of the math, the disk, and the policy-table validation is the `fno-agents admission` verb (`crates/fno-agents/src/admission.rs`), beside the other runtime-state readers. Python carries only thin call sites: the transport lives beside the state it reads (`cli/src/fno/adapters/providers/runtime_state.py`), the budget identity beside the binding proof it consults (`binding.py`; the Keychain read is the one thing that cannot move), and the config block beside the routing hub (`cli/src/fno/config/routing_blocks.py`). The state document's `reservations` block rides opaquely through every Python writer: the verb owns the rows and drops expired ones under the lock. Python sends one JSON payload; the verb validates, decides, locks, and persists.
