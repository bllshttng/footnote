...
- ~~The `fno doctor event emit` command used the legacy `events/log.py` path. That path produced a `payload` envelope incompatible with schema validation. The new builder bypassed the bug only for `phase_0_decision`. A separate fix still needed to align every event type.~~ Follow-up work fixed this. `fno doctor event emit` now uses `events._build` plus `append_event` for every event type. `--data` is canonical. `--payload` remains a deprecated alias and prints a stderr warning.
