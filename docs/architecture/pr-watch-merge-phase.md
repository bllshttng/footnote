...
- It re-loads the entry under the per-PR lock. Every unattempted row is skipped with a reason on `pr_watch_skipped`. The reason names the cause: `merged`, `parked`, `not-open`, `no-watermark`, `locked`, or `execute-budget`.
- The floor is `max(_FIRE_FLOOR_S, slowest)`. `slowest` is the longest attempt this drain has run. The drain never starts an attempt the slice cannot finish.
