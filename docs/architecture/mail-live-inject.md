...
`fno agents mail send` delivers an agent-to-agent message to a LIVE recipient first and queues the durable bus only as a fallback.

This unifies the G1 substrate around one send interface, one wire envelope, and one live primitive per provider. The bus stores sender audit history and the offline pending queue.
...
2. For a live recipient, attempt one fire-and-forget delivery through `_deliver_live`. After confirmation, write one `delivery: hosted` row with the exact id, addresses, body, provenance, and send-time `word_count`. The row is audit-only. It appears in sender and operator history but never enters delivery, rebuild, unclaimed, or withdrawal paths.
...
`_deliver_live` returns `True` on the first successful lane and `False` otherwise. `dispatch_send` writes a hosted audit row after `True` and a deliverable durable row after `False`.

Delivery takes exactly one path: a live transport or the durable queue. The bus field distinguishes delivered history from pending mail.
...
An unwrapped one-line payload rides verbatim. A payload that starts with `/fno:` or with `$fno:` names an fno verb. The verb rewrites that marker to the form of the receiving harness before delivery. A verb typed in one harness's dialect lands in the recipient's native one.
...
### `--harness codex`: the codex live rung
...
The cross-session relay PTY hop (`cli/src/fno/relay/envelope.py`) frames provenance on the same `<fno_mail>` tag, but as the SINGLE-LINE, no-close transport variant: `<fno_mail from="..."[ harness="..."]> <one-line body>`. It cannot carry the paired multiline form because the PTY Enter submits on newline. It shares the tag name so `grep <fno_mail>` reconstructs relay hops too. The line is built by the same sole renderer as the paired form.
