...
The comparison is family-wise and suffix-aware. The bracketed suffix names the context window the caller asked for, not a different model, so `glm-5.3[1m]` vs `glm-5.3` is a match. `glm-5.3[1m]` vs `glm-5.3-flash` is a substitution, and the spawn path says so at spawn time instead of printing the request back as the effect: a stderr line, a `model_substituted` event, and a receipt marker. The agent-list row derives `model_substituted` at emission from the observed payload it already carries. A substitution that surfaces late is therefore visible on the next list read.
