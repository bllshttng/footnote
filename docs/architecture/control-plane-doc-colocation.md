...
- The "Check control-plane doc colocation" step in [`.github/workflows/guards.yml`](../../.github/workflows/guards.yml)'s `guards-pr` job (consolidated there, x-b130) sets `continue-on-error: true`, so even if branch protection lists it the run still succeeds.
