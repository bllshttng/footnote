...
Two things keep their meaning without attribution, so both gate wherever they happen. A truncation is one. An unreadable REGULAR file is the other. That is the instrument failing, not the root churning. Advisory covers what cannot be attributed. It never covers what was never measured.

A rotation is not a truncation, though it has the same size profile. `events.jsonl` is renamed to `events.jsonl.1` past 8 MB and a small active file takes its place. `verify` reports that as `ROTATED` and does not gate, recognising it by the sibling that appeared or grew. Without that, every ordinary rotation on a live root fails a guard whose whole point is to leave ordinary churn alone.
