...
`cfg!(test)` is the stated limit, not an oversight. Integration targets under `crates/fno-agents/tests/` link the lib compiled WITHOUT `cfg(test)`, so they get no fence: to them the crate is a dependency, which is the same shape production sees. Widening the fence to reach them means making it a runtime check, and a runtime check fires in production, where an ambient `$HOME` resolution is the correct answer rather than the bug. So an integration target declares its root by pinning `FNO_AGENTS_HOME` or `FNO_SPACES_DIR` itself, and nothing refuses if it does not.
