...
A dev box is WATCHED, not skipped and not written. When `$HOME/.fno/graph.db` already holds entries that is a live operator root, so `plant` writes no marker and takes the baseline read-only. It used to stand down entirely, which was the defect. The motivating specimen is a run that cut a 2297-node graph to 64 bytes. A 2297-node graph IS a live root. So the instrument disarmed itself on exactly the machine class it was written for. Only the marker write ever needed an empty root. The snapshot and the comparison never did. A corrupt graph still counts as live. `plant` never overwrites an existing graph store either, not even an empty one: that file is the operator's.
