...
| `node.sessions[]` in `~/.fno/graph.db` | `_reachable_from_graph` (`discover.py:2407`) via `fno.graph.load.load_graph` | The weakest source: a session proven to have existed for some phase of some node. |
