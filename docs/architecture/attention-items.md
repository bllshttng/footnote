...
2. `resume` - a durable park under an idle asker. When the asker has a session id, the arm runs `fno agents resume <session_id> --message <answer>` (the one resume door, 30 s bound, stdin closed; the message carries the question id and a dedupe line). A later beat looks for the question id in the asker's transcript after the byte offset the resume saved; seeing it confirms delivery (`rung: resume, outcome: confirmed`).
