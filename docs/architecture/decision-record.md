...
The review-coverage gate is the reference consumer. Its merge predicate, its `fno do pr status` readiness read, and the Rust stop gate all resolve the standing subject `review-coverage-waiver` and the head-scoped subject `review-coverage-waiver:<owner/repo>#<pr>@<head>` through `fno.decide.current_law` (Python import) or the canonical query above (Rust subprocess, parsing only `current_law.status`). Only `single` waives; `none` leaves the ordinary predicate untouched; a conflict, a damaged row, or a failed query is UNKNOWN authority and answers UNANSWERED with the probe named, never permission and never a quiet `none`. A recorded waiver is a law row like any other: retraction, supersession, or head movement changes the verdict the next read returns, and a stale waiver green is replaced by the computed one. The full contract lives in [review-coverage-termination](review-coverage-termination.md).
