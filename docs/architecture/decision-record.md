...
The law door (`fno inbox law set`) stamps the recording session's project by default. Widening to `global` is explicit on the door itself: the `--global` flag carries it, because law is never inherited by silence. A repo outside the work map makes the door refuse and name the path it failed to place. The door fails closed so a guessed stamp never lands, while every reader fails open.

Every law reader filters. Global law reaches every session. A project law reaches only that project's sessions. A row with no scope reaches only fno sessions, because all 82 pre-scope rows were recorded in fno. The general ones get promoted by an explicit re-record widened with `--global`.
...
No session loads law in bulk. The SessionStart law read is deleted (ruling of 2026-09-14): it read every live row, blew the hook's ten-second bound, and delivered nothing. A law reaches a session two ways, and neither loads all law. First, a law the superuser confirms graduates through the `graduation` field every law row carries. `enforced` with `doc:<rules file>=>marker:<text>` points at a rule file the harness loads whole. `enforced` with `gate:<cmd>=>marker:<text>` points at a refusal at the moment the law applies. Second, `ask` keeps reading law for one subject at ask time, so the law door's refusal still fires where the question is asked.

## Gates that consume current law

The review-coverage gate is the reference consumer. Its merge predicate, its `fno do pr status` readiness read, and the Rust stop gate all resolve two subjects: the standing `review-coverage-waiver` and the head-scoped `review-coverage-waiver:<owner/repo>#<pr>@<head>`. Python imports the engine's law-lane live read. Rust shells the canonical query above and parses only `current_law.status` plus the single row's decision value. Only `single` waives, and only a row whose decision equals the affirmative value the coverage-waive command mints counts as that single. Row existence carries no polarity: a note or a denial recorded at a waiver subject is no waiver. A single row with no readable decision is malformed authority and answers unknown, never none. `none` leaves the ordinary predicate untouched. A conflict, a damaged row, or a failed query is UNKNOWN authority. The gate answers UNANSWERED with the probe named, never permission and never a quiet `none`. A recorded waiver is a law row like any other. Retraction, supersession, or head movement changes the verdict the next read returns, and a stale waiver green is replaced by the computed one. The full contract lives in [review-coverage-termination](review-coverage-termination.md).
...
Two events trigger the read. Claude fires `hooks/law-stage-inject.sh` on `PostToolUse` with matcher `Skill`. Claude and Codex also fire it on `UserPromptSubmit`, which covers typed and mailed review verbs. `PreToolUse` on `Edit|Write` now carries the edit read too. Both claude and codex accept `additionalContext` there, so the context lands next to the tool result. `PostToolUse` and `UserPromptSubmit` accept `hookSpecificOutput.additionalContext`. That output lands beside the review instructions the skill itself prints, and `hooks/review-hold.sh` runs on the same Skill event. The verb is `fno inbox law stage`, a nested group on the `fno` front, because d-fe66560a bars new top-level verbs. The front serves it by spawning the runtime worker's one-shot `--law-exec` lane, because the mux never links fno-agents (the product boundary). The laws come from `decision_index::live_laws`, a direct read of the index. Measured 2026-09-14: the Python read took 67.0s real at load 345 and hits a 10s hook bound. The direct read of the 1.2 MB index takes milliseconds. The hook's timeout wrapper passes stdin through to the matcher. The stage block lists every matched law: summarized under the 2000-byte cap, then one short id line per law the cap cut.

A law recorded with `--paths` prints once per session at the first `Edit` or `Write` of a matching file. The hook reads the target paths off the payload, and the verb's edit read matches them against the row's repo-relative globs with the crate's fnmatch. The once-per-session key is the decision id alone, kept in `law-edit-seen/<session>.json`. A changed law is a new id by supersede, so no second key is needed.
...
|---|---|---|
| `review` | 10 of 11 (misses the ruling filed under a node id) | 0 |
| `review\|attest\|round\|finding` | 11 of 11 | 3 |
...
The upstream provenance seam owns mail-origin classification, stamped carriage, and the law chokepoint. The law command owns law recording and coord-to-law promotion. It owns no human-origin proof. No harness exposes a discriminator. So recording is one user-invoked step. The row says `chat_attested` rather than claiming an origin it cannot show. See `skills/law/LIMITATIONS.md`. This lifecycle feature consumes those seams and owns expiry, retraction, review, export, and law-only reads. It does not add another origin classifier, law command, promotion path, or decision store.

## The operator conversation queue

A session receiving an operator ruling can record it only as `chat_attested`. Measured result: it often records nothing at all, because mail arrives as a discrete event and operator conversation arrives as a stream. `fno inbox operator` gives the stream the event boundary mail already has.

The queue is derived from the whole transcript by the Rust reader `fno-agents compaction operator-turns` (`crates/fno-agents/src/operator_turns.rs`) and cached by a per-session scan cursor. The undispositioned turns are the user turns in the transcript minus the ids already acked. Nothing writes at capture time, so a remark from an hour ago still queues. The transcript is the system of record. `<session-id>.scan.json` next to the ledger caches how far the last read got. Deleting it only forces a full rescan from byte 0. Record first through the existing verbs: `fno inbox law set`, `fno backlog capture add`, or `fno backlog idea --source-kind operator_request` for the origin line. Then `fno inbox operator ack <turn-id> --outcome law:<id>|capture:<fu-id>|node:<id>|nothing` appends one `{turn_id, ts, outcome, ref, why}` row to the per-session ledger at `~/.fno/operator-capture/<session-id>.jsonl`. That file is the receipt and the watermark at once.

The ack attributes a turn to an outcome. It never upgrades the authority lane of what was recorded: a captured law still lands `chat_attested`, and the queue classifier decides only what surfaces for disposition, never the origin a decision row claims.

## The read that produced a code fact

Eight rulings in two days named a code fact that was false. A worker re-measured each one downstream. Every catch cost a review round against the two-round cap. The shape was constant: a measured claim and an assumed one are written identically. Nobody can tell, from the ruling alone, whether the fact was measured. Specimen: a "no cwd resolves both" claim that resolved together in all eight directories tested. Specimen: a `+167` diff chunk described as a function it did not contain. Specimen: a function measured without its call graph. The 159-line function dragged a 401-line dependency with zero Rust files.

The remedy is an attached receipt, not softer language, and no confidence score: every one of the eight was stated plainly and was plainly wrong. A ruling body asserting a code fact carries `--read`, the command that produced it. The verb RUNS the command at record time (`sh -c`, 20s timeout, 5-command cap, `fno-agents evidence-gate`) and stores the exit code and output head on the row. A string nobody ran is prose. A row with an exit code is evidence. A non-zero exit with output is a measurement, not a failure - `grep -c` exiting 1 on a real zero is the read working.

Three rules bind the gate:

- **The exemption keys on the resolved authority.** An attended operator terminal (`authority_source == "operator"`) is exempt. This covers `fno do pr coverage-waive` by construction: its authority is hardcoded operator, and the verb refuses every other caller. `_resolve_decider` refuses an operator claim from a resolved session. The population the gate is for cannot claim the exemption.
- **A zero needs a control.** An all-zero answer set refuses. The standing pitfall: assert a positive marker, never an absence. A second `--read` must ride along, aimed at something known to be present. Check the axes that can move the reading: symbol, age, directory. A green control on the wrong symbol still reads as proof.
...
The one-step door (`fno inbox law set`) has no staged proposal, no content hash, and no resume path (ruling d-e1eec854). The old ceremony was deleted because it refused the honest headless path at the last step. An attended chat approved the same enactment without reading the hash.
