...
The queue is derived, not stored. The undispositioned turns are the user turns in the session transcript minus the turn ids already acked, so nothing writes at capture time and a remark from an hour ago still queues. Record first through the existing verbs: `fno inbox law set`, `fno backlog capture add`, or `fno backlog idea --source-kind operator_request` for the origin line. Then `fno inbox operator ack <turn-id> --outcome law:<id>|capture:<fu-id>|node:<id>|nothing` appends one `{turn_id, ts, outcome, ref, why}` row to the per-session ledger at `~/.fno/operator-capture/<session-id>.jsonl`. That file is the receipt and the watermark at once.
...
Eight rulings in two days named a code fact that was false, and every one was caught downstream by a worker re-measuring - a review round the two-round cap could not afford. The shape was constant: a measured claim and an assumed one are written identically, so nobody could tell from the ruling whether it was measured. Specimens included a "no cwd resolves both" claim that resolved together in all eight directories tested, a `+167` diff chunk described as a different function than it contained, and a function size measured without its call graph (the 159-line function dragged a 401-line dependency with zero Rust files).

The remedy is an attached receipt, not softer language, and no confidence score: every one of the eight was stated plainly and was plainly wrong. A ruling body asserting a code fact carries `--read`, the command that produced it. The verb RUNS the command at record time (`shell=True`, 20s timeout, 5-command cap, `fno.decide.evidence`) and stores the exit code and output head on the row. A string nobody ran is prose; a row with an exit code is evidence. A non-zero exit with output is a measurement, not a failure - `grep -c` exiting 1 on a real zero is the read working.

Three rules bind the gate:

- **The exemption keys on the resolved authority.** An attended operator terminal (`authority_source == "operator"`) is exempt, which covers `fno do pr coverage-waive` by construction: its authority is hardcoded operator and the verb refuses every other caller. `_resolve_decider` refuses an operator claim from a resolved session, so the population the gate is for cannot claim the exemption.
- **A zero needs a control.** An all-zero answer set refuses under the standing pitfall (assert a positive marker, never an absence): a second `--read` of the same shape aimed at something known to be present must ride along. Check the axes that could move the reading - symbol, age, directory - before trusting a zero; a green control aimed at the wrong symbol still reads as proof.
- **A contradicted citation refuses whatever is attached.** A `path:line` the repo contradicts (untracked path, ambiguous bare basename, line past EOF) is refused before any read runs. The note lane (`fno backlog note`) splits on purpose: a contradicted citation refuses the note even under `--quiet`, while a claim with no read only warns, because the note verb advises and never refuses a body.

The gate lives inside `record_decision` beside the session gates, not in the command bodies: the library is importable, and a check only the CLI enforces is a check anything using the library walks around. The claim vocabulary is two shapes and nothing else - a `path:line` citation over a source extension, and a count bound to a fixed noun list (`lines`, `call sites`, `callers`, `consumers`, `usages`, `occurrences`, `matches`, `files`), negatives included - listed in one constant in `fno/decide/evidence.py` so a reviewer can audit it in one read. A false positive on ordinary prose is the failure that gets this gate disabled.

## The chat-law trade, measured

The one-step door (`fno inbox law set`) has no staged proposal, no content hash, and no resume path (ruling d-e1eec854). The ceremony was deleted because it refused the honest headless path at the last step while an attended chat approved the same enactment without reading the hash. What the trade costs, measured rather than assumed: `require_marked_caller` answers `chat_attested` off `resolve_self_identity`, which walks process ancestry, so the door is not "a mail-injected slash command" but ANY process descended from a harness session, including an agent's own Bash call with no user-shaped text anywhere. The narrower mail shape is the one that is impossible to detect: across every transcript in this machine's claude project directory, 2173 user turns carrying an `<fno_mail>` envelope were recorded with `promptSource: "typed"` and 2439 with `origin: {"kind": "human"}`, and `fno agents mail send --raw` strips the envelope that is the one remaining marker. The door is the law LANE and never the `operator` VALUE: `_resolve_decider` still refuses an `operator` claim from a resolved session. The asymmetry that buys honesty: a session can mint a law row and cannot retract one, because `retract_decision` requires `operator`.

One family is closed to the chat door entirely: subjects under `review-coverage-waiver` (the merge gate's waiver evidence) refuse every non-operator authority at the write chokepoint, because a waiver asserts a person read the diff and no chat row can carry that fact. The attended `fno do pr coverage-waive` command is the only path.
