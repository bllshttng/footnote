...
The recording skew this store measured has a second half: a session receiving an operator ruling could record it only as `chat_attested`, and so it often recorded nothing at all, because mail arrived as a discrete event and operator conversation arrived as a stream. `fno inbox operator` gives the stream the event boundary mail already has. The queue is derived, not stored: the undispositioned turns are the user turns in the session transcript minus the turn ids already acked, so nothing writes at capture time and a remark from an hour ago still queues. Recording runs through the existing verbs first (`fno inbox law set`, `fno backlog capture add`, or `fno backlog idea --source-kind operator_request` for the origin line), then `fno inbox operator ack <turn-id> --outcome law:<id>|capture:<fu-id>|node:<id>|nothing` appends one `{turn_id, ts, outcome, ref, why}` row to the per-session ledger at `~/.fno/operator-capture/<session-id>.jsonl`. That file is the receipt and the watermark at once. The ack attributes a turn to an outcome; it never upgrades the authority lane of what was recorded. A captured law still lands `chat_attested`, and the queue classifier decides only what surfaces for disposition, never the origin a decision row claims.
