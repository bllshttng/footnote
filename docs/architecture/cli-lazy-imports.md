...
**The same wait, on all three provisioning paths.** Naming `update.py` alone was itself a one-of-N gap, and it cost a second bug. There are THREE places that install the tool and then verify it, and only one of them waited. `crates/fno/src/bootstrap.rs` verified with a single-shot `entry.exists()` the instant uv returned, so it raced the install it was verifying. The symptom: `fno` refused every verb, reran a full 19-package install each time, and reported "the install does not verify: no console script". `.claude-plugin/postinstall.sh` had the same single-shot shape. All three now spend the same 15 times 0.2s. All three RE-CHECK rather than sleeping blind, so a genuinely broken install still fails with the message it always had. The budget is duplicated across Rust, Python-emitted shell, and bash, because no implementation can be shared across those boundaries. A test pins the numbers together instead, so drift fails CI rather than review.
