...
1. **Create / reuse the worktree off `origin/main`** via `fno workspace worktree ensure`. That verb branches off `origin/main` (never local HEAD), reuses an existing worktree idempotently, refuses to nest inside a linked worktree, and prints the worktree path on stdout.
