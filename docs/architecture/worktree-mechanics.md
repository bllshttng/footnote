...
Cargo targets remain worktree-local so sibling builds never share Cargo's artifact-directory lock. `fno agents workspace worktree cargo-offload` (dry-run default, `--apply` executes) moves each `crates/<crate>/target` out of the repo root to `<base>/<repo>/<tree>/<crate>`, leaving a symlink at the old path. `<base>` is `paths.cargo_targets_base`, default `~/.fno/cargo-targets`. `<tree>` is `canonical` for the main checkout and the worktree name otherwise. Every tree keeps its own destination and its own artifact lock. The repo root is what a harness plugin update copies. Nearly all of its bulk is regenerable build output. Moving the bytes, not deleting them, is the lever: a cache can leave a tree the removal contract must never touch. A tree with a live target claim or any rooted process is reported and left for the next run. Never put `build.target-dir` in `.cargo/config.toml`: that file is tracked, so every sibling worktree shares ONE target directory and the fleet's builds serialize. The symlink is what keeps `crates/<crate>/target/release/<binary>` a working path.
