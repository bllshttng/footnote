...
The wrapper sets the shared sccache size to 30G unless `SCCACHE_CACHE_SIZE` is set. At 10G the cache sat at its limit on a fleet machine. It kept only 6.5 hours of history, so a branch parked overnight rebuilt cold. A running sccache server keeps its old size until it exits. sccache refuses proc-macro, build-script, bin and test crate types by design. About half the calls never reach the cache, and no setting changes that. The build dir stays per worktree. A shared dir could only add reuse of registry proc-macros and build scripts. It would also break the reclaim at worktree removal, and one `cargo clean` would wipe every worktree. The repo sets no `jobs` key, so a solo clone builds at full width. A per-user `jobs` cap slows every build and still does not stop two cargo runs at once, because cargo shares no jobserver across separate runs.
