...
Cargo targets remain worktree-local so sibling builds never share Cargo's artifact-directory lock. `fno agents workspace worktree cargo-offload` (dry-run default, `--apply` executes) moves each `crates/<crate>/target` out of the repo root to `<base>/<repo>/<tree>/<crate>`, leaving a symlink at the old path. `<base>` is `paths.cargo_targets_base`, default `~/.fno/cargo-targets`; `<tree>` is `canonical` for the main checkout and the worktree name otherwise, so every tree keeps its own destination and its own artifact lock. The repo root is what a harness plugin update copies, and nearly all of its bulk is regenerable build output, so moving the bytes, not deleting them, is the lever: a cache can be moved out of a dirty, unpushed or unmerged tree that the removal contract must never touch. A tree with a live target claim or any rooted process is reported and left for the next run. Never put `build.target-dir` in `.cargo/config.toml`: that file is tracked, so every sibling worktree would share ONE target directory and serialize the fleet's builds. The symlink is what keeps `crates/<crate>/target/release/<binary>` a working path.

The cleanup sweep follows those symlinks so a relocated cache stays reclaimable: its inventory accepts a link whose resolved directory sits under the offload base AND carries `CACHEDIR.TAG`, and deletes the resolved directory first, then the link. Both conjuncts are load-bearing: the base is fno-owned and the tag is Cargo's own marker, so following a link can never reach a directory fno did not create. A link failing either conjunct reads `reason=link-not-owned`, counted and never deleted. Select only `crates/*/target` by path, never by directory name: `cli/src/fno/target`, `skills/target` and `tests/target` are source dirs, and a name-based sweep deleted 66 of them across 26 worktrees on 2026-09-02.

After linking succeeds, setup runs `fno agents workspace worktree cleanup --cargo-targets --apply` (inspect first by omitting `--apply`). It reaps inactive targets older than seven days first, then the oldest inactive targets until allocated bytes sit at or below the effective ceiling. The ceiling is `min(64 GiB, --free-share-pct percent of free disk space)`, default 50 percent. A nearly full disk therefore tightens the ceiling instead of leaving an under-cap `ok` verdict on a full volume. `FNO_CARGO_FREE_BYTES` overrides the free-space read and `FNO_CARGO_TARGETS_BASE` the offload base, for tests. A live target claim or rooted process protects its worktree. Protected bytes that prevent the ceiling return `over-cap-protected` instead of deleting an active build, and the summary line always names `free_bytes` and `effective_cap_bytes`. Repository Cargo config uses the wrapper at `scripts/lib/cargo-rustc-wrapper.sh`, gated by `incremental = false`. Sccache shares a 10 GiB cache, machines without it run rustc directly.
