...
Cargo writes intermediates outside the checkout, and final binaries inside it. The tracked `.cargo/config.toml` sets `build.build-dir = "{cargo-cache-home}/build/{workspace-path-hash}"`, so a bare clone sends every intermediate to `~/.cargo/build/<h2>/<hash>` and leaves `crates/<crate>/target` holding only the final binaries. fno exports `CARGO_BUILD_BUILD_DIR=<base>/{workspace-path-hash}` (via `fno.paths.cargo_build_dir_value`, the doctor-test child env, the `fno config plugin install` env exports, and the shell rc), which moves the same layout under `<base>`: `paths.cargo_targets_base`, default `~/.fno/cargo-build`. The hash is per workspace root, so every checkout, every worktree, and the two workspaces inside one tree each get their own build dir and their own artifact lock: parallel builds never serialize on a shared directory. Never put `build.target-dir` in `.cargo/config.toml`: that key has no workspace hash, so every sibling worktree would share ONE build directory and the fleet's builds would serialize again. Harness plugin installs copy from the filtered stage (`fno config plugin install`), so the caches never carry build output either.
