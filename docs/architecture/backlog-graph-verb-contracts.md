...
``--blocking`` turns the record into a review finding: the body lands in the findings store (its own aggregate and table, never a journal line), loop-check denies the session's success terminal while it stays open, and delivery shrinks to the finding pointer (node, id, resolve command) when a reader is live. With nobody bound the write still lands and the receipt says it gates the next worker; the nobody-bound refusal is a note-only rule. ``note --resolve <finding-id>`` stamps ``resolved_at``: an unknown id exits 1, a second resolve exits 0 naming the prior resolution. ``fno backlog notes findings [<id>] [--open]`` is the reader: a count line prints even at zero, and a store read error exits 1 rather than reading as clean. The retired ``fno backlog annotate`` spellings forward here for one release.
...
Dispatch a fire-and-forget autonomous ``/target`` (or ``dispatch_verb``) worker.
    ...
    ``headless`` for codex/others. The workflow verb is DECIDED once, in Rust:
    ``backlog_ready.rs``'s ``effective_verb`` (served through the graph
    keeper; Python is its client) runs one conditional over the node's declared
    verb, plan rung, difficulty, and doc kind. At INTAKE (plan rung
    ...
    ``ready``/``in_progress``/``in_review`` derive ``/target`` only when the
    linked doc reads as a blueprint (``is_blueprint_doc``: a blueprint
    ``kind`` or ``type`` value - ``quick-plan``, ``plan``, ``implementation-plan``,
    ``blueprint`` - or no declared kind and an ``## Execution Strategy``
    heading; a declared ``research``/``findings``/``think``/``stub`` kind is a
    hard no above the heading) - and otherwise derive ``/blueprint``, so a
    research doc that merely carries ``status: ready`` never dispatches
    ``/target``. A DECLARED target-family ``dispatch_verb`` WINS: the
    decision returns it as declared and the decision trail names what the
    table would have answered, so a stale declaration costs one more pass on
    its declared verb, never a silent override. Refusals still fire before
    the declared verb. An out-of-family verb (``/think``) keeps declared
    precedence; ``unreadable``/``done``/``superseded`` plan rungs and
    ...
    table reads the RESOLVED verb (declared or derived), so
    ``agents.profiles.blueprint`` reaches
