...
The permanent question index stores the boundary reference because the project journal rotates at 8 MiB and keeps only one rotated file. The index is recall and provenance, not a second source of question truth. Open questions still come from the existing lifecycle fold, and a failed index append names the boundary id after the project append. A failed index append also hides the boundary from the reuse check. A retry on the same day then appends a second journal row under a new id. The row without an index entry is the orphan to ignore. This is the decide retraction's own failure shape.
