...
The inverse is `fno backlog update <id> --parent null`, which un-contains a node as it moves it away. The release also drops the PR refs the containment bind inherited from the owner and records `released_from`, so the owner's `Fixes` line no longer closes the node; containing it again clears the marker.
