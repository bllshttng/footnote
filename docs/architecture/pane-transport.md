...
The same rule now governs the keeper-hosted threads. A send confirms only from the hosted harness's own accepted-turn record, read past the byte boundary captured before the paste: pi's session file, materializing at the first turn attempt, matched on the full envelope line. Composer echo and scrollback repaint are typing progress and never evidence, so a harness with no local record (cursor-agent, agy) types the message and stays unconfirmed for the whole confirm budget, and the durable floor writes the recovery copy. A paint match on a truncated marker prefix once reported delivered for a draft that was never submitted; the prefix matcher is gone (x-175a).
