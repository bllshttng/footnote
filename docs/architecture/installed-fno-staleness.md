...
Guidance names `fno agents restart --mux` for a routine wire-version bump, and `fno mux kill-server` for a wedged server a plain restart cannot reach. `kill-server` no longer depends only on the control channel it exists to recover from. It escalates through SIGTERM then SIGKILL, using a pid sidecar the server writes at bind. That makes it work in the wedged or wire-rejecting case an operator reaches for it. Every operator surface (`mux ls`, the attach error, `restart.py`) names this escalation, not a manual kill.
