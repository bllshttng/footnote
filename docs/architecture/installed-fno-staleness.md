...
The one input `doctor` lacks is whether an update breaks the live mux **wire protocol**. A server whose wire is below `MIN_COMPAT_PROTO` rejects the handshake outright. `update_readiness` (in `cli/src/fno/update.py`) compares both constants from the source checkout's `crates/fno/src/proto.rs` against each **live** server's `wire_version`. Below the floor or newer than the source both read as a wire bump. The newer-than-source case guards a downgrade: the older installed binary's equality gate refuses the running newer server. That field comes from `fno mux ls --json` and matches `SessionRow::wire_stale` in `crates/fno/src/mux_cli.rs`. A pane-less stale-wire server is auto-restarted. A server with live panes is reported and spared unless the operator passes `fno agents restart --mux`. A forced restart names the shells it ends and the workers `--revive` brings back, via the shared `is_revivable` predicate in `cli/src/fno/restart.py`. A compatible wire means the shells survive a plain `fno doctor update` and reattach.
