...
Guidance names `fno restart --mux` for a routine wire-version bump, and `fno mux kill-server` for a wedged server that a plain restart cannot reach. `kill-server` no longer depends solely on the control channel it exists to recover from: it escalates through SIGTERM then SIGKILL, resolved from a pid sidecar the server writes at bind, so it now works in precisely the wedged or wire-rejecting case an operator reaches for it. Every operator surface (`mux ls`, the attach error, `restart.py`) names this escalation rather than a manual kill.
