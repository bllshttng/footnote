...
The one input `doctor` lacks is whether an update breaks the live mux **wire protocol**. A server whose wire is below `MIN_COMPAT_PROTO` rejects the handshake outright. `update_readiness` (in `cli/src/fno/update.py`) compares that floor from the source checkout's `crates/fno/src/proto.rs` against each **live** server's `wire_version`. That field comes from `fno mux ls --json` and matches `SessionRow::wire_stale` in `crates/fno/src/mux_cli.rs`. A pane-less below-floor server is auto-restarted; a server with live panes is reported and spared unless the operator passes `fno agents restart --mux`. A forced restart names the shells it ends and the workers `--revive` brings back, via the shared `is_revivable` predicate in `cli/src/fno/restart.py`, called from both `update_readiness` and `_revive_orphans`. A compatible wire means the shells survive a plain `fno doctor update` and reattach.

Guidance names `fno agents restart` for pane-less wire skew, `fno agents restart --mux` when the operator accepts ending live panes, and `fno mux kill-server` for a wedged server a plain restart cannot reach. `kill-server` no longer depends only on the control channel it exists to recover from. It escalates through SIGTERM then SIGKILL, using a pid sidecar the server writes at bind. That makes it work in the wedged or wire-rejecting case an operator reaches for it. Every operator surface (`mux ls`, the attach error, `restart.py`) names this escalation, not a manual kill.
