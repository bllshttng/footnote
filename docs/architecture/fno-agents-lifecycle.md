...
10. **Revival of an exited claude row is probe-first.** An exited claude bg row stays revivable. `resume <name>` is the smart human verb. It probes reality first (`locate_session` plus a 250ms socket connect), never the registry `status` field. On a live supervisor it wakes the session headlessly and verifies the state reached Working. It no longer attaches directly. On a dead one it execs `claude --resume <id>` in the recorded cwd. That read is registry-read-only, so the row stays exited and revivable later. `attach <name>` stays strict live-only. A dead claude row with a recorded uuid now refuses with the two revival commands instead of dead-ending. The colliding row must be exited, with a uuid matching its own. Then `spawn <name> --resume <uuid> --substrate bg` revives the row **in place** (fresh short_id, same uuid, status live). Every other same-name case (live row, uuid mismatch, no `--resume`) stays fail-closed. Probe-first is load-bearing. Two writers in one transcript is the failure this check exists to prevent. Liveness is always checked before the resume lane fires. A resume may not continue the same uuid: claude has been seen carrying a transcript across under a new id, which is why rule 12 exists.
11. **A claude re-entry carries the recorded account and route, or refuses.** Every fno-controlled door to a claude session resolves one canonical re-entry plan before launch. The doors are the CLI attach and resume arms, every mux gesture, restore, and recovery. The plan reads the row's launch account and validated route settings. It applies both to the provider invocation together. A row whose account no longer resolves, or whose route file is missing or gutted, refuses with the missing field named. No door rebuilds its own launch shape. Secrets stay inside the 0600 route file. The plan and every printed command carry paths and ids only.
12. **A fork is additive: both ids stay valid on one row, forever.** A claude SessionStart reporting a new id never replaces a recorded one. An empty primary fills. A second different id fills the one optional `related_session_id` slot. A third distinct id refuses the write and names the two recorded ids. Arrival order decides which id is primary. The set of two is the durable fact. Both ids address the row, and either one can re-enter it. There is no lineage graph and no successor protocol.
13. **`recover <agent> [--session <id>]` restores a recorded session with explicit selection.** With one recorded id that id is used. With two the verb refuses until `--session` names one of them. Recovery chooses a valid id, never a winner. The selected session relaunches under the row's account and route, onto the recorded mux destination when there is one. `--print-command` prints the inspection form and touches nothing. The single-writer claim keys on the selected id. The primary and the fork can never run as two live writers.
