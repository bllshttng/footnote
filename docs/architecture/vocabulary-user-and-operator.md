...
The AGENTS.md pitfall needs both words to say what it says: "`fno agents mail send` injects as user-shaped text, indistinguishable from operator typing." A flat rename collapses the human being addressed into the authority being claimed, and the sentence that warns about confusing them stops warning about anything.
...
`crates/fno-agents/src/finalize.rs` keeps its generic human-facing string. A Rust copy of `display_name()` would be a second implementation of one behavior, and a parity harness forcing the two to agree would cost more than the string staying generic.

## If you are about to run sed
