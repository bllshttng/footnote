...
The round is the attestation law's unit (`d-608344c1`, 2026-09-03): a PR is reviewed when the ledger holds, for the current head or a head whose interdiff to it is under `review.carry_interdiff_lines` (default 100), one round with zero findings or two rounds with every finding disposed. A round is keyed by HEAD. Two verdicts at one unchanged head are one round, and a GitHub App review beside a local attestation at one commit is also one round - the counter is one shared set of heads across both evidence axes, not two axis counters reconciled by a max. A cap whose size depends on how a reviewer batches its output is not a cap.
