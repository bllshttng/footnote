...
The loop in one number: 32 review objects and zero approvals on a single pull request. Every round of that loop was a `COMMENTED` review. None carried a verdict anyone could act on. The pass condition was a clean round at head, and it never came. The gate this document describes exists to make that loop terminate, and this document states plainly what it does and does not close.

## The pass condition

A head is covered when the attestation chain's reviewed ranges tile `merge_base..head` AND every finding in that chain is terminal. A finding is terminal when one of four things holds. The four are listed below.
...
| 5 | IMPOSSIBLE | The round budget is spent with blocking findings still non-terminal. Re-reviewing cannot clear it. |
...
`config.review.max_rounds` (default 2, at least 1) budgets the review rounds on a branch since the last pass. A round is a review VERDICT. CI failures, lint failures and rebases are not rounds, and a pass resets the counter. When the rounds exceed the budget with blocking findings still non-terminal, the gate answers IMPOSSIBLE. It never teaches one more round.
