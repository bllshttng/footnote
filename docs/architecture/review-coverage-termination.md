...
`config.review.max_rounds` (default 2, at least 1) budgets the review rounds across the whole life of a PR. A round is a review VERDICT. A pass is one round like any other and refunds nothing, though it still satisfies coverage on its own terms. CI failures, lint failures and rebases are not rounds. A PR merges after one to three reviews and never waits for a round to come back clean.
