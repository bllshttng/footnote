...
- **A schema-validated `blast_decision` event instead of (or alongside) the stderr announce** - the announce is the whole record, deliberately. `target_cli.py` emits no events at all today, so the first one is not a call site: it is an `events/schema.yaml` entry, a row in `cli/tests/events/parity_corpus.jsonl`, and a name that `scripts/check-event-schema-parity.sh` must see identically from both the Python and Rust emitters. That is standing cross-language surface for a router that ships `enabled = false`, and there is no consumer on the other end - whether footnote does telemetry/analytics at all is still an open decision, so the event's shape would be guessed against a reader that does not exist. Revisit when an aggregate consumer is real and can say what fields it needs; the announce string is not a stable contract and should not be parsed in the meantime.
