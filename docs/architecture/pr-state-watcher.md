...
The tick's heal phase (armed via `config.auto_heal.enabled`) runs the same drive loop detached, and that loop can now rebase a PR whose base drifted: a conflicting PR, or the one holding the `merge-slot:<base>` claim, gets `fno do pr push` from its own worktree (fetch, rebase onto `origin/main`, preflight, push once), capped at six rebases per run. That is the actor behind the stale-base remedy: when `require_fresh_ci` holds a merge naming `fno do pr rebase <n>`, the next heal tick runs the equivalent push instead of leaving the hold to a human. The two actions and their receipt rows are documented in [pr-heal](pr-heal.md).
