...
Each tick the merge phase asks `grant-queue` for the granted PRs. It executes each under its per-PR lock. It emits `merge_grant_execution` `reserved` naming the actor, the PR, the node, and the grant session. Then it calls the internal durable-grant entry point. That entry is canonical `fno do pr merge` with the receipt as authority. Every ordinary guard runs identically: plan hold, in-flight review, incarnation fence, stub manifest, review coverage, review posture, CI green. The watcher can never merge a PR that an interactive merge refuses. The core's verdict lands as `executed`, `held` (retryable, no failure budget consumed), or `failed` (retry-bounded, then parked).
