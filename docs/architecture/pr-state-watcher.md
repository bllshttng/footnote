...
A dispatch's merge decision is recorded on the worker's own append-only `phase: do` graph row as a `merge_grant` receipt (`approved`, `source`, `recorded_by`, `recorded_at`), written by the SPAWNER at spawn time - a worker can never mint or rewrite one. A `--no-merge` dispatch records an explicit `approved=false`. One owner reads that record for status, merge, and the watcher: the Rust verdict in `crates/fno-agents/src/merge_grant.rs`, reached through the `authorized-merge` verb's `grant-verdict` and `grant-queue` ops (`fno/pr/_merge_grant.py` is the transport). It grants on nothing but a positive receipt: absence, malformed data, ambiguous PR-to-node links, a live or unreadable node claim, and a live config that no longer says `enabled=true, grant=dispatch` all refuse.

Each tick the merge phase asks `grant-queue` for the granted PRs and executes each under its per-PR lock: it emits `merge_grant_execution` `reserved` (naming the actor, the PR, the node, and the grant session), calls the internal durable-grant entry point, and that entry is canonical `fno do pr merge` with the receipt as authority. Every ordinary guard - plan hold, in-flight review, incarnation fence, stub manifest, review coverage, review posture, CI green - runs identically, so the watcher can never merge a PR an interactive merge would refuse. The core's verdict lands as `executed`, `held` (retryable, no failure budget consumed), or `failed` (retry-bounded, then parked).
