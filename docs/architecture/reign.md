...
The crown survives a compact; its evidence does not. After a compaction the session context holds a summary, and nothing forces that summary back against the store before the king acts on it. Two machines close the gap. The post-compact hook (`king-postcompact-reinject.sh`) reads the newest `isCompactSummary` entry from the transcript, by line position and never by timestamp, extracts the node ids the published grammar matches, resolves every candidate through one `fno backlog get` batch, and injects the answer as `unresolved:` and `resolved:` rows with one instruction: act on these rows, not on the summary. The stop gate adds a check beside the term gate: past `king.compaction_ceiling` (default 3) compactions since the manifest's `created_at`, a crown handoff doc that is missing or more than 24 hours old blocks exit and names the refresh command, `precompact-canon-doc.sh`. The doc gate counts boundaries only on claude transcripts and fails open on anything it cannot measure, because it blocks exit and a false block traps a session. One reader-authority rule covers both machines: a node's state comes from `fno backlog get`, never from a `graph.json` read, because with the sqlite backend the file is the relational store's stale twin.
