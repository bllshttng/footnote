...
## Manifest-only crowns

When no live row holds its scope, the court lists its manifest. Only the newest manifest naming a session counts, so a re-scope leftover is not a crown. A holder the claude roster lists as finished drops out. A holder the roster does not list stays, because absence is not a death proof. Liveness keys on the session id, and new manifests carry no `owner_pid`. An attended `fno agents king done --scope <scope>` clears a listed leftover.
