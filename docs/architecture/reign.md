...
1. **Fleet settled-PR wake, 600s.** The stop hook fires on an agent stop, and on nothing else. A session can stop while its PR is pending. Its CI can settle an hour later. Nothing watches for that. This arm does. The daemon nudge ladder is the central poke for every quiet session on an open PR. Its resume rung delivered 1 of 150 resumes from 2026-09-19 to 2026-09-21, so each crown keeps this wake over its own rows as the backstop. The court fold marks each node `owned` by one crown, so an L1 and an L2 never both poke one session. The join, the key and the skips live in the skill's Arm the beat section, and this page does not restate them.
