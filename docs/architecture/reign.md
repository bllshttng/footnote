...
`fno agents king ledger` renders `<state_dir>/reign.html` (`--out` overrides) as the Crown Ledger page: a masthead with the generated stamp, a verdict card (the court agrees with itself, holds no live crowns, cannot be read, or disagrees with itself) with count tiles and the readers line, crowns grouped into rungs with per-crown cards (rung badge, status dot, agree and source tags, node/active/done stats, a stacked status bar with legend, and an Active territory table listing active nodes), plus uncrowned-epic and orphan-leaf sections. Only active nodes render as table rows; settled members show as counts in the bar and legend. The data path is the court's own. `gather_court` and the native court-fold read run in Python. The page assembly runs in the native `reign-ledger` verb (the king-history split, so the Python-tree ratchet holds). The page renders that answer and never re-derives the scope join, so the ledger, `fno agents court -n`, and the fold cannot disagree about who holds a node. The `/crown` bridge route serves the current file at once and, when it is older than five minutes, starts one background `fno agents king ledger` render (single-flight, so concurrent reads start no second child); the page prints its generated stamp and a relative age, never its own stale verdict. An empty court renders "no live crowns" as a measurement. An unreadable registry renders the named reason. An unresolved fold states why in place. Never a blank or falsely healthy page.
