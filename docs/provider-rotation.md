...
A credential the endpoint *rejects* (401/403) is dead and set aside; a credential whose call merely goes unanswered (timeout, 429, 5xx) blocks the whole slot, because a live account we failed to ask about could be the one `claude` is actually reading.
