...
A 2026-08-15 incident found the theory does not hold for a *blocked* row. A supervisor can be registered as reachable and still sit at a populated prompt, with no live model turn in flight. Non-interactive `claude attach` has no pty and may print "Attaching..." before exiting without delivering anything. The Rust live arm now reads the target account's Claude roster, refuses a working or done row when a message was requested, and sends the wrapped message over `control.sock` with transcript confirmation. Python no longer wakes Claude rows.

**The Rust arm now owns live Claude resume directly.** `resume` is in `RUST_CLIENT_VERBS`, so it auto-routes to the installed `fno-agents` binary. Its `claude_live_route` in `crates/fno-agents/src/resume_wake.rs` reads the plan-pinned account roster and sends messages over `control.sock`; it no longer delegates back to Python or pins `FNO_AGENTS_RUNTIME=python`. The Python `resume_cli.py` refuses Claude rows with exit 13. The dead-row `claude --resume <uuid>` arm still restores a recorded route through `--settings`.
