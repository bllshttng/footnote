...
Every attempt persists as one history row with a unique `attempt_id` and an `obs` block of structured observations (fixture prepared, worker started, grader ran, grader result). The `fno-agents evals-attempt` door classifies each row from those observations alone: `graded` (a valid task grade), `infrastructure` (fixture never prepared; retryable), `unavailable` (worker lost: spawn refused, timeout, cancellation, gate; retryable), `ungraded` (worker ran, no grader result; retryable), or `legacy` (no structured evidence). Legacy rows are never reinterpreted, and a task whose rows are all legacy keeps the pre-attempt boolean fold, so old history keeps its alarm semantics.
...
The report never lets a plumbing failure dilute the headline. A modern task's `pass@1` divides passes by **valid grades**; infrastructure, unavailable, and ungraded attempts stay visible in `attempts` and in `runs`. A task's rows are never dropped: with `--planned '{"task": N}'` the report also shows `expected_attempts`, `missing_attempts`, and `completion`, so unrun work can never improve the picture.

**Declared cohorts** live in `evals/bank/cohorts.yaml`: `bank_rev` (the pinned revision) plus `train`, `validation`, and `qualification` id lists. The split is validated natively (existence against the bank, uniqueness, disjointness) before any worker call, and again at export and qualification time, fail-closed: an unreachable door refuses. A split pinned to a bank rev is void when the bank files change after the pin; redeclare it. Nothing infers a percentage split from a tiny bank: no declaration, no held-out claim.
...
Without a forcing function an eval harness dies. The bank sat fully built while its history went quiet. A 40-day-old 100% rendered exactly like a fresh one. The demand is built in now. When the bank ages past the schedule, the tick launches the regression tier detached. The health surfaces go red past the window.

- **The tick and the arm.** The daemon's evals phase resolves paths and schedule, then invokes the native `fno-agents evals-arm`. The arm prints one JSON receipt line. It never runs the bank inside the tick, whose wall budget is 480s shared by nine phases. When the newest regression run is older than `evals.schedule_days` (default 7, `0` disables the leg), the arm fires. It also needs spawn-gate headroom. Then it launches `fno doctor evals run --tier regression -y` detached. The child leads a new process group with stdio on /dev/null. The arm holds the `evals:scheduled-run` claim across ticks. No second run starts while one is in flight. The run is bounded by its own 3600s budget, above the tier's 50-minute task-timeout sum. The group is killed on timeout. Graded fails are data: the run still journals as scheduled. Plumbing failures are could-not-fire: a refused gate, a timeout, a non-zero exit, an exit-0 run with no rows. A gate refusal means slots at `agents.max_live` or RAM under `agents.min_free_gb`.
- **The journal and the notice.** Every outcome lands in the global events journal. A pass or graded fail journals `evals_scheduled_run` with task count, passes, and duration. Rows are attributed by their own timestamps, never by a count delta. A plumbing failure journals `evals_stale` with the reason. Past twice `evals.stale_days`, it also carries one OS notice per schedule window. The journal row IS the dedup, so no state file of its own exists.
- **The red row.** `fno backlog triage health` renders the evals line with the newest run's age (`age 9d STALE`). When the trend names regressions, it appends ` REGRESSED <n>`. When the trend drops a task, `fno doctor` adds the REGRESSING line. A stale bank prints STALE with the age and the run command. No history or no regression row prints UNKNOWN. Unknown never asserts staleness. A fresh, trending-clean bank prints nothing. A red evals row is an escalation, never a gate. Nothing refuses while the bank is stale. The row is what demands the run.
...
- **Threshold: three jobs.** Set `config.evals.scratch_threshold`, default 3. In the census, every shape at three or more jobs had a verb remedy. Every shape at two or fewer was one-off or other-project. Two can be one agent twice, because a resume mints a new job dir. Shapes rank by jobs, then files.
- **One node per run.** `MAX_FILES_PER_SWEEP` is 1. A long list of new nodes is the failure the operator named. Five shapes crossing in one day file one node today. The next files tomorrow. A shape whose `scratch_shape_filed` row names a live node folds onto it (`--wave-of`, no new id). A node done inside the window suppresses. A node done before the window refiles with `--caused-by`.
- **State words on stdout.** One per line, never silent: `ok`, `insufficient: <reason>`, `filed:<id>`, `folded:<id>`, `suppressed:<id>`, and `would-file:<shape>` under `--dry-run`. The sweep classifies shipped fixture specimens before touching the real jobs dir. A classifier that finds nothing there prints `insufficient: control failed` and emits nothing. A broken classifier can never read as a quiet machine.
- **Verb hint.** Details name the census rule, the file and job counts, and the three newest specimens. The remedy names the existing verb plus its missing flag, or `no verb today: ...`. Before filing, the sweep probes `fno help <group> --all`. That surface lists hidden leaves, and `fno <group> --help` does not. A remedy that ships hidden is named as existing, never filed as missing.
- **Reading it.** `fno doctor scratch report` renders the ranked table from the journal: shape, files, jobs, monitors, first and last seen, filed node. `--json` serves machines. The sweep runs as stage five of the daily eval-sweep ignition. It is bounded and logged to `.fno/logs/eval-sweep.log` like every other stage.
...
Every scoreboard view reads one delivery classifier (the Rust keeper's
`scoreboard_classify` op). It distinguishes a merged PR, a delivered code
node, a delivered document and a session stop. A confirmed merge delivers
with no ledger row; a session terminal on a known unmerged node records
evidence and never a delivery; a terminal on a node the graph lost counts
as `inferred`, never as a confirmed merge.

Quality cohorts judge only deliveries that completed the 14-day observation
window; younger deliveries are reported pending, not in the denominator.
The human_touch emission-failure counter is server-instance telemetry, read
from `fno mux stats --json` with its own measurement window; an unreachable
server renders as Unknown, not as a measured zero.
