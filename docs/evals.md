...
- **The recurrence unit is (job, shape).** Files per job measure one agent's habit. Distinct jobs measure the fleet's need. One king writing 158 mail wrappers in a night is one recurrence. Each new pair emits one `scratch_shape_observed` row to the global journal. A pair already recorded inside the window never re-emits. The journal read covers `events.jsonl` and its rotated sibling. That is the ceiling: one 8 MB generation plus the one before it.
- **Threshold: three jobs.** Set `config.evals.scratch_threshold`, default 3. The threshold counts every walked pair in the window, remembered or not. Two can be one agent twice, because a resume mints a new job dir. Shapes rank by jobs, then files.
- **Only a missing remedy files.** A shape at three jobs does not always have an fno remedy. The census measured `file_patch` at 26 jobs and `git_wrap` at 17 with none. Each shape's remedy row says whether it may file. A report-only shape still emits observed rows and shows in the report. Before filing, the sweep runs the row's help probe. A match means the remedy shipped, so it prints `served:<shape>` and files nothing. A probe call that fails files with the table hint, so a broken probe never hides a missing remedy.
- **One node per run.** `MAX_FILES_PER_SWEEP` is 1. A long list of new nodes is the failure the operator named. Five shapes crossing in one day file one node today. The next files tomorrow. The filing index is the graph. Every sweep node carries `origin_evidence` `scratch-shape:<shape>` and is minted with `--separate`, so text similarity never picks the node. A live node with that key folds (`--wave-of`, no new id), but only when a scratch file is newer than the node's last write. A same-day re-run adds nothing. A keyed node done inside the window suppresses. A keyed node done before the window refiles with `--caused-by`.
- **State words on stdout.** One per line, never silent: `ok`, `insufficient: <reason>`, `filed:<id>`, `folded:<id>`, `suppressed:<id>`, `served:<shape>`, and `would-file:<shape>` under `--dry-run`. The sweep classifies shipped fixture specimens before touching the real jobs dir. A classifier that finds nothing there prints `insufficient: control failed` and emits nothing. A broken classifier can never read as a quiet machine.
- **Verb hint.** Details name the census rule, the file and job counts, and the three newest specimens. The hint names the missing verb or flag. A probe on a hidden leaf reads `fno help <group> --all`, because that surface lists hidden leaves and `fno <group> --help` does not.
