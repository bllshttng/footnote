...
The repeated stress jobs executed 1,265 cases across 23 baseline trials. Cutover will execute 1,104. That is 161 fewer cases. The changed-smoke packet adds a one-trial sample with 48 cases after skips. The ordinary Rust integration runs all 50 cases once. CI gains three Bash specs and 16 Python cases. They were not routed before. `scripts/tests/test-spec-suite.sh` passed the smoke registry. Claims passed 12/12. Executor passed 17/17. Blueprint phase-close passed. Pytest passed 16/16. The smoke shard wiring test passed 1/1. The post-merge full run passed one trial: 27 daemon, 15 persistence, and 8 workspace cases, with no daemon leaks.
