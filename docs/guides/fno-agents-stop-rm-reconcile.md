...
Worktrees are not removed here for non-claude harnesses. Nothing on the registry row distinguishes an isolated worktree from the canonical checkout, so `rm` will not guess. Reap them with `fno agents workspace worktree cleanup --merged --apply`.
