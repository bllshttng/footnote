...
Nothing on the registry row distinguishes an isolated worktree from the canonical checkout, so `rm` will not guess; reap them with `fno workspace worktree cleanup --merged --apply`.
