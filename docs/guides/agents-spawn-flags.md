...
`--crown/-k` grants an orchestrator crown over the named territory. Repeat it with epic id(s) to crown a Director over the set. One project name crowns a project king. Several projects crown a portfolio. The altitude derives from what you name. There is no `--level`. A node that is not an epic is refused, because implementers get no crowns. The grantor derives from THIS session and is never self-declared. Crowns work on the pane and thread substrates. Thread crowns are claude-only until the court plumbing learns the opencode serve lane. Headless refuses, because its one-shot exits before it can reign. If the caller already holds the named territory, or a live holder occupies it, add `--succeed`. From an agent's own session, that flag transfers the agent's own crown to the heir, stripping it atomically in the same write. It cannot take over a crown another live agent holds. An attended shell (no agent identity) with `--crown <scope> --succeed` transfers the crown from any live holder, because a human can grant any scope. Without `--succeed`, a crowned spawn over a scope another live row holds refuses before launch instead of stranding an heir with no crown. A live crown over overlapping territory refuses the spawn too, with or without `--succeed`. A spawn over one epic of a live epic-set crown never launches. So does a spawn over a set that shares an epic with a live crown. `--succeed` hands down only an identical crown. A project or portfolio crown above the scope is the new king's court, not a rival, so it does not block. The transfer is checked again at the registry write: if the holder changed since the check, even to a new session under the same name, the heir launches uncrowned and prints `crown declined`.
