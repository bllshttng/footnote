...
`--session-phase` stamps the sessions row that a node-bearing spawn opens on the node. Empty infers from the message through the verb table (`crates/fno-agents/src/spawn_phase.toml`): the first token's verb names the phase in its bare, `/fno:` and `$fno:` spellings. A `--node` spawn the table cannot label is refused before any worker launches. Pass the phase explicitly. A spawn with no `--node` writes no row.
