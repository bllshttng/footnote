...
A red is named for the kind of row that failed. Once a real check-run reaches a failing conclusion, `ready_blockers` carries `ci_red`. When every failing row is `CANCELLED` or `STALE`, it carries `ci_cancelled_retrigger`. The verdict and exit code remain red, but the name tells the caller to trigger a newer run. When every failing row is a commit status and no job failed, it carries `commit_status_red`. All three block. A concluded failure dominates a cancelled or stale sibling, so a mixed result remains `ci_red`.

A review that is still RUNNING blocks too. It carries its own names, apart from every other review signal. `review_coverage` answers what verdicts EXIST for a head. A review still writing its fixes has produced none, so it stayed invisible. When a review registers a hold on the PR's branch, `ready_blockers` carries `review_in_flight`. When that hold will not read, it carries `review_hold_unreadable`. When the local worktree on the branch says the merged code is not the code someone is editing, it carries `worktree_dirty`, `worktree_head_mismatch` or `worktree_probe_failed`. The whole reading rides in `review_activity` whether or not it blocked. So a clear answer shows up as a probe that ran, never as a silence. This conjunct is deliberately config-independent. A running review blocks whether or not the project requires any review. See [review-lanes](../architecture/review-lanes.md#the-in-flight-review-hold).

The answer can come from the coalescing cache. That cache keys one row per repo, PR and head, so a verdict never answers for a commit it was not computed at. A served answer says so in `cached`. It says how second-hand it is in `cached_at` and `cached_age_seconds`. The `head` field names the commit the verdict was computed at. To refuse the cache for one read, pass `--refresh` (`--no-cache` is the same flag). When a verdict looks wrong, use that flag by hand. Do not put it in a poll loop: the fleet's one GitHub request budget (ledger at `~/.fno/locks/github-request-budget.json`, readable with `fno-agents gh-budget status`) is what admits every gh call against the shared secondary limit.
