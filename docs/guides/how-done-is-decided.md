...
A review that is still RUNNING blocks too, and it is named separately from every other review signal. `review_coverage` answers what verdicts EXIST for a head; a review still writing its fixes has produced none yet, so it was invisible. `ready_blockers` now carries `review_in_flight` when a review registered a hold on the PR's branch, `review_hold_unreadable` when that hold could not be read, and `worktree_dirty`, `worktree_head_mismatch` or `worktree_probe_failed` when the local worktree on the branch says the code that would merge is not the code someone is working on. The whole reading rides in the `review_activity` field whether or not it blocked, so a clear answer is visible as a probe that ran rather than as a silence. This conjunct is deliberately config-independent: a review that is running blocks whether or not any review was ever required. See [review-lanes](../architecture/review-lanes.md#the-in-flight-review-hold).
