...
A red is named for the kind of row that failed. Once a real check-run fails, `ready_blockers` carries `ci_red`. Once every failing row is a commit status and no job failed at all, it carries `commit_status_red` instead. Both block and the verdict is red either way. A failing status is a real red, so it is renamed rather than dropped: `stacked-base-guard` is a status and no other blocker names it.
