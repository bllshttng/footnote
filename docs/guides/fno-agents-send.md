...
The recipient sees the body inside the `<fno_mail from="..."[ harness="..."][ from_rank="..."][ to="..."][ to_rank="..."][ id="..."]>` envelope. `from` holds the sender's full session id when one is proven. The framing marks the sender as a peer, not the operator. The receiving model responds in a directed style rather than treating the injection as a user interrupt. See [docs/architecture/mail-live-inject.md](../architecture/mail-live-inject.md) for the envelope and delivery model.
