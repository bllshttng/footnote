...
The recipient sees the body inside the `<fno_mail from="..." id="...">` envelope. The framing marks the sender as a peer, not the operator. The receiving model responds in a directed style rather than treating the injection as a user interrupt. See [docs/architecture/mail-live-inject.md](../architecture/mail-live-inject.md) for the envelope and delivery model.
