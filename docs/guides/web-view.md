...
On a bridge bound to loopback, the node panel can also act. You can edit a node's title, priority, size and status, rank a card to the top or bottom of its column, and launch a blueprint or a target for the node. Every write runs the same `fno` verb an agent runs, and the verb's own refusal is what you see. Three conditions gate every write: the bridge must be bound to loopback, the request's Host header must name loopback, and the Origin header, when your browser sends one, must name loopback too. The reason is plain: a web page on another site must not be able to drive the bridge through your browser. On a bridge bound wider than loopback, the page shows no write controls at all.
