...
The **crown** page serves `reign.html`. The bridge re-renders it if it is older than five minutes. The **fleet** page serves `fleet.html`. The fno-agents daemon renders it every 30 minutes. If `/fleet` returns 404, the daemon has not ticked yet.
