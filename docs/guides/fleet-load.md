...
The daemon's `fleet_page` arm rewrites `fleet.html` every 1800 seconds. The web bridge serves the same file at `/fleet` behind its existing token. A 404 means the daemon has not completed its first page tick; run `fno-agents intel --fleet --html` when an on-demand render is needed.
