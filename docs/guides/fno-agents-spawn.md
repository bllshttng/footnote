...
In the `fno mux` sideline selector, plain Enter on an attachable agent row attaches it watch-only as a new tab.
Press `p` on that row to choose the placement instead:
...
Every other key revalidates before sending, and reports a notice rather than acting if the agent stopped being attachable or the target workspace disappeared.
