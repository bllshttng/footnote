...
2. Optionally add a bot-side sentinel plugin. When a `<promise>` tag appears in the assistant response, it writes `.fno/target-promise.signal`. The protocol lives in `docs/harnesses/promise-sentinel.md`.
