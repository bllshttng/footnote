...
| `fno mail send <name> "<text>"` | yes | yes | yes | queues durable | queues durable | Async, live-inject first; never waits for a reply. A live delivery (`delivered (hosted)`) writes nothing to the bus; only a miss falls to a durable envelope (`queued (durable)`), which is recovery rather than delivery. Live lanes exist for claude/codex/gemini; a suspended/watch-only worker takes the durable fallback, so `resume` it and re-send instead of waiting. Registered-row required; unknown names exit 16. |
