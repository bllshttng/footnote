...
| Revive a **dead** session | `spawn --resume <uuid>` (bg lane) or `recover` | no | no | no | no | yes: a fresh process on the same `(cwd, session_id)` pair recalls prior context and prints no creation warning | no |
