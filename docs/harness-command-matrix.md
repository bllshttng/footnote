...
| `spawn --harness <h>` / `-H <h>` | selector | selector | selector | selector | selector | Canonical CLI-binary selector (`claude\|codex\|gemini\|opencode\|agy`); the `--harness` vocabulary the rest of fno uses. `--provider`/`-p` is the deprecated alias (a conflicting pair exits 2). Reassigned from headless: `-H` now takes a harness value, not a one-shot toggle. |
