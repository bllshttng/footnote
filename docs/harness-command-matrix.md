...
| Headless one-shot (`--substrate headless` / `--headless` / `--once`) | yes (`claude -p`) | yes (`codex exec`) | yes (one-shot) | yes (`agy -p`) | **no** (refuses, pointing to pane) |
...
| `spawn --substrate headless` / `--headless` / `--once` | yes | yes | yes | yes | no | One-shot: create + exchange + teardown. stdout is the provider reply. (`-H` is NOT a headless spelling; it selects the harness - see below.) |
| `spawn --harness <h>` / `-H <h>` | selector | selector | selector | selector | selector | Canonical CLI-binary selector (`claude\|codex\|gemini\|opencode\|agy`); the `--harness` vocabulary the rest of fno uses. `--provider`/`-p` is the older spelling of the same axis (a mismatched pair exits 2). Reassigned from headless: `-H` now takes a harness value, not a one-shot toggle. A model VENDOR (`zai`, ...) is never a harness value; that axis is `--route`. |
