...
|---|---|---|---|---|---|---|---|---|
| Substrates | pane, **thread**, headless | pane, headless | pane, headless | pane, headless | pane, **thread**, headless | pane | pane, **thread** | pane only |
| Persistent-thread lane (`--substrate thread`) | yes (`claude --bg`) | no (hard error, use headless) | no | no | launch-only (a shared `opencode serve` over HTTP; `ask`/steering unbuilt, so the capability bit reads false and autonomous dispatch defaults to headless until the steering lane ships its own journey test) | no. The transport exists and is tested (`pi --mode rpc`: strict JSONL, typed events, an `id` per request, native `steer` injection, stdin held open because rpc exits on EOF mid-turn with status 0), and the keeper lane is proven (the restart journey, `test_thread_keeper_journey.py`), but the dispatch_spawn arm is still unbuilt, so the bit reads true and the public spawn still keeps pi on `pane` until that arm ships. | yes. fno's keeper hosts the TUI's pty on the generic lane-B entry: `create-chat` mints the chat id before launch, the keeper survives supervisor death, and the journey backs it (`test_cursor_agent_keeper_journey.py`). No daemon and no bidirectional transport exists; the pane remains the attended substrate. | no (refused naming the harness; pane is its only substrate) |
| Headless one-shot (`--substrate headless` / `--headless` / `-p` / `--once`) | yes (`claude -p`) | yes (`codex exec`) | yes (one-shot) | yes (`agy -p`) | yes (`opencode run`) | the argv is built (`pi --print`), but no dispatch arm reaches it | no. The output-only `--print` stream has no input half; refused by name. | no |
