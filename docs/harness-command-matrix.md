...
Before calling a newly declared lane supported, run `fno doctor harness <name> --live`. The probe reads behavior on the real lane for SPAWN, IDENTITY, CLAIM, MAIL BOTH WAYS, VIEW, SURVIVE, ROW MATCHES, and MANIFEST PINNED; the capability table is input only to ROW MATCHES and is evidence for no other line. Without `--live`, it prints all eight markers and the argv it would run without spawning.
...
The support probe is the onboarding gate for this table: `fno doctor harness <name> --live` must produce a positive marker for every non-skipped line before a row is treated as support. A missing credential is a named `skip`, not a pass or fail. Run `scripts/ci/check-harness-capabilities-fresh.sh` alongside the honesty sweep for the three committed table copies.
