...
| `resume <name> [--print-command] [--message/-m]` | yes (live only) | yes | yes | no | yes | Re-exec the harness's resume CLI in the agent's recorded cwd. A **live** claude row still exec's `claude attach <short_id>`. A **blocked/stopped** claude row instead wakes the session headlessly (pty-backed, route-settings-restored) and verifies it moved, since `claude attach` run non-interactively just prints "Attaching..." and exits. Not a dead-session revival (that is `spawn --resume`). `--print-command` prints the shell snippet instead of exec'ing; `--message`/`-m` is the text delivered on wake. |
