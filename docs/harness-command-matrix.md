...
- **opencode** is pane-hostable with a readiness detector and badge manifest, but fno does not probe or resume its session IDs this release; fno drives it via the pane. The fno plugin exposes the footnote verbs in opencode's command palette AND headlessly (`opencode run --command fno:target` resolves the plugin registry - verified against opencode v1.14.50), so dispatch renders the native `/fno:verb`, not a prose brief.
