...
- The wait is bounded by `--wait-timeout`. The default is 5400s (90m). A value of `0` restores the old immediate fail with exit 3. While waiting, the holder's pid, start time, and elapsed print every 60s.
