...
Preflight is an OPT-IN rehearsal of CI, never the gate. CI on the PR head decides the merge; a stock config (`config.preflight.required` absent or false) never runs this before a push. What it buys a worker or project that opts in: `scripts/ci/preflight.sh` runs CI's verdict locally before you push, so a local green means a green PR and the push-wait-red-fix loop (each ~10-minute CI round surfacing one new failure) dies before it starts. The default is false because the rehearsal costs 22 to 47 minutes and cannot even start under the 256-file-descriptor limit launchd hands every spawned worker: on 2026-08-19 six concurrent runs aged past an hour at load average 415 with zero completions while three workers held green commits they could not push. On a stock config the pre-push obligation is the focused checks (`cargo fmt --check` when rust changed, markdown style lint on changed `.md`, PR-body length, the blast-radius tests); CI re-runs all of it. Set `preflight.required = true` to restore the mandatory rehearsal per project.
...
- Local verification is off by default; a stock config never requires it. On an opted-in project, `FNO_SKIP_PREFLIGHT=1` before `fno pr create` or `fno pr check` skips this one push and relies on CI instead.
...
Preflight runs before a push only when the policy asks for it. The single decision point is `fno pr evidence-required`: it reads `config.preflight.required` (default false) plus the `FNO_SKIP_PREFLIGHT=1` escape, and the ship phase in `skills/target/references/ship-phase.md`, `fno pr create`, the worker ship lane, and the batch lane all ask it, so the bash path and the Python lanes cannot disagree. A failed or unparsable policy call fails open: no local preflight, CI verifies the PR head.

On a stock config the pre-push obligation is the focused checks, seconds each: `cargo fmt --check` when rust changed, `fno lint style --surface markdown --files <changed .md> --diff-base origin/main` when markdown changed, `bash scripts/ci/check-pr-body-length.sh` before opening the PR, and the tests covering the diff's blast radius. There is deliberately no wrapper script for them. Every check already runs in CI, and a new mandatory local runner would rebuild the gate this contract removes, one rung smaller.
...
The runner guard is an existence check (`[[ -x scripts/ci/preflight.sh ]]`), a repo-neutral convention that no-ops in any repo which does not ship the script; the policy call sits in front of it. Skips stay explicit and auditable: a stock config, `FNO_SKIP_PREFLIGHT=1`, or a docs-only diff (only documentation, the vault dir, and `*.md` files). The scripts never self-skip. The skip decision lives in `fno pr evidence-required`.
