...
- Run only the tests that cover the files you changed (`fno doctor test <test files>`); never the whole suite locally - CI runs every suite on every PR. A whole-suite local run, when genuinely needed, starts as a background task: it queues on `test:suite` and the turn never blocks.
