...
- **Exit 4**: no provable per-worker identity, or this holder identity is live under another process. A stop, not a skip: emit `<help reason="task claim needs a session id" evidence="task <id> of node <node>">`.
