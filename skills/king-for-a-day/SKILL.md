...
**Place the king in the mission workspace too, and for a court that is not optional.** A court king spawns every teammate into the epic-named workspace, so a king that took the cwd-routed default sits outside the court it is running - and since no CLI path moves a running pane, that is not fixable afterwards without killing the king. Pass `--workspace <epic>` at coronation, and pass it again when you anoint a sub-king. A pass does not need it (it abdicates before layout matters), but it costs nothing there either.
