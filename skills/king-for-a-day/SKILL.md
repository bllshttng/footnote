...
3. **Ask for a review.** A minion cannot self-invoke its harness's native review verb (claude `/code-review`, codex `/review`) - it is user-triggered. When it finishes a unit of work it reports `RESULT: ...` and mails you for the review; your reply injects as user-shaped text in the minion's pane and its OWN harness serves the verb. **A done report is a review request: answering it is your job**, the same class as answering an in-scope question. Mail the trigger so the review runs in the worker's harness, do not run it in yours, and never fan out a sigma panel the worker did not configure. The verb per harness, the retry rule, and the never-substitute-silently contract are in [references/review.md](references/review.md) - read it before you answer, because mailing a claude verb to a codex worker sends an unknown command.
