...
**What an unstamped row costs, so nobody mistakes the silence for success.** Three readers key on those fields and every one of them fails CLOSED and QUIETLY. `fno agents king done` refuses, so the king cannot abdicate and its manifest is orphaned on disk. `fno agents king manifest-path` answers EMPTY at exit 0, so the stop hook leaves `KING_STATE_FILE` unset. `hooks/king-postcompact-reinject.sh` exits 0 before injecting, so the post-compact brief - the one that tells you to reuse a live session with `fno agents retask` before spawning a cold one - never arrives. A king with an armed manifest and an unstamped row looks exactly like a working king until it tries to put the crown down. `king init` now warns when it sees that state; heed the warning rather than proceeding.
