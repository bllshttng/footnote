...
**Coronating in place cannot place you, and that is fine.** `fno agents crown <handle>` writes the crown onto an existing row and has no workspace option, so a session coronated where it already sits cannot be relocated - no CLI path moves a running pane. Do not try to fix it: the workspace is an identity, not a location, so declare the workspace you are already in to be the mission workspace, name it in your opening line, and let teammates anchor to you there. What actually matters is that the court is co-located with its king, and `--at current` delivers that from whatever workspace you woke up in.
...
- **You cannot migrate a running pane from the CLI, so place it right at spawn.** No `fno mux pane` verb moves one between workspaces: `break` detaches a pane into a new tab, and that tab stays where it was. The limit is on scripted control, not on the mux - a human at the keyboard can move a pane, send a whole tab to another workspace, and recruit a running agent into a named workspace as a watch-only member. All three are TUI paths, and a king drives CLIs, so none of them is yours. Adopt an already-running worker logically instead - it holds the node claim and it reports to you by mail - and never kill a healthy one to improve layout. Placement corrects itself for free at the next real handoff, because the successor is spawned into the mission workspace.
