...
**Rule.** A ruling that changes what a worker does is recorded with `fno backlog note <node> <text>` - the king's channel, which mails the row's live holder and the epic's king as it writes, so the ruling reaches the worker without a second call (`--quiet` suppresses the mail). When nobody bound to the row would be told, it exits 3 and writes nothing; read the refusal, then mail a reader by name or pass `--quiet`. `fno backlog note` is the ruling verb because `fno backlog decide` refuses every agent session including a crowned one (operator authority is never inherited by an agent, and the refusal is by design; `decide`'s `--authority` enum is `operator|crown|agent|beastmode`, and a resolved session is refused from the operator value regardless). A ruling that CONDITIONS A MERGE is different: a note reaches the worker, but only the hold reaches the merge gate, so set it with `fno do pr hold set <node> --reason <condition> --release-when <proof> --set-by <crown>`; `fno do pr merge` and pr-watch refuse the held PR naming the condition, the worker lifts it with `fno do pr hold release <node> --evidence <proof>` once the condition holds, and the ruling author can release it too. For a durable rule the OPERATOR asked for, `fno inbox law set <subject> <decision> --rationale "<why>"` records it as chat-attested and can never supersede the operator's own law; `fno backlog decisions <same>` and `fno inbox law decisions <subject>` read them back, newest first. A subject matches exactly, so never mint a near-synonym. Every ruling is machine-local project policy: a rule a stranger cloning the repository must obey does not reach them from here, so land it in the code, a doc, or a gate, in a PR. See [decision-record](../../docs/architecture/decision-record.md).
