...
- **never batch a task with an order to compact.** Compaction drops the rest of that turn. Send the compact instruction, confirm it landed, and only then send the task in a separate message.
