...
- **never batch a task with an order to compact.** Compaction drops the rest of that turn. Send `fno mail send <handle> '/compact' --raw`, confirm it landed, and only then send the task in a separate message.
