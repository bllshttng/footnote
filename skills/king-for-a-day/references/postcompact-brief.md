...
- **Vote once per session on what cost you time.** `fno backlog encounter <id> --evidence "what it cost"` records one encounter with a node you hit while doing something else. Evidence is required and the cap is one per session, because a vote no transcript can vouch for is a poll. `fno backlog demand` is the read, and it is a column you rank FROM, never a rank it writes.
