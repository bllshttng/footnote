...
- **Reap by last activity, not by a live pid.** `fno agents rm <name>` tears down the harness session record and the fno row together, in that order. Retain the full `harness_session_id` as the recovery handle. The transcript survives on disk, so a reap you regret is reversed with `fno agents adopt <harness_session_id>`; a short id is only a best-effort lookup while durable evidence still resolves it. Never run a bare `claude rm <id>`: it forfeits resumability and buys nothing rm did not already do.
