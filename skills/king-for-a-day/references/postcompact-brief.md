...
- **Vote on each node that cost you time.** `fno backlog encounter <id> --evidence "what it cost"` records one encounter with a node you hit while doing something else. Evidence is required, and the cap is one vote per node per session, so a session that hits three nodes files three. A vote no transcript can vouch for is a poll. `fno backlog demand` is the read, and it is a column you rank FROM, never a rank it writes.
