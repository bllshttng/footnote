<!-- style-exception: the five rules are the operator's verbatim corrections, re-issued unchanged across three compactions; shortening them to meet the sentence cap would alter the operator's words, and the carrier is model context, not human mail. -->
...
- **Reap by last activity, not by a live pid.** `fno agents rm <name>` tears down the harness session record and the fno row together, in that order. Retain the full `harness_session_id`; recover one-step with `fno agents resume <full-harness-session-id> --cross-project --cwd <existing-checkout>`, or re-adopt identity only with `fno agents adopt <full-harness-session-id> --cross-project`. Never rely on a short id for machine-wide recovery. A pruned worktree's uncommitted contents are not recoverable by this verb. Never run a bare `claude rm <id>`: it forfeits resumability and buys nothing rm did not already do.
