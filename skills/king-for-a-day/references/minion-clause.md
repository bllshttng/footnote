...
CLAUSE
fno agents spawn --name <node-name> "$payload" --substrate pane --workspace <w> --split <dir> --effort high
```

The clause itself is substrate-independent - it rides every spawn payload, pane or not. Only the placement half of that command line is pane-only: drop `--workspace` and `--split` when the substrate is `bg` or `headless`, which refuse them, and let the graph carry the mission instead.
