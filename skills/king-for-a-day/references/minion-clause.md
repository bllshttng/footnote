...
- Delivery doctrine: send with --from-self, and treat any receipt that is not `delivered (hosted)` (or `delivered (woken)`) as NOT delivered - re-resolve my handle and re-send, never re-queue.
