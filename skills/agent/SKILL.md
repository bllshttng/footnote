...
- `result=already-running ...` -> a worker already exists for this node/name; no second loop was created. Point at its logs. To replace a stuck worker instead, run `fno agents stop <worker>`, then spawn again with `--node <node>` (add `--resume <old session id>` on claude to carry the old transcript); the stop frees the node at once, so neither step needs `rm --force`.
