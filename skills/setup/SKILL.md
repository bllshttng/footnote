...
The `attention` field rides this same model. Ask it in plain words: "When you are away from the terminal, where do questions reach you?" The choices are "a notes file (Obsidian or any markdown app)", "phone push" (after the ntfy sink ships) and "nowhere yet". An empty answer configures nothing. When the user names a file, write `[[attention]]` through `fno config set config.attention`. Then run `fno-agents needs --items --json` yourself. Tell the user in one line what will appear in the file, and how to answer: tick a box, or write words under the line. The user runs no command. See [attention-items](../../docs/architecture/attention-items.md).
...
A rejected value (e.g. a reserved `id_prefix` like `tgt-`, or a length/charset violation) exits non-zero and leaves the file unchanged. Re-prompt and retry. NEVER hand-write a scalar leaf to bypass validation. The one sanctioned exception is the Step 3 `config.work` structural map, validated by `fno config doctor` immediately after.
...
Map the answer to keys and write them (use `--local` for a per-repo gate; global otherwise). `/target` runs whatever you set here before it promises - so a `reviewers`/`peers` gate always has a producer and never hangs the loop:
...
fno config set config.review.peer_token_env GH_PEER_TOKEN --local
```
...
`fno config doctor` checks your paths. `fno doctor --blockers` checks this machine: stale install, dead LaunchAgents, low open-file limit, and the other findings that make the fleet misbehave. Relay its lines to the user verbatim. Do not summarize them away. A blocker is exactly the thing a new user needs to see and act on.

For the full, generated reference of every key (type, default, doc), point the user at `docs/configuration-guide.md` (regenerated by `fno config schema --markdown`).
