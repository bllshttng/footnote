...
- Recording law from chat works. Retracting it does not. `retract_decision` requires `authority_source` to be exactly `operator` on a law-lane row, so a chat-enacted law needs an attended terminal to withdraw. Supersession is not affected and accepts `chat_attested`.
