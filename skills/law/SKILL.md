...
2. Classify the statement. A rule limited to this PR, node, target, change, or merge is coordination. Refuse to label it law and offer `/fno:mail` or an ordinary agent ruling. A rule every clone needs is design law. There is no checked-in file to write it to. The repository catalog was deleted on 2026-08-29. A decision is the same class of object as a node, and the PR is the durable record. Only the operator writes law, through this verb.
