...
0. **Authoring gate:** before writing it, answer the four questions in [test-audit](../test-audit/SKILL.md) - the behavior it protects, the regression that fails it, why existing coverage misses that failure, and no test-only production seam. A test matching a junk pattern there does not get written; the gate is cheaper than a bad test.
