...
Emit `<promise>MISSION COMPLETE: ...</promise>` when the PR is up and CI is green - **promise early**, the external reads hold it. An unsatisfied read just blocks-and-retries naming what is missing; a premature promise never short-circuits the gate. **While waiting on an async check (CI/review) with nothing to do, stay silent:** set ONE background watcher for the blocking condition and emit NO user-facing prose on the stop-hook nudges until it changes - re-reading the poller and re-posting the same status every fire is pure noise (the loop-check wait-state nudge says as much). Narrate only when the state actually changes or there is an action to take. You are a bg/unattended agent, so your terminal state is a **green, reviewed, mergeable PR**, not a merged one: hand the merge to a human (any out-of-band merge also satisfies `done()`). The gate reads `config.review.required_bots`; the loop-check code default is empty `[]` (no review gate, so a fresh install never hangs on an unconfigured bot), and a maintainer sets it explicitly (e.g. `["chatgpt-codex-connector"]`) to require an external pass. Internal `/review` is advisory.
