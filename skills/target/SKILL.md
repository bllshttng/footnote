...
- **The ship step requests the fno review lane on the final HEAD by default - BEFORE the PR exists.** After validate, run `fno do target request-self-review` with no `--pr`: it pins the local branch and HEAD against the origin base, refuses a detached HEAD, and sends one payload (`/fno:review`, or `$fno:review` on Codex) naming `<branch> HEAD <sha> against origin/<base>`, with no `--comment`. On the prompt-line lane, review runs as the session's next turn, so this request must be the last tool action of the turn. One clean round is enough; findings cost a fix, commit, and re-request on the new HEAD. The post-push form is `fno do target request-self-review --pr <n>`: it resolves the PR head and base and refuses a mismatch. Do not downgrade `refused` or `unconfirmed`. A Codex mux request that reports `queued` is a positive turn-boundary receipt and the last tool action of the turn. For `started`, the inline lane surfaces its own findings and its emit step records the head-pinned attestation; handle P1/P2 findings, then commit and request a review on the new HEAD. A rebase also stales a pre-push attestation; rerun on the new HEAD. Do not route the act path through a king, daemon notification, or external reader. Do not emit `<promise>` until the clean head-pinned `code-review` attestation exists. The decision is `preship_review_plan(config.review.reviewers)`, and the lane runs inline on every harness. `sigma` is retired: a config naming it is refused at init with the lane named as replacement, and no post-ship panel run exists ([references/ship-and-promise.md](references/ship-and-promise.md)).
...
**Drain reviews BEFORE you promise - even when `required_bots` is empty.** CI green does not clear posted findings. Read every posted review's inline comments at first-post, and run any configured local review gate before `<promise>`. The inline fno review surfaces its findings and its emit step records the attestation. The full ordering, cursor-file watch protocol, three review-gate flavors, and head-pinning are in [references/ship-and-promise.md](references/ship-and-promise.md) - load it when the PR is up.
