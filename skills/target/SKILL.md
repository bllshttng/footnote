...
**Preflight is opt-in, CI is the gate.** A stock config never runs `scripts/ci/preflight.sh` before a push. Every ship path asks `fno pr evidence-required`, which reads `config.preflight.required` (default false) plus the `FNO_SKIP_PREFLIGHT=1` escape. The Python lanes and the bash ship path cannot disagree. The pre-push obligation is the focused checks: `cargo fmt --check` for rust changes and the tests covering the diff's blast radius. `fno lint style` remains available as a hand-run CLI verb, including `fno lint style --surface markdown --files <changed .md> --diff-base origin/main`, but it is not a target or CI gate. A project that wants the full 22-to-47-minute local rehearsal back sets `preflight.required = true`. Full contract: [references/ship-phase.md](references/ship-phase.md) and the repo-root `docs/preflight.md`.
