...
- **The ship step requests the fno review lane on the final HEAD by default - BEFORE the PR exists.** After validate, run `fno do target request-self-review` with no `--pr`. It pins the local branch and HEAD against the origin base and refuses a detached HEAD. It sends one leading-slash payload naming `<branch> HEAD <sha> against origin/<base>` through `--to-self --raw`, with no `--comment`. The post-push form is `fno do target request-self-review --pr <n>`, for a round requested after the PR exists. Never downgrade `refused` or `unconfirmed` to advisory prose. A Codex mux request that reports `queued` is the last tool action of this turn. The review starts at the next turn boundary. For `started`, the Codex Stop hook nudges the worker to act on P1/P2 findings in its own context. Do not route this act path through a king, a daemon notification, or an external reader. The round budget is `review.max_rounds` (default 2) across the PR's life, and it bounds every re-request. A rebase with an unchanged code delta carries the attestation and owes no round. When `fno do pr status <n>` reads `rounds_exhausted: true`, the review phase is complete: request no review, emit no attestation, merge on green CI. The promise waits for the stop gate to read review coverage covered. The decision is `preship_review_plan(config.review.reviewers)` in `cli/src/fno/review_capability.py`, and the named reviewer is the lane, which runs inline on every harness. `sigma` is retired, and a config naming it is refused at init ([references/ship-and-promise.md](references/ship-and-promise.md)).
