...
2. **Waiting on an async check with nothing to do** (CI pending, a bot review not yet posted, or a local run you started: a test suite, a build, a review fork) -> arm ONE **harness-tracked** watcher whose command carries a hard timeout, then end with the tag and NOTHING else. Both waits are ONE plain command polling REST at a 60s interval; NEVER `gh pr checks --watch` or a `gh pr view` loop - those spend the per-USER GraphQL quota every session on the machine shares, and a fleet of GraphQL watchers is what starves the merge guard. The plain-command shape is also the only one a worktree session's Bash isolation always admits: an inline `while`/`$(...)` watcher loop is refused there as too complex to verify, which wedges the very turn the watcher was meant to unblock:
   ...
   - local run: the run itself is the watcher. Start it as a background Bash task; the harness wakes you when it exits.
   - then: `<watching reason="ci|review|merge_slot" pr="<N>" timeout="30m">` (merge_slot: the PR is green, shipped, fully reviewed, and its sole blocker is another PR holding the merge slot; the watcher is `--until review --timeout 30m`, whose 30m bound is the heartbeat while the slot frees outside this PR), or `<watching reason="local" timeout="30m">` for a local run. `pr` is a real PR number or left out. Never write `pr="0"` or `pr="pending"`: a king reads that as a watch on a PR that does not exist.
   loop-check renews your node claim lease over the window and idles the session to ZERO re-invocations until the watcher fires. That idle reads no PR state, so the tag must name what you actually wait on. The watcher MUST be harness-tracked (background Bash / Monitor) - a detached process (`nohup`, `disown`, or a trailing `&` on the task itself) exits without waking anyone and the session idles forever. On wake: if it settled, proceed; if the bound fired and it is still pending, re-arm and re-emit.
