...
- The ship step requests the fno review lane on the final HEAD before the PR exists. Run `fno do target request-self-review` without `--pr` before `/fno:pr create`. It pins the branch and HEAD against the origin base. It refuses detached HEADs. It sends one payload with `/fno:review` or `$fno:review` on Codex, naming the branch, HEAD, and origin base. It omits `--comment` because no PR exists. On the prompt-line lane, review starts as the session's next turn. Make the request the turn's last tool action. One clean round is enough. For `started`, the inline lane surfaces findings and emits the head-pinned attestation. Fix P1/P2 findings, commit, and request review on the new HEAD. The post-push form is `fno do target request-self-review --pr <n>`. It resolves the PR head and base. It refuses a mismatch. Do not downgrade `refused` or `unconfirmed`. A Codex mux `queued` result confirms a turn boundary. It must be the last tool action. A rebase stales pre-push attestation. Rerun review on the new HEAD. Do not route through a king, daemon notification, or external reader. Wait for the clean `code-review` attestation before `<promise>`. `preship_review_plan(config.review.reviewers)` chooses the fno lane on every harness. A config naming retired `sigma` is refused at init. No post-ship panel exists ([references/ship-and-promise.md](references/ship-and-promise.md)).
