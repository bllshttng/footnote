...
**If the invocation carried a `no-merge` token, pass `--no-merge` to `fno target start` (or to `fno target init`).** `start` resolves its argument to a bare node id before calling init, so the token cannot reach the manifest through `--input` on that path; the flag is the deterministic carrier. Init also honors a whole-token `no-merge` left in `--input`, but that only helps when the original argument is forwarded verbatim, so prefer the flag. There is deliberately no `--auto-merge` twin: granting stays on config / `TARGET_AUTO_MERGE`.
...
Read that RESOLVED field, never `fno config get auto_merge` directly. Init folds the config together with this run's modifiers, and a per-run `no-merge` - which `/target bg` injects **by default** - sets it false even when `auto_merge.enabled` is true, so the raw config would tell you to merge against an explicit per-run prohibition. The fold is deliberately one-directional and every refusal outranks every grant: a whole-token `no-merge` in the invocation revokes merge authority even over an inherited `TARGET_AUTO_MERGE=1`, while the `auto-merge` token grants nothing on its own (it needs `auto_merge.enabled` or `TARGET_AUTO_MERGE=1`), because honoring a grant found in free text would let arbitrary prose manufacture merge authority. `fno pr merge` reads the same field and refuses too, but that is a backstop: decide from the manifest rather than firing the verb and hoping it catches you.
