...
The manifest is WRITE-ONCE: `fno do target init` writes it at session start; the ONLY legal post-init mutation is first-fill of an empty `plan_path` via `fno do state set --field plan_path`. Any other field write exits 5 (ab-d0337fbc).
