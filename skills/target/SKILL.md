...
- **Before the PR exists, run the fno review lane on the final HEAD, in this session.** After validate, run `/fno:review <size> --comment` (Codex: `$fno:review <size> --comment`) on the final local HEAD, before `/fno:pr create`. Size the review by the diff: `medium` under 300 changed lines, `high` above, `xhigh` when the diff touches a risky surface (merge, claims, graph writes, hooks, auth). No mail, no paste, no turn boundary to wait on: the review runs in this session's own context and emits the head-pinned attestation itself. With no PR yet, `--comment` holds the findings keyed to branch and HEAD; when the PR opens, the publish-review step posts them as one PR comment. Fix P1/P2 findings and review the new HEAD while `review.max_rounds` (default 2) permits. A rebase with an unchanged code delta carries the attestation. When `fno do pr status <n>` reads `rounds_exhausted: true`, the review phase is complete: request no review or attestation, and merge on green CI. The promise waits for the stop gate to read review coverage `covered` ([references/ship-and-promise.md](references/ship-and-promise.md)).
