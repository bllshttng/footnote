...
- **Before the PR exists, request the fno review lane on the final HEAD.** After validate, run `fno do target request-self-review` with no `--pr`. It pins the local branch and HEAD against the origin base and refuses a detached HEAD. It sends a leading-slash payload naming `<branch> HEAD <sha> against origin/<base>` through `--to-self --raw`, without `--comment`; the payload runs `$fno:review` on Codex or `/fno:review` elsewhere, inline in the target worker session. The post-push form is `fno do target request-self-review --pr <n>` for a round requested after the PR exists. Never downgrade `refused` or `unconfirmed`. A Codex mux request with `queued` is the last tool action and review starts at the next turn boundary; `started` hands the session to the inline lane, which surfaces findings and emits the head-pinned attestation. Fix P1/P2 findings with a new-head request while `review.max_rounds` (default 2) permits. A rebase with unchanged code delta carries the attestation. When `fno do pr status <n>` reads `rounds_exhausted: true`, the review phase is complete: request no review or attestation, and merge on green CI. The promise waits for the stop gate to read review coverage `covered`. `preship_review_plan(config.review.reviewers)` selects the reviewer; the fno lane runs inline on every harness. `sigma` is retired, and a config naming it is refused at init ([references/ship-and-promise.md](references/ship-and-promise.md)).
...
**Drain reviews before promising.** CI green does not clear posted findings. Read every posted review's inline comments at first post. Run configured local review gates before `<promise>`. The inline fno review surfaces findings and emits its attestation. Load the full ordering, cursor watch protocol, review gate flavors, and head pinning from [references/ship-and-promise.md](references/ship-and-promise.md) after the PR opens.
