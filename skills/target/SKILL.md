...
Read that RESOLVED field, never `fno config get auto_merge` directly. Init folds the config together with this run's modifiers, and a per-run `no-merge` - which `/target bg` injects **by default** - sets it false even when `auto_merge.enabled` is true, so the raw config would tell you to merge against an explicit per-run prohibition. The fold is deliberately one-directional: `no-merge` anywhere in the invocation revokes merge authority, but the `auto-merge` token grants nothing on its own (it needs `auto_merge.enabled` or `TARGET_AUTO_MERGE=1`), because honoring a grant found in free text would let arbitrary prose manufacture merge authority. `fno pr merge` reads the same field and refuses too, but that is a backstop: decide from the manifest rather than firing the verb and hoping it catches you.
