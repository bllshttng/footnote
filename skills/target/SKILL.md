...
- **only if** you were handed a bare idea (no plan): run `/blueprint "<idea>"` directly, then implement. The operator's ruling (x-42c5): `/think` is a RESEARCH verb outside the delivery path, never an automatic prerequisite to `/blueprint`. `/blueprint` grounds itself on a bare idea via its own discovery gate ([references/discovery-gate.md](../blueprint/references/discovery-gate.md) in the blueprint skill) - it runs `fno think inspect` for the receipt and asks its own unknowns; it does not require or await a `/think` doc. Run `/think` first ONLY as a deliberate choice (yours or the operator's) when a question needs deeper investigation than the receipt gives - never reflexively.
...
The delivery pipeline runs in order (blueprint/plan -> do -> review -> validate -> docs -> ship -> external review). `/think` is a research verb outside this path (x-42c5): it never runs automatically as a pipeline phase, only as a deliberate choice before `/blueprint` when deeper investigation is warranted. The review phase is an advisory self-review by default; sigma only when configured (see above). Phases are not enforced by gates; completion proof is the world itself (PR green + reviewed). The acceptance-criteria check that runs before `/do waves` is documented in [references/phase-transition-guards.md](references/phase-transition-guards.md).
