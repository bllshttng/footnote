...
**Preflight is opt-in; CI is the gate.** A stock config never runs `scripts/ci/preflight.sh` before a push: every ship path asks `fno pr evidence-required`, which reads `config.preflight.required` (default false) plus the `FNO_SKIP_PREFLIGHT=1` escape, so the Python lanes and the bash ship path cannot disagree. The pre-push obligation is the focused checks, seconds each: `cargo fmt --check` when rust changed, `fno lint style --surface markdown --files <changed .md> --diff-base origin/main` when markdown changed, `bash scripts/ci/check-pr-body-length.sh` before opening the PR, and the tests covering the diff's blast radius. A project that wants the full 22-to-47-minute local rehearsal back sets `preflight.required = true`. Full contract: [references/ship-phase.md](references/ship-phase.md) and the repo-root `docs/preflight.md`.
