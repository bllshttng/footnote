...
- **Before the PR exists, request the fno review lane on the final HEAD.** After validate, run `fno do target request-self-review` with no `--pr`. It pins the local branch and HEAD against the origin base. It refuses a detached HEAD. It sends a leading-slash payload naming `<branch> HEAD <sha> against origin/<base>`. The payload goes through `--to-self --raw` without `--comment`. The post-push form is `fno do target request-self-review --pr <n>`. Use it for a round requested after the PR exists. Never downgrade `refused` or `unconfirmed` to advisory prose. A Codex mux request that reports `queued` is the last tool action of this turn. The review starts at the next turn boundary. For `started`, the Codex Stop hook nudges the worker to act on P1/P2 findings in its own context. Do not route this act path through a king, a daemon notification, or an external reader. The budget is `review.max_rounds` (default 2) across the PR's life. It bounds every re-request. A rebase with an unchanged code delta carries the attestation and owes no round. When `fno do pr status <n>` reads `rounds_exhausted: true`, the review phase is complete. Request no review or attestation. Merge on green CI. The promise waits for the stop gate to read review coverage covered. The reviewer decision is `preship_review_plan(config.review.reviewers)` in `cli/src/fno/review_capability.py`. The lane runs inline on every harness. `sigma` is retired. A config naming it is refused at init ([references/ship-and-promise.md](references/ship-and-promise.md)).
