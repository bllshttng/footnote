...
domains:
  {domain-name}:
    phases:
      ...
      review: {skill-name}      # default: native review; fno:review (sigma) only when sigma is a configured reviewer
...
| review | native review (sigma via `fno:review` only when `config.review.reviewers` names it) | Code quality + integration tests |
