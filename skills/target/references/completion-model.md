...
The machinery behind the one paragraph in `SKILL.md`'s "Completion: what you do". You rarely need this; read it only when debugging why a `<promise>` did or did not terminate the loop. The actionable contract (promise early, merge per `config.auto_merge`, `fno test`, `fno pr status`) lives in the skill body.
...
- **Merge authority is CONFIG-driven, not attendance-driven.** Read `fno config get auto_merge` before promising. With `auto_merge.enabled: true` and `require_checks_pass` satisfied (CI green, reviews drained), MERGE: `fno pr merge <n>`, then `fno backlog reconcile` to close the node (a merge from inside a worktree skips the local post-merge step). Config set once IS the standing authorization; re-asking each time re-imposes the step it was configured to delete. With auto-merge off (the default), the terminal state is a **green, reviewed, mergeable PR** and the merge goes to a human. Either way emit `<promise>MISSION COMPLETE: ...</promise>` once the PR is green and reviewed. **Any merge path counts as done:** a PR merged out-of-band (web/mobile UI, or `gh pr merge`) satisfies `done()` as `DonePRGreen` on the next stop-hook fire - the loop stops re-poking a finished session.
  - There is no attendance check anywhere in the merge path. The two-factor guard in `hooks/git-protection.py` (`_check_pr_merge_allowed`) gates on `auto_merge_approved` plus external-review evidence, both config- and session-driven. This bullet previously claimed the guard "refuses an unattended actor", which sent every bg session hunting for a human approver that nothing required.
