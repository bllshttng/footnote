...
## Review Phase (4) - Native final-head review

The ship review is a native request BEFORE `/fno:pr create`, on the final local HEAD. Run `fno do target request-self-review` with no `--pr`. It pins the local branch and HEAD against the origin base and refuses a detached HEAD. It sends `<branch> HEAD <sha> against origin/<base>` through `--to-self --raw`, without `--comment`. The `--pr <n>` form is post-push. It resolves the PR head and base, refuses a mismatch, and sends `HEAD <sha> of PR <n> against origin/<base>`. The promise waits for the stop gate to read review coverage covered: a clean round that tiles the head, or the budget spent. The decision is `preship_review_plan(config.review.reviewers)` in `cli/src/fno/review_capability.py`:

- **sigma NOT configured (the default):** run the request on the final local HEAD before any push. The reviewer is the fno review lane. It runs inline wherever the plugin runs. A `started` result waits for the completion marker. Blocking findings enter the fix, commit, and re-request loop inside the round budget. A clean round or a spent budget can precede the push. A `refused` or `unconfirmed` result stops with its lane error. Never downgrade either to advisory prose. A busy Codex mux pane can return `queued`. That result is the last tool action this turn. The next turn boundary starts the review without a human interrupt. Do not poll, inspect the pane, read mail, retry, or promise. A docs-only payload or explicit `self_review_required = false` opt-out keeps its exemption.
...
**The request is substrate-aware and fail-closed.** The raw self-send router selects the live harness lane. A positive `queued` result is a turn-boundary handoff, not permission to continue the same turn; any unreadable frame, missing target marker, mismatched queued payload, or failed lane returns `unconfirmed` or `refused`, never a clean review.
...
When a lens row's condition holds for the diff under review, read that lens file and apply its question to the built code. These are the code lenses blueprint retired from drafting under lean dispatch. The review phase carries those checks now. The index: [review-lenses.md](review-lenses.md).
