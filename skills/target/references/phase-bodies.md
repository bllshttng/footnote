...
The ship review runs the fno review lane in this session, BEFORE `/fno:pr create`, on the final local HEAD. Run `/fno:review <size> --comment` (Codex: `$fno:review <size> --comment`). Size by the diff: `medium` under 300 changed lines, `high` above. Use `xhigh` for a risky surface (merge, claims, graph writes, hooks, auth). No mail, no paste, no turn boundary. The review runs as ordinary tool calls and emits the head-pinned attestation itself. With no PR yet, `--comment` holds the findings on branch and HEAD. Once the PR opens, the create flow's publish-review step posts them as one PR comment. The promise waits for the stop gate to read review coverage covered: a clean round that tiles the head, or the budget spent.
