...
The ship review is a native review request after `/fno:pr create` returns the PR number. Run `fno do target request-self-review --pr <n>`; it resolves the PR head and base, refuses a mismatch, and sends one explicit payload naming `HEAD <sha> of PR <n> against origin/<base>` through `--to-self --raw`. The request is the producer, not advisory prose, and the promise waits for its clean head-pinned `code-review` attestation. The decision is `preship_review_plan(config.review.reviewers)` in `cli/src/fno/review_capability.py`:

- **sigma NOT configured (the default):** run the native request on the final pushed HEAD. A `started` result waits for the native completion marker; blocking findings enter the existing fix, commit, push, and re-request loop. A `refused` or `unconfirmed` result stops with its lane error and never downgrades to an advisory read. A busy Codex mux pane may positively return `queued`; that request is the last tool action of the turn, with no polling, pane inspection, mail read, retry, or promise, so the next turn boundary starts the review without a human interrupt. A docs-only payload or explicit `self_review_required = false` opt-out keeps its existing exemption. A harness whose verb no lane can fire (opencode today: the daemon lane submits text and its mux row declares submit unsupported) refuses by design; that session asks the operator for review or spawns a claude/codex reviewer instead of stalling on the default.
