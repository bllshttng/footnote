...
The ship review is a native review request BEFORE `/fno:pr create`, on the final local HEAD. Run `fno do target request-self-review` with no `--pr`; it pins the local branch and HEAD against the origin base, refuses a detached HEAD, and sends one explicit payload naming `<branch> HEAD <sha> against origin/<base>` through `--to-self --raw`, with no `--comment`. The `--pr <n>` form is the post-push form: it resolves the PR head and base, refuses a mismatch, and sends one payload naming `HEAD <sha> of PR <n> against origin/<base>`. The request is the producer, not advisory prose, and the promise waits for its clean head-pinned `code-review` attestation. The decision is `preship_review_plan(config.review.reviewers)` in `cli/src/fno/review_capability.py`:

- **sigma NOT configured (the default):** run the request on the final local HEAD before any push. The reviewer it names is the fno review lane, which runs inline wherever the plugin runs; a `started` result waits for the completion marker; blocking findings enter the existing fix, commit, and re-request loop (a re-request runs on the new HEAD; the push happens after the clean round). A `refused` or `unconfirmed` result stops with its lane error and never downgrades to an advisory read. A busy Codex mux pane may positively return `queued`; that request is the last tool action of the turn, with no polling, pane inspection, mail read, retry, or promise, so the next turn boundary starts the review without a human interrupt. A docs-only payload or explicit `self_review_required = false` opt-out keeps its existing exemption.
