...
Pass `--beastmode` (or `TARGET_BEASTMODE=1`) when the invocation carried the `beastmode` modifier, so init stamps `authority: full`. Without it the field is absent and the session keeps the default stop-and-ask posture. See [SKILL.md §Authority](../SKILL.md#authority-the-beastmode-grant).
