...
- Stock config: no local preflight runs before a push. The focused checks replace it: fmt, markdown style lint, PR-body length, the blast-radius tests. CI is the gate. Six concurrent full preflights at load 415 with zero completions set this default.
