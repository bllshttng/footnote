...
- Under `--all-ready` only `ready` nodes dispatch. An **explicitly-named** node also dispatches when its status is `idea` (the triage pile; naming it is the human's vet, the dispatched worker runs `/target`'s own bare-idea branch - `/blueprint` directly, `/think` NOT automatic - x-42c5); `blocked`/`deferred` are always **parked** (pre-planned future work), never launched. A node a live worker already holds (`node:<id>` claim) is **already-running**, never double-dispatched.
