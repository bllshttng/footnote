...
The loop-check verb emits a `termination` event to `.fno/events.jsonl` with a `TerminationReason` when it stops the session. Use `fno whoami status` to read the latest termination reason:
