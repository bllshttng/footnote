...
- **/blueprint**: run it whenever you started from a bare idea, or the bound plan is still design-stage. It self-grounds on a bare idea via its own discovery gate (`fno think inspect` + up to 3-5 targeted questions) and never awaits a `/think` doc (x-42c5); a prior `/think` doc, if one exists, is consumed as already-cited findings, not required. A blueprint-complete plan skips straight to implement.
