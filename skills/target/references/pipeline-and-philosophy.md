...
- **review**: before the PR exists, run `fno do target request-self-review` (no `--pr`) on the final local HEAD. The `--pr <n>` form is post-push, for a round requested after the PR exists. Require a positive result. The promise waits for the stop gate to read review coverage covered. Coverage means a clean round that tiles the head, or the budget spent. A busy Codex mux `queued` result ends the turn at the boundary. Docs-only payloads and the explicit opt-out keep their exemptions.
