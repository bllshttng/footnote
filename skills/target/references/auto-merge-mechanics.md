...
`init-target-state.sh` implements this chain and is the only place it lives. The resolved value is recorded in `target-state.md` as `auto_merge_approved`, and `fno do pr merge` and `fno-agents finalize` resolve it identically: a refusal (`false`) outranks every grant, while a grant additionally needs the live `auto_merge.enabled` switch OR its own `auto_merge_source: env-target-auto-merge` stamp (the spawn-time `TARGET_AUTO_MERGE=1` grant), so an operator disarm mid-flight still withholds. The git-protection hook guards raw `gh pr merge` on the same field. `TARGET_AUTO_MERGE` is read at init only; exporting it on a merge command line grants nothing.
