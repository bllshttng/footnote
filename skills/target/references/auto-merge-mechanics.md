...
`init-target-state.sh` implements this chain and is the only place it lives. The resolved value is recorded in `target-state.md` as `auto_merge_approved`. `fno do pr merge` and `fno-agents finalize` resolve it identically. A refusal (`false`) outranks every grant. A grant also needs the live `auto_merge.enabled` switch. Or it needs its own `auto_merge_source: env-target-auto-merge` stamp. That stamp is the spawn-time `TARGET_AUTO_MERGE=1` grant. An operator disarm mid-flight therefore still withholds. The git-protection hook guards raw `gh pr merge` on the same field. `TARGET_AUTO_MERGE` is read at init only. Exporting it on a merge command line grants nothing.
