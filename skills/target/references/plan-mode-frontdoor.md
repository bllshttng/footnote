...
- `pending` -> run the **backfill adapter**, then confirm:
  ...
  3. **Synthesize (LLM step, the one new piece of reasoning):** if the section is reported absent, append it to the enriched doc from the native plan's intent - a `## Acceptance Criteria` section with all 5 BDD types (`#### AC1-HP:` / `AC1-ERR:` / `AC1-UI:` / `AC1-EDGE:` / `AC1-FR:`). A section reported present is REUSED, never duplicated (AC2-EDGE). Preserve the native body verbatim - ADD sections only (AC2-FR).
