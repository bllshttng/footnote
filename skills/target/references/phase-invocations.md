...
`validate` is a bash command (from `domain_phases.validate`), not a skill invocation. Validate runs the tests that directly cover the files this run changed (`fno doctor test <changed test files>`; for rust the narrow `--lib <module>::` form), never the whole suite: CI runs every suite on every PR, and a missed downstream break is CI's job. When a whole-suite local run is genuinely needed, start it as a background task: it queues on the machine-wide `test:suite` slot and the turn never blocks. Run validate, and on a non-zero exit loop into the validation-failure-recovery flow ([failure-recovery.md](failure-recovery.md)) to either rollback to the pre-execute checkpoint or fix forward. The local run is the work - it is how failures get caught and fixed BEFORE pushing.
