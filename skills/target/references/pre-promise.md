...
It prints nothing when every required bot's provider has headroom (or none are configured), and emits one `quota_required_bot_exhausted` decision event per exhausted bot. If it prints a warning, in attended mode surface the same facts so the user can act (swap accounts / wait for the reset) rather than discovering the wedge later:
