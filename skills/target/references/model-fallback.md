...
A phase transition never changes the model on its own. The model in effect stays whatever the user set interactively, or whatever the resolved lane set unattended (via `fno agents spawn`'s grid), until one of those two actors changes it again.
