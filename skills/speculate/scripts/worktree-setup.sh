...
if [[ -n "$_gate_repo" ]] && command -v fno >/dev/null 2>&1; then
    ...
    # Two-step deploy window: an installed fno older than the fold lacks the
    # canonical spelling, so fall back to the retired root one.
...
# Session-keyed create-attempt cap (kept in sync with hooks/worktree-setup.sh):
# repeated failing worktree ceremony in one session aborts at the cap, the
# supported way - exit 0 with EMPTY stdout. A manual /speculate call carries
# no session_id in its payload, so it is never counted.
_CAP_SESSION=""
if command -v jq >/dev/null 2>&1; then
...
# The successful create ends this ceremony episode: the latch's writer owns
# its lifetime (no-op for manual calls, which never wrote one).
