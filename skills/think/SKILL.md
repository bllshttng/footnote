...
**Handoff, not runtime invocation (router contract override).** panel.md's report step offers downstream verbs - both the default handoff options and the optional `--chain think|plan`. In router mode those are **presented as copy-paste handoff lines** (`/think ...`, `/blueprint ...`, `/fno:target ...`) for the user to run next; the panel mode never invokes another skill at runtime. Routers load modes inline and dispatch only via Task/Agent, and a self-contained install may ship the `think` folder without the chained skill present, so `/think panel --chain plan ...` resolves the recommendation and then emits the `/blueprint ...` line rather than calling it.
