...
4. When it returns, read back two positive markers. `fno backlog get <node>` holds a `blueprint` session row with this session's id and an `ended_at`. `fno agents claim status node:<node> --json` no longer names the holder step 1's receipt printed.
5. When either marker is missing, the subagent did not finish. Run `fno agents claim release node:<node> --holder <the holder step 1's receipt printed>` (never with `--stamp-do`) and report the failure. Never backfill the row with `fno backlog session add`. A row records a completion, and nothing completed.
...
2a-bis. **The five questions** - answer before designing. Write them in a `## Five questions` section. Each answer names a thing or says `none`.
  - Who hits this? Name a person and cite what it costs them today.
  - Which existing verb, skill, or config does it extend?
...
See [references/product-lenses.md](references/product-lenses.md) for the drafting lenses. Do not read the grader's own files while drafting.

An answer that changes the deliverable changes the plan. That is why you ask before designing. Step 3's judge grades the answers.

Obey rulings in the stage block (`## Law governing blueprint`) and rulings named in `decisions_acknowledged`. Do not re-derive or re-ask them.

If the user or king gives a directive without a ruling, challenge it once. Name the alternative and ask once with each side's cost. Then follow the answer. A second challenge on the same subject in this session violates this rule.

If the blueprint concludes that the node needs superseding or narrowing, write a question row with `fno inbox outstanding ask "<question>" --node <id>`. This lists it in `fno inbox outstanding`.

If the question has options, use `--question-file` with the template in `docs/architecture/attention-items.md`. Without its context fields, the question is not ready and is never delivered. Close the session with `--summary`. Never record the decision yourself.
