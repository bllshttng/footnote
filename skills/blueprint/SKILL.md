...
1. Run `fno backlog session open <node> --json`. A nonzero exit is the answer. Relay the refusal line and launch nothing. Exit 0 with `status: joined` means the spawn claimed the node for this session, so plan under it, and the receipt's `holder` is the holder steps 4 and 5 read back and release.
