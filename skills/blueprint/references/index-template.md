...
# node per project (linked by blocked_by) via `fno backlog decompose`, giving each
# group a `project` (cwd resolved from the work-map) or explicit `cwd` to route it
# into its own repo. Each node ships its own PR and spawn-into-project carries the
# cross-repo handoff.
