...
**Take the node claim.** Right after `CLAIMS_ID` resolves, read the claim. A
live or suspect claim means a caller already covers this run (the subagent
wrapper, a spawn-handover worker, a crown): print the holder and open
nothing. Otherwise open the claim and keep the holder, so an early halt can
release exactly that holder.
...
**Halt release.** Every halt after this block and before `session close`
runs `fno agents claim release "node:$CLAIMS_ID" --holder "$OPENED_HOLDER"`
when `OPENED_HOLDER` is set. Never pass `--stamp-do`. The halts in scope are
the Consolidation Gate halt, the validate-and-finalize failure, and the
post-write claims refusal.
