...
Besides the five questions, four source readers each grade against one outside source: epic_fit against the parent epic and its siblings, mission_fit against the nearest vision_path ancestors and `config.project.vision`, customer_fit against `PRODUCT.md`, code_truth against the plan's own line citations resolved on disk. A missing source, or a missing lens file, is a coverage gap and not a fail. A source reader's fail quotes its source; a fail that quotes nothing real stays a gap.
