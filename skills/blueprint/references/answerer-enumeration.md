...
- Walk every language tree the symbol lives in, at the tree root, never a narrowed subpath. During a port the twin in the other tree often keeps the same name. The validator greps each changed answerer's symbol - the identifier after its path in `at:`, and each free `name(` call in `reads:` - at the merge base and refuses a reader in another language that no answerer names.
...
Two more, measured 2026-09-13 and 2026-09-14, and in both the enumeration that was supposed to catch them had already run and passed. In the first, the plan's sweep named a narrowed Rust pathspec, two files, and its own pattern returned a seventeenth file over that pathspec and a twentieth over the whole crates tree: the store's work-state reader, which a review round then found instead. The changed answerer's `reads:` quoted the open-row predicate, and that name exists in both trees, which is how the cross-language walk now names the store and the keeper. In the second, a plan porting a provider counter from Python to Rust found five Python readers beyond the gate by hand: the capacity probe, an explain gate, the top lane's rows, route resolution and advance width. A Rust-only count would have opened a new dual implementation, and nothing sweeping one tree would have refused it.
