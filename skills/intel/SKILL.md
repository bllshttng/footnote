...
- Operator is witnessed, not residual: the mux writes an `operator_submit` row when a person presses Enter in a pane or portal, and the fold binds turns to those rows. Turns outside the witness read `unknown`, never `operator`. Uncovered paths: typing in a bare terminal, the desktop apps, or claude.ai jobs (never passes the mux); a hand-started harness in a shell pane (its submit is `resolution: unresolved`, nothing joins it); a submit queued past the 30s bind window. The report's `witness.unwitnessed_sessions` counts sessions with unknown turns and no submit row at all.
