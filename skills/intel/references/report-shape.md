...
The scrub runs before anything renders. Secret shapes become `[redacted]`: the known token prefixes (sk-, gh_, github_pat_, AKIA, xox), PEM key headers, JWTs, bearer values, and any unbroken 40-character base64-shaped run. Key/value pairs (`password=`, `secret=`, `token=`, `api key=`) keep the key name and redact the value. Home paths (`/Users/<name>`, `/home/<name>`) become `~`. Fenced code blocks and `>` blockquotes drop whole as `[omitted: quoted block]`. Double-quoted spans outside `Operator corrections` become `[quote omitted]`. Inside `Operator corrections`, those quotes remain as user corrections.
