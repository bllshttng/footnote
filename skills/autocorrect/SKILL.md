...
Capture surfaces (write to `~/.claude/corrections.log`, never invoked by the agent):
...
2. **Single artifact.** Everything funnels through `~/.claude/corrections.log`. Three writers, one consumer. Resist the urge to add ad-hoc per-source logs.
...
| File | Purpose |
|---|---|
| `~/.claude/corrections.log` | The canonical capture artifact. Mode 0600. |
...
| `~/.claude/corrections-rejected.log` | Items the user rejected during triage. |
