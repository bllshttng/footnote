...
1. Run the smallest owner and sibling tests with the repo's documented runner (on this plugin's own repo: `fno doctor test [paths...]` for Python, which pins `PYTHONPATH` and returns the real exit code, and `cargo test -p <crate> <filter>` for Rust; never a bare `pytest`, which can import the wrong tree).
...
4. Run the changed-surface gate the repository policy requires; on this repo that is `bash scripts/ci/check-file-budget.sh` after any commit that grows code, plus `fno doctor lint style --surface markdown --files <changed .md> --diff-base origin/main` for markdown.
...
Commit, push, or open a PR only when the run's authority allows it. Land one coherent PR at a time; after landing, merge current `main` into the branch (never rebase a pushed branch) and rerun read-only discovery for the next high-confidence batch.
