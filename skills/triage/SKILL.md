...
2. **LLM reasoning.** Spawn a subagent (Task tool) with the context JSON
   ...
   **Tournament ordering (large backlogs).** When `count` (from `triage
   ...
   Fold that order into the proposal as `priority_changes` entries, banding the best-first order across `p1..p3`, so it reaches the graph only through step 5's approved `apply`. p0 is not yours to hand out. It needs `--blocks-everything` and means the fleet is down. Never through `fno backlog rank`, which is the operator's pin and refuses an agent session, and never through `fno backlog update` here: a rank applied before the approval boundary is the arrival-order failure a careful comparative judgement once serialised through min-minus-one writes. Feed the ranking into the `priority_changes` rationale too.
