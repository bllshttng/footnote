...
**A durable receipt is not delivery.** `fno mail send` injects into the recipient's live session and only falls back to the durable bus on a miss, so read the receipt: `delivered (hosted)` landed and the peer acts on it this turn; `queued (durable)` did NOT land and waits on a drain that may never run. Do not wait on a durable send — the handle you mailed is the same id `fno agents peek <handle>` (is it alive?), `resume <handle>` (idle → live, then re-send), and `attach <handle>` (drive it yourself) take. Getting an answer now beats queueing one for later.
