...
**Mail is user-shaped.** Fallback when a worker's own invocation is refused: `fno agents mail send <worker> --raw '/<verb>'`. A mail probe proves user-triggered behavior, never autonomy; the valid test is a transcript scan with no user-shaped prompt before the call: `scripts/diagnostics/autonomy-probe-audit.py`. No live king means [advisory self-review](docs/architecture/review-lanes.md).
