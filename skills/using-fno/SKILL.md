...
**Pane drives carry an envelope; `typed` is not `delivered`.** `fno mux pane send` wraps in `<fno_mail>` by default and refuses a pane showing an option prompt (a submit there picks its default). `--raw` types bytes verbatim: a digit, a control key, a shell command. On a `live-miss` you read as busy rather than dead, `fno mail send --force` types the wrapped body into the pane, keeping the msg-id, reply handle and outbox row a verb switch used to lose. Its receipt says `typed (pane <id>)` and claims no more: bytes into a PTY is not delivery. [Details](docs/architecture/pane-transport.md).

**Codex: full session_id or pane; never head-8.** A codex id is UUIDv7, so its first eight hex are a ~65.536-second bucket, not random - siblings from one minute collide and `mail reply` refuses. `fno mail send` refuses a codex head-8 outright. Claude ids are UUIDv4, so either form works. On an old ambiguous message, `mail reply --sender-session <full-id>` answers one candidate and keeps the thread.
