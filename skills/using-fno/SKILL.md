...
**Read send evidence literally.** `delivered (hosted)` is confirmed. `queued (durable)` can sit undrained - no receipt is no coordination. A send KILLED mid-flight (timeout, SIGKILL) now leaves the durable row too: the bus row is written before the recipient is resolved, so a dead process no longer takes the message with it; on a later confirmed live delivery the row is marked `delivered_at` and never drains twice. Before re-sending, `peek` (busy can still receive), then `resume`/`attach`. A `[DND (bus-only)]` queue drains. The recipient's turn-boundary `notify-self` surfaces it. A bus-only receipt IS coordination, never a stranded message.
