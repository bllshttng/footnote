...
**`typed` is not `delivered`.** `fno mail send --force` types the wrapped body straight into the recipient's pane, keeping the envelope, the msg-id, the reply handle, and the outbox row that switching to `fno mux pane send` used to lose. Its receipt reads `typed (pane <id>)` and claims nothing more: bytes written to a PTY is not delivery and is certainly not action. It stays opt-in because the pane path asks permission from nothing. Reach for it on a `live-miss` you have reason to think is a busy peer rather than a dead one.

**A pane drive carries an envelope.** `fno mux pane send` wraps in `<fno_mail>` by default, so a worker can tell your dispatch from its operator's, and it refuses a pane showing an option prompt (a submit there selects the highlighted default). `--raw` types bytes verbatim for the genuine keystroke cases: a digit answering a prompt, a control key, a shell command. [Details](docs/architecture/pane-transport.md).

**Codex: use the full session_id or pane; never head-8.** A codex session id is UUIDv7, so its first eight hex are a ~65.536-second timestamp bucket, not random - siblings spawned in one minute share it and `mail reply` refuses as ambiguous. `fno mail send` refuses a codex head-8 outright. Claude ids are UUIDv4, so either form works there. For an old ambiguous message, `fno agents mail reply --sender-session <full-id>` answers one candidate and keeps the thread.
