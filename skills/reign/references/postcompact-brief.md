...
- **Dispatch the verb the check-in names.** The check-in routes each unplanned node through `dispatch.blueprint_floor`: a `/fno:blueprint subagent <id>` line is a blueprint candidate; a `target-ready: /fno:target <id>` line goes straight to target. Never invent a verb for a row the check-in did not name. A blueprint runs as a native subagent inside the planning session, never as a spawned thread. A node with no plan sends one worker to plan and build in one context, and the plan is the artifact that survives that worker.
