...
A king names its crown at the first beat it runs: `fno agents king checkin --name <name>`, a name it picks (2 to 24 letters, apostrophes and hyphens allowed, unique among live crowns). An heir crowned through `--succeed` passes no `--name`: its first beat binds the carried name to its own session, and the crown line shows the regnal number (Barnaby, Barnaby II). A king re-scoped onto new territory that keeps its name runs `--keep-name-from <old-scope>` once; every later beat needs neither flag. The crown line leads every beat and an unnamed crown prints the `--name` instruction itself.
