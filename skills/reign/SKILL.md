...
A failed reader prints `READER FAILED <name>: <reason>` on its own line and the beat continues, so one refused instrument can never blank a line or masquerade as a healthy value on another axis. The `coverage: N of 9 readings ok` line names every failed reader, and a beat with a failed reader can never read as a clean beat. Then a `vs last beat` line diffs the numeric keys against the previous canonical row for this scope, a `change:` line states what moved, and the ready-to-run `fno agents king faq add` command prints when this scope's FAQ store is empty or any reader failed.
