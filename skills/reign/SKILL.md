...
1. **Fleet settled-PR wake, 600s.** The stop hook only fires when an agent stops, so a session that stopped while its PR was pending and whose CI went settled an hour later has nothing watching for it. This one arm is the fleet's query for exactly that. The until-loop exits when a roster row that has gone quiet, parked or unknown carries a node whose `pr_number` reads settled. The join takes two reads. `fno agents list --json` rows carry the status word (`quiet`, `parked`, `unknown` - probe failure, not health). `fno agents court --nodes` returns one row for every live crown on the machine, never only yours: five on 2026-09-21. Take the one row whose `manifest_session` equals the `harness_session_id` that `fno whoami --json` prints. From its `scope_nodes.nodes`, keep only the rows marked `owned: true`. The fold gives each node one owner, the deepest crown that lists it, so an L1 king never pokes a session its L2 king also pokes. Use this key and no other: on 2026-09-20 two kings each wrote their own filter and picked two different keys. No row matching your session is a probe failure, never a quiet board. Roster rows rarely carry `node` on their own (measured 2026-09-19: 1 of 23). A monitor joined on that field alone never fires. Read the PR with `fno do pr status <n>`, one `green|red|pending|unknown` verdict where `green` is settled; never hand-rolled jq. Green alone is not settled work, and three skips gate the exit before any poke: the `fno do pr status` payload the loop already fetched carries `pr_state`, so a row whose `pr_state` reads `MERGED` or `CLOSED` is skipped, because a merged PR still reports `verdict: green` and `settled: true`; a row that passes that skip reads its node with `fno backlog get <node>` and is skipped when the node `status` reads `done` or `superseded`. The third skip reads `claude agents --json` and skips a claude session whose `status` reads `busy`, because a busy session is mid-turn, not stranded. That read returning no rows at all is a probe failure. Measured 2026-09-19: the probe fired SETTLED on PR 2253 while its driver session read busy. Without them, every crown merge whose worker has parked wakes the crown to resume a finished worker: observed 2026-09-15, the wake fired `SETTLED pr=2037 node=` minutes after the crown merged 2037 and closed done at 13:39:35Z. On wake, poke the stopped session with `fno agents resume <id>`, which confirms the wake by content in the transcript; never a fresh dispatch. Measured by hand on 2026-09-10 over PRs 1650, 1694 and 1649: three pokes, three resumed sessions, zero slot cost. One arm per crown beats six. The daemon nudge ladder already pokes every quiet session on an open PR, and this arm is the crown's backstop while that ladder's resume rung rarely lands: it delivered 1 of 150 resumes from 2026-09-19 to 2026-09-21.
