...
- `blocked_child:` a child under this crown emitted `<help>` and nothing answered it inside the grace window - the node, the session, and the age.
 - `held:` lists nodes blocked by an open user question, oldest first. Each row carries the question id, its age, and the `fno backlog decide <node> "<ruling>" --question-id <id>` command that clears it. When no question holds a node, the line reads `held: none`.
...
Then the levers, in this order, stopping at the first that applies per row: mail the stalled worker; `fno backlog encounter <node> --evidence "what it cost"` to vote the node up, and `fno backlog update <node> --priority p1` when the evidence contradicts the priority it was filed at (p0 needs `--blocks-everything` and means the fleet is down), then, for a row no crown covers, start a new small epic rather than growing a running one, because neither a vote nor a priority dispatches (see [A finding starts a new epic](#a-finding-starts-a-new-epic)); `fno backlog undefer` or `supersede` when the row is the problem; keep at least one blueprint subagent running on the top unplanned node in the territory, so unplanned work gets designed without the user asking. For each `start` line the check-in printed, run `/fno:blueprint subagent <id>` in that order. Start no node the check-in did not name, since the ceiling is its call. A `skip` line needs no action; the row already records it. `fno inbox outstanding ask` when a lever needs the user.
...
Rank is not yours. It is the user's pin, and `fno backlog rank` refuses an agent session. A king who wants a row run next says so with `--priority p0`, which is bounded, receipted, and visible to the user as a split vote on `fno backlog demand`.
