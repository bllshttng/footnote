...
2. **Board change, 120s.** Do NOT poll `fno inbox board --json` (about two minutes at load). Read the journal with the canonical reader, never a hand-rolled tail of one file: `fno doctor event find <kind> --since <window> -J` covers every journal, every rotation and the `.ephemeral` sibling in one call, and reports `file_count`, `match_count` and `unreadable_files` separately, which is the positive control this paragraph demands. Board motion is `advance_dispatched`, `termination`, `plan_stamped` and `blocked`; a freed node is `claim_released`, and it lives in `events.jsonl.ephemeral`, not `events.jsonl`. There is no PR event kind at all: the PR signal is the `pr_number` field on the graph node, which is what the feed derives `pr_created` from. Trigger on the board digest, never a per-kind count: hash `(id, status, _kanban_column, priority)` over the scope, the four fields the daemon's own board trigger already hashes at `cli/src/fno/pr_watch/_king_wake.py:240`. A non-empty `unreadable_files` is probe failure, never a quiet board. When you do read the board, a bare call is already scoped to this crown (the caller's own manifest is the default); pass `--state <path>` only to read outside it.
...
4. **Main branch CI, 300s.** A red main blocks every merge in the fleet. Read `gh api repos/<owner>/<repo>/commits/<sha>/check-runs` and `gh api repos/<owner>/<repo>/commits/<sha>/status`, the REST reads [references/cli-commands.md](references/cli-commands.md) already names, and reduce them to one verdict token before comparing: `red` when any check-run `conclusion` is `failure`, `timed_out`, `cancelled`, `action_required` or `startup_failure`, or the legacy status `state` is `failure` or `error`; `green` only when at least one check run exists, every run is `completed`, none is red, and the combined `state` is `success`; `pending` otherwise. An empty check-run set is `pending`, never green: no data is CI that has not started, and `check-runs` cannot see legacy statuses, so a failing status beside green runs is red. Wake only when the token differs from the previous sample. `total_count` and any per-conclusion tally are never compared: measured 09:11Z to 09:23Z on one push, this arm woke three times on success counts 5, then 16, then 20, with zero failures and one identical verdict. A count moves on every finishing job; the verdict moves when the fleet's merge posture changes, which is the only thing this arm exists to report.
