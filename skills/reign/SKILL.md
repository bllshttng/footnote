...
4. **Main branch CI, 300s.** A red main blocks every merge in the fleet. Read `gh api repos/<owner>/<repo>/commits/<sha>/check-runs`, the REST read [references/cli-commands.md](references/cli-commands.md) already names, and reduce it to one verdict token before comparing: `red` when any `conclusion` is `failure`, `timed_out`, `cancelled`, `action_required` or `startup_failure`; `green` when every run is `completed` and none is red; `pending` otherwise. Wake only when the token differs from the previous sample. `total_count` and any per-conclusion tally are never compared: measured 09:11Z to 09:23Z on one push, this arm woke three times on success counts 5, then 16, then 20, with zero failures and one identical verdict. A count moves on every finishing job; the verdict moves when the fleet's merge posture changes, which is the only thing this arm exists to report.
