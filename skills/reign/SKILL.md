...
- `blocked_child:` a child under this crown emitted `<help>` and nothing answered it inside the grace window - the node, the session, and the age.
 ...
 - `scope <scope>:` leads with the owned active count (the active nodes no deeper live crown holds), then the active count in scope and the node total, then the active rows with worker, PR and session (the `fno agents court -n` join).
