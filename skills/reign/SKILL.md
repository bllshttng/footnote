...
- `held:` nodes an open operator question blocks, oldest question first, each with its question id, its age, and the `fno backlog decide <node> "<ruling>" --question-id <id>` command that clears it. `held: none` when no question holds a node.
- `scope <scope>:` the scope node counts, then the active rows with worker, PR and session (the `fno agents court -n` join).
- `epics:` one line under the scope line: each epic in the scope that holds an open direct child, fullest first, as `open/cap` against `backlog.epic_max_open_children`. `19/15 full` means the next child is refused, and `19/- (cap unset)` means no cap is set. The lead plans the split from this line, before a write bounces.
...
- `parked:` open PR parks, each with its reason, age and node, and the `fno-agents pr-park unpark <key>` remedy. `parked: none` when nothing is parked.

A failed reader prints `READER FAILED <name>: <reason>` on its own line and the beat continues, so one refused instrument can never blank a line or masquerade as a healthy value on another axis. The `coverage: N of M readings ok` line counts M as the readings this beat ran and N as the ones that answered, and a `failed readers:` line names each one that failed, so a beat with a failed reader can never read as a clean beat. Then a `vs last beat` line diffs the numeric keys against the previous canonical row for this scope, a `change:` line states what moved, and the ready-to-run `fno agents king faq add` command prints when this scope's FAQ store is empty or any reader failed.
