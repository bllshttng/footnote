...
Claude-Code primary. `ship pr` needs everything `/pr` needs (`fno`, `gh`, `git`; the `create` worker is whatever `/pr`'s configured role routing resolves - `/pr` owns that contract, not this skill). `ship doc` needs `fno` (the `research` + `evals grade` verbs). If a dependency is missing, the type fails loud and reports it - it never fakes a PR, a brief, or a grade.
