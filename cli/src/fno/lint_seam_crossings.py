...
on (rule, path, line-content) as a MULTISET; the line number is not recorded
at all, because it moves on every unrelated edit above it, and a ratchet that
churns on unrelated PRs teaches people to regenerate it, which is how a
ratchet stops ratcheting. Duplicate identical lines in one file are counted,
so deleting one of two identical sites still fails.
"""
...
def _read_baseline(baseline_path: Path) -> Counter[tuple[str, str, str]]:
    ...
    for line in baseline_path.read_text(encoding="utf-8").splitlines():
        ...
        if not stripped or stripped.startswith("#"):
            continue
        # rule<TAB>path<TAB>content. maxsplit=2 keeps any tab inside
        ...
        fields = [f.strip() for f in stripped.split("\t", 2)]
        if len(fields) < 3 or not all(fields[:2]):
            continue
        rule, rel, content = fields
...
def build_baseline(repo_root: Path) -> str:
    ...
    # Rows keep source order (the sort sees the line column the format
    # omits), so a regenerated file diffs as movement only when a site
    # is added or removed.
    sites = enumerate_sites(repo_root)
    out: list[str] = []
    for rule in _RULES:
        for rel, _line, text in sorted(sites[rule]):
            out.append(f"{rule}\t{rel}\t{text}")
...
def _baseline_header() -> str:
    return (
        ...
        "# Format: rule<TAB>path<TAB>content. Matching keys on"
        " (rule, path, content)\n"
        "# as a multiset; the line number is not recorded, so the file"
        " changes only when a\n# crossing is added or removed. pydoor takes"
        f" NO baseline: {PY_DOOR_FILE}\n# stays the only Python file that"
        " execs fno-agents by literal name.\n"
    )
