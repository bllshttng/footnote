...
# lane's bot identity. Deliberately a SEPARATE set from
...
def _push_to_parent(
    ...
    *,
    ...
    if result.returncode == 0 and result.stderr:
        # A successful send can still carry an origin-downgrade notice on
        # stderr; swallow it and a scheduler declaration made from inside an
        # agent session vanishes without a trace. A failed send reports its
        # own stderr below.
        typer.echo(
            f"push: note: {result.stderr.decode('utf-8', 'replace').strip()}",
            err=True,
        )
...
def find(
    ctx: typer.Context,
    kind: Optional[str] = typer.Argument(None, help="Event kind to match."),
    field: list[str] = typer.Option(
        [], "--field", help="Field filter FIELD=VALUE; may be repeated."
    ),
    ...
    """Query every event journal and retained rotation with a denominator.

    The reader accepts the three envelopes in the store. It names every file
    it covered and reports unreadable segments separately from a measured zero.
    """
