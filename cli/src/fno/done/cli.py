...
def done_command(
    ...
    # flag surface is --skip-stamp / -F --force / -R --reason. The four flags
    # below (--backfill, --pr, --link, --note) and the title-substring /
    # branch-autodetect query live only here, so the notice names the gap
    # instead of promising a drop-in swap. Removing this spelling means porting
    # them first. Only this stderr line is added, because callers parse stdout.
    typer.echo(
        "fno done is deprecated; use `fno backlog done <node-id>` to close a "
        "node. The flag surfaces differ: --backfill, --pr, --link, --note and "
        "the title/branch query exist only here and have no destination yet, "
        "so this spelling stays until they are ported.",
        err=True,
    )
