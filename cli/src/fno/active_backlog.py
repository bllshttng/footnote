...
def resolve_drain_targets(*, strict: bool = False) -> list[DrainTarget]:
    ...
    stays IGNORED (x-7f1f): targets are territory-keyed, never a config value.
    A territory that cannot be rooted (a rung-2 scope whose first member epic
    has no workspace path, an unmapped project) is skipped. Fail-safe throughout.
    """
    try:
