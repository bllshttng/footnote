...
def deferred_liveness_head(target: str) -> str:
    ...
    Both oracles are read off the module at call time. Binding them with
    ``from ... import`` would capture the function this module imported
    first, so a caller that replaces the attribute later would be ignored.
    """
    truth = session_truth.resolve_session_truth(target)
    state = truth["state"]
    if state == "unknown":
        return f"mail: liveness of {target} could not be established; " + _TAIL
    age = session_truth._humanize_age(truth["last_activity_age_s"]).strip()
    if state == "stalled":
