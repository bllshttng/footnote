...
def stop_agent(
    ...
    *,
    ...
    """Stop an agent, then release the claims it holds (x-9c91 change 5);
    the codex/gemini no-op arm releases nothing, because no stop happened."""
    result = _stop_agent_inner(
        name, lock_timeout=lock_timeout, shellout_timeout=shellout_timeout
    )
    if not result.noop:
        _release_stopped_claims(name)
    return result


def _release_stopped_claims(name: str, session_id: Optional[str] = None) -> None:
    """Release a stopped worker's claims via ``release-stopped``; best-effort,
    since the stop already happened and the reaper is the backstop.
    """
...
def _deliver_live(
    ...
    if entry.mux:
        ...
        for attempt in (1, 2):
            ...
            if (
                attempt == 2
                or failure_reason != "pre-submit"
                or getattr(entry, "exited", False)
                or not entry.mux
            ):
                break
        return False
