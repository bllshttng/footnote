...
RUST_CLIENT_VERBS = frozenset(
    {
        ...
        "prove-it-verdicts",
        # honesty-sweep (x-83ca): direct dispatch in client.rs; parity-test sync.
        "honesty-sweep",
...
RUST_ONLY_VERB_HELP: dict[str, str] = {
    ...
    "honesty-sweep": "Declared-vs-measured sweep over declared populations (x-83ca): --population harness-capabilities (the default) sweeps the compiled capability table, --population king-manifests sweeps the space's king manifests, --rows-json <path|-> [--rows-key <key>] [--name <label>] sweeps piped or filed JSON rows. Four passes: uniform fields, uniform among declarers, negative-claim counts, and (harness-capabilities only) negative claims paired with harness-named exported definitions plus hardcoded harness-name lists. Exit 2 unmeasured (unreadable source, <2 rows, invalid JSON), 0 otherwise - a finding is a candidate, never a verdict.",
