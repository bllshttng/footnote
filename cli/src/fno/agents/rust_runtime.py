...
RUST_CLIENT_VERBS = frozenset(
    {
        ...
        # parity test in sync.
        "probe-run",
        # Terminal prove-it record reader (x-6d64): walks every node's
        # `<plan>.artifacts/` for the `fno-prove-it:` terminal line and emits
        # one row per node verdict. Dispatched directly in client.rs before
        # build_request (no daemon RPC); this entry keeps the client.rs<->router
        # parity test in sync.
        "prove-it-verdicts",
