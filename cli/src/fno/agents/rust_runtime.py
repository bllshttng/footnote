...
def _is_pane_substrate_spawn(verb: str, args: Sequence[str]) -> bool:
    ...
    forbids). An ABSENT ``--substrate`` still routes here on purpose: the
    pane back half is Python-owned and its body resolves the built-in default
    (thread where the harness seats one, else pane), so a bare spawn takes the
    thread lane from Python without the binary re-deriving a second default.
...
def _is_keeper_thread_spawn(verb: str, args: Sequence[str]) -> bool:
    ...
    scans stop at ``--argv`` and a bare ``--``), and a headless spawn of
    these harnesses is NOT carved out: its honest refusal is the stance check
    the Python seam runs.
    """
    if verb != "spawn":
        return False
    harness = (_spawn_flag_value(args, "--harness", "-H") or "").strip().lower()
