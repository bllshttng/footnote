...
#:
#: The seed can cost a second round: one `_SEED_RETRY_DELAY_S` sleep plus a
#: second `_submit_spawn_seed` and its mux RPCs. That retry fires only from
#: `unattempted`, which an argv seed no longer reaches at all, so the worst case
#: is now the pane-send path (agy) rather than every slow-painting pane. Keep the
#: budget sized for it anyway: that path is slowest exactly when panes paint
#: slowest, so the two delays arrive together rather than independently. A miss costs
