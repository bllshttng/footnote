...
# ordered advance drain. Unconfigured it fails safe to the primary Anthropic
# model - no hardcoded tier. PR creation runs inline in the invoking session
# and dispatches no lane.
...
def unrouted_model_clear_notice(cleared: Sequence[str]) -> str:
    ...
    return (
        "fno: cleared "
        + ", ".join(cleared)
        + " from this child's env: a model the launching shell exported never "
        "reaches a child. The child's model comes from its --model flag, else "
        "its account's own default."
    )
