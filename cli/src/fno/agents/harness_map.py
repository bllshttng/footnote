...
def resolve_effective_verb(
    *,
    ...
    difficulty decides. Re-dispatch: the plan rung decides, and a build
    answer needs ``plan_blueprint`` True - a research doc with ``status:
    ready`` derives ``/blueprint``, not ``/target``. A declared target-family
    ``verb`` WINS over the table's answer and is returned as declared
    (d-834b6ff1); out-of-family abstains to declared precedence. Returns
    ``(canonical_verb, decision)``; ``None`` = abstain. Raises
    :class:`DispatchResolveError` on a refusal rung, or planless without
    low/medium/high difficulty - refusals fire before the declared verb.
    ``plan_rung`` is a Rung value; ``plan_blueprint`` None means not proven.
    The refusal leads with ``node_id`` when the caller holds one, so the
    subject of the failure is never read off a citation."""
    ...
    if rung == "none" and d in _DIFFICULTY_ANSWERS:
        ...
        why = f"plan {rung}"
        if answer == "/target" and plan_blueprint is not True:
            answer = "/blueprint"
            why = f"plan {rung} not a blueprint"
    else:
