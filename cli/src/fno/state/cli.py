...
def path(
    name: str = typer.Argument(
        ...,
        metavar="NAME",
        help="target-state | run-log | events | plans | inbox | kings | scratchpad | status-sinks",
    ),
    as_dir: bool = typer.Option(False, "--dir", help="Print the containing directory."),
) -> None:
    """Print the resolved space path for a project state file.

    Shell hooks call this instead of spelling ``<repo>/.fno/<file>`` so the
    location has one owner. The space is keyed on the canonical repo root, so
    the answer is identical from every worktree.
    """
    from fno import paths

    table = {
        "target-state": paths.target_state_path,
        "run-log": paths.run_log_path,
        "events": paths.project_events_json,
    ...
    if accessor is None:
        typer.echo(f"error: unknown state path {name!r} (known: {', '.join(sorted(table))})", err=True)
        raise typer.Exit(code=1)
    resolved = accessor()
    print_path = resolved.parent if as_dir else resolved
    typer.echo(str(print_path))
