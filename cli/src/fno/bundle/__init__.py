"""fno doctor bundle subcommands - thin wrappers over canonical bundler scripts.

Surface:
    fno doctor bundle         - regenerate per-skill bundles from skill-bundles.yaml
                         (default action when no subcommand given)
    fno doctor bundle check   - verify committed bundles match canonical (freshness gate)
    fno doctor bundle lint    - run the marketplace-readiness lint on driver skills
