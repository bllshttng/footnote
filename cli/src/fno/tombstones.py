...
name into ``tombstones.toml`` with the replacement, not deleting the name
...
def _tombstones() -> dict[str, str]:
    """The table, read once from the sibling ``tombstones.toml``.

    The entries are data, so they live in a data file beside this loader (the
    pattern ``agents/harness_capabilities.toml`` already uses). Read lazily and
    cached: this module is imported on the CLI's hot path, where an unknown
    command is rare and a file read per start is not worth paying.
    """
    global _TABLE
    if _TABLE is None:
        import tomllib

        path = Path(__file__).with_name("tombstones.toml")
        _TABLE = {k: str(v) for k, v in tomllib.loads(path.read_text(encoding="utf-8")).items()}
    return _TABLE
...
class TombstoneGroup(typer.core.TyperGroup):
    """A group whose unknown-command error consults ``tombstones.toml``.
