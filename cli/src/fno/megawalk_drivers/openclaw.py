"""
OpenclawDriver - wraps the `openclaw` CLI.

Mirrors scripts/lib/driver-openclaw.sh. Returns is_available()=False when
openclaw is not on PATH; the walker treats unavailable drivers as a hard
...
class OpenclawDriver:
    ...
    def invoke_review(self, *, prompt: str, **kwargs) -> InvokeResult:
        """Review mode is not supported by the openclaw driver.
        ...
        raise UnsupportedDriverMode(
            "openclaw driver does not support review mode; "
