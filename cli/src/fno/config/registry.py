...
FIELD_META: dict[str, Meta] = {
    ...
    "agents.max_swap_pct": Meta("advanced", "Swap-pressure ceiling in percent used for spawn preflight, checked beside agents.min_free_gb on EVERY spawn; spawn refuses at or above it (<= 0 disables; default 90). The RAM floor cannot see swap: available counts reclaimable pages, so a machine paging at 93% swap can read six times its floor. The receipt names the failing term; raising min_free_gb does not fix a swap refusal.", default_source="default"),
