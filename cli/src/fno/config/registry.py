...
FIELD_META: dict[str, Meta] = {
    ...
    "agents.defaults.substrate": Meta("advanced", "Default substrate for bare spawns (pane|thread|headless); bg is a deprecated alias for thread for one release. An explicit substrate wins, empty = unset (per-provider default). Config-sourced value degrades open with a warning if incompatible with the resolved provider (e.g. thread is claude + opencode).", default_source="default"),
    ...
    "dispatch.substrate": Meta("advanced", "Default dispatch substrate (thread|headless|pane); bg is a deprecated alias for thread for one release. Empty = per-harness default (claude + opencode=thread, else headless).", default_source="default"),
