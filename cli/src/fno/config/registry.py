...
FIELD_META: dict[str, Meta] = {
    ...
    "recovery.watchdog": Meta("advanced", "External fleet watchdog lane on the same tick: off (default) | report (publish the unfinished-work report: in_progress nodes with free claims, done branches ahead of origin/main, dirty ownerless worktrees, and ownerless PRs older than 24h, each finding naming its clearing verb) | wake (report, plus resume positively stalled sessions). Reap and reroute never fire from a tick; they need a manual `fno agents watchdog --apply-all`."),
