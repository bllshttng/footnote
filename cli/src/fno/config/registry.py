...
FIELD_META: dict[str, Meta] = {
    ...
    "dispatch.verb_registry": Meta("advanced", "Dispatch verbs from outside fno, as [dispatch.verb_registry.<name>] tables with the descriptor fields (invocation, invocations, requires, takes_node_id, asserts). Unioned with the allowed_verbs allowlist at resolve; a key naming an allowlisted verb is dropped, so a project cannot redefine a shipped one. invocation is the worker's exact spelling and is rendered verbatim (never fno-namespaced); invocations maps harness to spelling and doubles as the harness allowlist, so a dispatch to any other harness refuses. requires=skill probes the harness skill roots at resolve and refuses naming them; asserts records what a completion proves (pr | doc | invocation).", default_source="default"),
