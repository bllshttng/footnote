...
FIELD_META: dict[str, Meta] = {
    ...
    "recovery.watchdog.end_after_drives": Meta("advanced", "Whether the SILENCE lane may end a row once recovery.max_nudges drives went silent (default false: drive and report, never stop). Ending stops the harness, force-releases the node claim, removes the registry row, then hands the node back through `fno backlog advance` for a fresh thread or harness."),
