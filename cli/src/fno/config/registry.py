...
FIELD_META: dict[str, Meta] = {
    ...
    "agents.crown_config_grant": Meta("advanced", "Authorize `fno agents crown` from an unattended session with no superset crown (a standing, level-0-equivalent human grant; DEFAULT OFF). Authorizes the grant path only - never bypasses the self-grant / one-live-crown-per-scope refusals.", default_source="default"),
    ...
    "agents.profiles": Meta("advanced", "Per-verb spawn-defaults overlay keyed by the seed's leading slash-verb (agents.profiles.<verb>.{provider,model,effort,substrate,permission_mode}), one rung above agents.defaults in precedence. The verb key is the seed's first token, /-stripped and fno:-namespace-stripped, lowercase (/fno:think and /think resolve alike). No content-based inference; a table with no matching verb resolves exactly as agents.defaults.", default_source="default"),
    ...
    # --- config.recovery.* ---
    "recovery.enabled": Meta("advanced", "Enable the bg-session recovery sweep: provider failover on swap-class deaths plus close-surfacing for finished-but-lingering sessions (rides the pr_watch tick)."),
    "recovery.idle_threshold_seconds": Meta("never", "How stale a bg session must be (seconds) before recovery acts on it."),
    "recovery.max_nudges": Meta("never", "Per-session cap on recovery actions (close-surface notifications) before it stops acting on this session."),
