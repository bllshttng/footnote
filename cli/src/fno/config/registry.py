...
FIELD_META: dict[str, Meta] = {
    ...
    "agents.max_load_per_cpu": Meta("advanced", "Load at which spawn preflight stops trusting load average and consults fleet CPU attribution: this factor times the CPU count on the 1-min load average (<= 0 disables the whole check; default 8). A TRIGGER, not a refusal - it refused outright until x-7c0f, twice on load the fleet did not cause, because load average counts blocked processes and measures nobody's CPU share. Measured motivation for keeping a load signal at all: load 309 on 12 CPUs while the RAM floor held ten times its margin.", default_source="default"),
