...
FIELD_META: dict[str, Meta] = {
    ...
    "agents.defaults.provider": Meta("advanced", "Default provider for bare `fno agents spawn` / `/agent spawn` (claude/codex/gemini/agy/opencode); an explicit -H flag wins, empty = unset (harness inference then claude). Validated at the spawn seam.", default_source="default"),
    ...
    "agents.profiles": Meta("advanced", "Per-verb spawn-defaults overlay keyed by the seed's leading slash-verb (agents.profiles.<verb>.{provider,model,effort,substrate,permission_mode,route,account}), one rung above agents.defaults in precedence. The verb key is the seed's first token, /-stripped and fno:-namespace-stripped, lowercase (/fno:think and /think resolve alike). The overlay reaches every spawn carrying a slash-verb seed, including autonomous dispatch; an explicit flag always wins. No content-based inference; a table with no matching verb resolves exactly as agents.defaults.", default_source="default"),
    ...
    "agents.max_lanes": Meta("advanced", "Immediate-refusal concurrent live-worker caps keyed by model provider; unlisted providers are uncapped (default zai = 5).", default_source="default"),
    ...
    "recovery.watchdog": Meta("advanced", "External fleet watchdog lane on the same tick: off (default) | report (classify and emit) | wake (also resume stalled sessions) | handoff (also permit a proved cross-provider outage transaction). Reap and reroute never fire from a tick; they need a manual `fno agents watchdog --apply-all`."),
