...
FIELD_META: dict[str, Meta] = {
    ...
    "agents.reap.roster_scope": Meta("advanced", "Which rows the roster sweep may retire: off retires nothing and reads roster=off on the tick, provenanced (the default) only rows whose provenance resolves to fno and whose work is done, all widens to rows fno itself spawned (sessions or registry provenance) with open work. The daemon's retire arm runs the sweep after the registry sweep, so off stops the scheduled pass; the manual verb keeps its own default. An unowned session retires only on an ownership marker: a sessions[] row fno wrote, the registry's stored node field, or a reap receipt an earlier retirement staged for the same session. A present value that cannot be honored degrades in place to the default and is named by fno config doctor.", default_source="default"),
