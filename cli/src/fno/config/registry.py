...
FIELD_META: dict[str, Meta] = {
    ...
    "agents.defaults.model": Meta("advanced", "Default model for bare spawns, forwarded as --model but provider-scoped: it applies to the harness it was written for (the configured provider, else the inferred one), so a spawn resolving to a different harness leaves the model to that harness rather than forcing an incompatible one. An explicit -m flag always wins; empty = unset (provider default). Passthrough (provider CLIs own model names).", default_source="default"),
