...
FIELD_META: dict[str, Meta] = {
    ...
    "recovery.watchdog": Meta("advanced", "External fleet watchdog lane on the same tick: off (default) | report (publish the unfinished-work report: in_progress nodes with free claims, done branches ahead of origin/main, dirty ownerless worktrees, and ownerless PRs older than 24h, each finding naming its clearing verb) | wake (report, plus resume positively stalled sessions) | handoff (also permit a proved cross-provider outage transaction). Reap and reroute never fire from a tick; they need a manual `fno agents watchdog --apply-all`."),
    "recovery.watchdog_mail_to": Meta("advanced", "Mail handle the unfinished-work digest is pushed to when the finding set changes (agent name, short id, or project:<slug>). Empty (default) mails nobody."),
