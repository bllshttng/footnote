...
FIELD_META: dict[str, Meta] = {
    ...
    "agents.reap.roster_scope": Meta("advanced", "Which rows the roster sweep may retire: off retires nothing, provenanced (the default) only rows whose provenance resolves to fno and whose work is done, all widens to resolved rows with open work. A row that resolves to no fno node is never retirable at any value, including all, so a session the operator started by hand is safe by construction. A misspelled value degrades to the default and is named by fno config doctor.", default_source="default"),
