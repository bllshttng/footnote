...
@dataclass
class EpicHeldOpen:
    """One terminal epic (``done``/``superseded``) holding a live child open.

    The inverse of :class:`EpicStuck`: that class asks which open epics
    cannot advance, this asks which closed epics should not have. Read-only
    by construction, same as :func:`stuck_epics` - this module surfaces, it
...
def stuck_epics_closed(entries: list[dict]) -> list[EpicHeldOpen]:
    """Terminal epics (``done``/``superseded``) holding at least one live child.

    Reuses :func:`holds_epic_open` unchanged: a live child is one that still
    holds an epic open under the same two-rule vocabulary ``stuck_epics``
    uses (``superseded`` and a ``wont_do`` deferral never hold; everything
    else, including an unclassified deferral, does). The only difference
    from ``stuck_epics`` is which epics are asked about - terminal ones
    instead of open ones - so a ``wont_do`` child never appears here for the
    same reason it never appears there.
    """
    by_id = {
        e.get("id"): e
        for e in entries
    ...
    for e in entries:
        ...
        if not kids:
            continue
        epic_status = e.get("status")
        if epic_status not in _EPIC_TERMINAL_STATUSES:
            continue
        holders: list[dict] = []
        for k in kids:
            child = by_id.get(k.get("id"))
