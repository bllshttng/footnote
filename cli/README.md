...
|---|---|---|
| **PyPI / uv** | `uv tool install fno` or `pip install fno` | The `fno` CLI, all three Rust binaries (`fno-agents`, `fno-agents-daemon`, `fno-agents-worker`) on PATH, and the internalized verbs (`fno plan`, `fno notify`, `fno phase kill-check`, ...). `fno target` and `fno bundle` need the plugin clone - on a bare install they exit non-zero with an "install the plugin" message rather than a traceback. |
