...
> **`fno` vs `fno-py`.** The `fno` you type is the Rust mux front door: bare `fno` launches the mux, `fno mux <verb>` runs natively, and every other verb forwards to the Python CLI. The Python CLI ships as the **`fno-py`** console script - the forward target, not something you type. Today only the **cargo** channel (or a manual `cargo install --path crates/fno` / `fno update`) installs the `fno` front door; the py-wheel channels below (PyPI/uv, brew, fno.sh) install `fno-py` + the `fno-agents*` binaries. Bundling the mux binary into those channels so they deliver `fno` too is tracked as follow-up. Run `fno doctor` to see whether `fno` on PATH is the mux (front-door: active) or a Python `fno-py` is shadowing it. Where the cells below say "the `fno` CLI", read "the `fno-py` CLI" for the py-wheel channels.

A channel is a complete front door - you pick one, never "install both". What each delivers:

| Channel | Command | You get |
|---|---|---|
| **PyPI / uv** | `uv tool install fno` or `pip install fno` | The `fno` CLI, all three Rust binaries (`fno-agents`, `fno-agents-daemon`, `fno-agents-worker`) on PATH, and the internalized verbs (`fno plan`, `fno notify`, `fno executor`, `fno phase kill-check`, ...). `fno target` and `fno bundle` need the plugin clone - on a bare install they exit non-zero with an "install the plugin" message rather than a traceback. |
