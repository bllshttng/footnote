...
# --------------------------------------------------------------------------- #
# x-8151: route_to_rust is the carrier gate for the spawn lane. The wrapper
# applies harness_map.apply_merge_posture_env to os.environ before exec, so
# the binary's children inherit the verdict and no client-side predicate
# re-derivation exists. The stand-in group carries the SAME positional shape
# the real spawn parser has (one message argument); the family vocabulary
# under test lives in test_dispatch_no_merge_env.py.
# --------------------------------------------------------------------------- #

def _spawn_standin_group():
    import click
    ...
    def spawn(message, name):
        raise AssertionError("the door must exec, never invoke the command")

    return grp


def test_route_arms_carrier_for_flag_family_spawn(monkeypatch, tmp_path):
    import os
    ...
    def fake_exec(path, argv):
        raised.append(os.environ.get("TARGET_NO_MERGE"))
        raise SystemExit(0)

    with pytest.raises(SystemExit) as exc:
        rr.route_to_rust(
            ["spawn", "/target --no-merge x-1", "--name", "w"],
            binary=binary,
            group=_spawn_standin_group(),
            _exec=fake_exec,
        )
    assert exc.value.code == 0
    assert raised == ["1"], "carrier must cross into the exec"


def test_route_clears_inherited_carrier_for_flagless_family_spawn(
    ...
    with pytest.raises(SystemExit):
        rr.route_to_rust(
            ["spawn", "/target x-1", "--name", "w"],
            binary=binary,
            group=_spawn_standin_group(),
            _exec=fake_exec,
        )
    assert raised == [None], "the message is authoritative in both directions"


def test_route_prose_spawn_leaves_inherited_carrier_alone(monkeypatch, tmp_path):
    import os
    ...
    with pytest.raises(SystemExit):
        rr.route_to_rust(
            ["spawn", "review the no-merge rollout plan", "--name", "w"],
            binary=binary,
            group=_spawn_standin_group(),
            _exec=fake_exec,
        )
    assert raised == ["1"], "non-family messages clear nothing (round 8)"


def test_route_non_spawn_verb_skips_the_gate(monkeypatch, tmp_path):
    import os

    binary = _make_exe(tmp_path / rust_binary.BINARY_NAME)
    monkeypatch.setenv("TARGET_NO_MERGE", "keep")
    raised = []
...
def test_route_gate_parse_failure_execs_with_a_loud_line(monkeypatch, tmp_path):
    """A parse blowup must never take the exec hostage, but it may not pass
    silently either: a dropped refusal posture reads as a grant downstream."""
    binary = _make_exe(tmp_path / rust_binary.BINARY_NAME)
    ...
    with pytest.raises(SystemExit) as exc:
        rr.route_to_rust(
            ["spawn", "/target --no-merge x-1"],
            binary=binary,
            group=ExplodingGroup(),
            _exec=fake_exec,
        )
    assert exc.value.code == 0  # the exec still happened
