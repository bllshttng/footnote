...
subscription, debug builds of fno-agents + the Rust front from the worktree:
**GREEN, all seven steps, in 17s.** One session file carries both turns
(``<id>`` in its name matches both prompts): the codeword goes in before the
kill, ``PIKEEPER`` comes back after both supervisors were SIGKILLed and the
daemon had rebound the row. Two fixes were needed to get here, both
instructive. (1) The row's keeper socket path was spelled with ``.resolve()``,
which canonicalizes ``/var`` to ``/private/var`` on macOS; the daemon sweep
derives the raw path and byte-compares, so the row never matched its keeper
and the sweep logged an orphan instead of a rebind. Never canonicalize one
side of a byte-compared path pair. (2) The session-store snapshot is taken
AFTER the first turn: writing a session file per turn is pi's normal
behavior, so the baseline must already contain the first turn's file - the
assertion is that the RESTART mints nothing, and that strength is unchanged.
...
def _send_mail(text: str, session_id: str, monkeypatch, capsys) -> None:
    ...
    # The injector's own confirm window is short; a cold pi's first turn can
    # land after it. The receipt is not the proof - the transcript is. The
    # step-7 answer assertions below are the gate, so a queued (durable)
    # receipt here is recorded, never fatal.
...
def test_AC1_HP_the_restart_journey_on_a_real_pi_thread(tmp_path, monkeypatch, capsys) -> None:
    ...
    try:
        ...
        # ---- Step 2: first turn, then snapshot pi's session store. The
        # baseline must ALREADY contain the first turn's session file: the
        # claim under test is that the restart mints nothing, not that pi
        # never persists a turn. A pre-turn baseline fails on any correct pi
        # (run 8: the first turn's own file was the sole diff).
        ...
        rebound = _rebound_emitted()
        while not rebound and time.monotonic() < deadline:
            time.sleep(0.5)
            rebound = _rebound_emitted()
        # Failure-path observability: name the sweep branch that fired
        # instead of a bare absence (an orphan or wedged event here says
        # which leg diverged; none at all says the sweep never ran).
        sweep_events: list[str] = []
        if not rebound and events.exists():
            sweep_events = [
                line.strip()
                for line in events.read_text(encoding="utf-8").splitlines()
                if line.strip() and "keeper_" in line
            ]
        assert rebound, (
            "the daemon start never emitted keeper_row_rebound; keeper "
            f"events: {sweep_events or 'none at all'}"
        )
