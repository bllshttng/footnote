...
UNREAD_OUT=$(uv run fno-py agents mail unread --name proj-b --json)
...
uv run fno-py agents mail ack "$MSG_ID" --name proj-b
UNREAD_OUT=$(uv run fno-py agents mail unread --name proj-b --json)
if [[ "$UNREAD_OUT" != "[]" ]]; then
...
set +e
NOTIF_OUT=$(uv run fno-py agents mail send --to-project proj-b --kind notification \
