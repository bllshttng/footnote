...
PARENT_OUT=$(uv run fno-py agents mail send --to-project api --kind question \
...
REPLY_OUT=$(uv run fno-py agents mail send --to-project api --kind fyi \
...
# 4) fno agents mail reply sends to the ORIGINAL sender (web here), not back to api.
#    Since web's inbox has no thread yet, an orphan thread lands in web/inbox/.
THIRD_OUT=$(uv run fno-py agents mail reply --to "$PARENT_MSG" --kind fyi \
