...
help_out=$(uv run fno done --help 2>&1)
...
set +e
nomatch_out=$(uv run fno done nonexistent-xyzzy-smoke 2>&1)
nomatch_rc=$?
set -e
...
uv run fno done ab-smoke0001 --note "smoke test marker" >/dev/null 2>&1
status=$(python3 -c "
import json
...
JSON
uv run fno done ab-smoke0002 --pr 99 >/dev/null 2>&1 || true
rollup=$(python3 -c "
import json
d = json.load(open('$TMP/.fno/graph.json'))
...
"
uv run fno done ab-smoke0002 --backfill >/dev/null 2>&1
