#!/usr/bin/env bash
# Scenario B: Q/A/lesson chain between fno (supervisor) and acme-web.
...
TMP=$(mktemp -d /tmp/fno-scenario-B-XXXXXX)
...
export FNO_INBOX_KNOWN_PROJECTS="fno,acme-web"
...
echo ""
echo "--- Step 3: verify fno inbox has answer with reply_to chain ---"
FNO_INBOX="$INBOX_ROOT/fno.md"
if [ ! -f "$FNO_INBOX" ]; then
    echo "FAIL: fno inbox file not created at $FNO_INBOX"
    exit 1
fi
...
if not matches:
    print('FAIL: MSG_A not found in fno inbox')
    sys.exit(1)
m = matches[0]
errors = []
...
if errors:
    ...
    for e in errors:
        print('  ' + e)
    sys.exit(1)
print('OK: answer in fno inbox with correct reply_to chain')
" <<< "$LIST_ABILITIES_JSON"
...
# Step 5: Simulate supervisor (fno) writing a lesson to memory and sending it
echo ""
echo "--- Step 5: fno writes lesson memory and sends it to acme-web ---"
...
if m.get('from') != 'fno':
    errors.append('from expected fno, got ' + str(m.get('from')))
...
# fno inbox: should have MSG_A
fno_msgs = json.load(open('$INBOX_ROOT/fno.md'.replace('.md', '.md')))
" 2>/dev/null || true
