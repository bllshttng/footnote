...
def test_exact_ceiling_passes_and_one_byte_over_fails(tmp_path: Path) -> None:
    """AC2-HP: the comparison is strictly greater than the ceiling."""
    _write_fixed_roots(tmp_path, agents_bytes=37_890)
    at_ceiling = _run(tmp_path)
    assert at_ceiling.returncode == 0, at_ceiling.stderr
    assert _reported_total(at_ceiling) == 38_000

    (tmp_path / "AGENTS.md").write_bytes(b"a" * 37_891)
    over = _run(tmp_path)
    assert over.returncode == 1
    assert _reported_total(over) == 38_001
    assert "ceiling by 1 " in over.stderr


def test_missing_fixed_root_fails_loud(tmp_path: Path) -> None:
    """AC3-ERR: an absent fixed root is not treated as zero bytes."""
    _write_fixed_roots(tmp_path, agents_bytes=100)
    (tmp_path / "AGENTS.md").unlink()
...
def test_empty_rules_glob_is_legal(tmp_path: Path) -> None:
    """AC5-EDGE: no rule files contributes zero without a literal glob."""
    _write_fixed_roots(tmp_path, agents_bytes=100)
    ...
    assert result.returncode == 0, result.stderr
    assert "*.md" not in result.stdout
    assert ".claude/rules/" not in result.stdout
    assert "AGENTS.md" in result.stdout
    assert "CLAUDE.md" in result.stdout
    assert "skills/using-fno/SKILL.md" in result.stdout


def test_all_zero_byte_inputs_pass(tmp_path: Path) -> None:
    """AC5-EDGE: a discovered but empty preamble remains under budget."""
    _write_fixed_roots(tmp_path, agents_bytes=0)
    (tmp_path / "CLAUDE.md").write_bytes(b"")
    (tmp_path / "skills" / "using-fno" / "SKILL.md").write_bytes(b"")
...
def test_breach_explains_trade_before_ceiling_raise(tmp_path: Path) -> None:
    """AC6-FR: failure output teaches the preferred remediation order."""
    _write_fixed_roots(tmp_path, agents_bytes=38_000)

    result = _run(tmp_path)

    assert result.returncode == 1
    assert re.search(r"AGENTS\.md\s+38000", result.stderr)
    assert re.search(r"skills/using-fno/SKILL\.md\s+100", result.stderr)
    assert re.search(r"CLAUDE\.md\s+10", result.stderr)
    assert "tok/turn" in result.stderr
    assert result.stderr.index("Trade:") < result.stderr.index("Raise CEILING_BYTES")
