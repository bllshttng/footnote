...
def test_defer_kind_roundtrips_through_read_graph(tmp_graph):
    ...
    node = {e["id"]: e for e in read_graph_strict(tmp_graph)}["x-dddd"]
...
def test_defer_rejects_unknown_kind(tmp_graph):
    ...
    assert {e["id"]: e for e in read_graph_strict(tmp_graph)}["x-dddd"].get("status") != "deferred"
...
def test_retract_defers_and_stamps_the_kind(tmp_graph):
    ...
    assert node["deferred_reason"] == "filed on a false premise"
...
def test_backfill_refuses_bad_map_before_touching_graph(tmp_graph):
    ...
    assert {e["id"]: e for e in read_graph_strict(tmp_graph)}["x-aaaa"].get("deferred_kind") is None
