...
class TestTickRecordsAndDeadline:
    ...
    def test_default_read_pr_state_bounded_by_the_phase_clock(self, monkeypatch):
        """AC1-HP (x-713b): with 20s of phase left, the rich read's timeout
        keeps the slice's 10s persist reserve instead of outliving the alarm."""
        import time as _time
    ...
    def test_default_read_pr_state_keeps_30s_without_a_phase(self, monkeypatch):
        """AC2-EDGE (x-713b): no phase armed, the read keeps today's 30s bound."""
        import fno.pr_watch._discover as disc
        from fno.pr_watch import _dispatch as d
...
class TestDurableGrantExecution:
    ...
    def test_execute_budget_skips_without_calling_merge(self, tmp_path, monkeypatch):
        ...
        budget = [
            ...
            and e["data"].get("reason") == "execute-budget"
        ]
        assert [e["data"]["pr"] for e in budget] == [1]
        from fno.pr_watch._state import WatermarkStore
...
class TestCandidateReadDiscipline:
    ...
    def test_cold_ritual_fires_with_a_full_slice(self, tmp_path, monkeypatch):
        """AC3-HP (x-713b): 150s of phase left holds the cold ritual; the
        merge dispatch runs once."""
        from types import SimpleNamespace
        ...
        res = self._tick(
            tmp_path, deps, store_path,
            listing=self._listing({"owner/repo#7"}), ritual_fn=fake_ritual,
            deadline=clock["t"] + 600.0,
        )
        ...
        assert res.acted == 1
        dispatched = [
            e for e in deps["events"] if e["type"] == "pr_watch_dispatched"
        ]
    ...
    def test_cold_ritual_held_back_under_the_ritual_floor(self, tmp_path, monkeypatch):
        """AC4-ERR (x-713b): 60s of slice cannot hold a ritual that needs
        ~100s; the merge is skipped as fire-budget, burns no retry, and the
        scan still counts the candidate."""
        from types import SimpleNamespace
    ...
    def test_review_fire_keeps_the_thirty_second_floor(self, tmp_path, monkeypatch):
        """AC5-EDGE (x-713b): a review check at 60s of phase left still fires;
        only the merge ritual waits for the 100s floor."""
        from types import SimpleNamespace
    ...
    def test_read_records_the_sweep_dispatch_sub_step(self, tmp_path):
        """AC6-HP (x-713b): a rich read runs inside the sweep:dispatch step,
        so a cut there names the stall instead of a bare timeout."""
        import fno.pr_watch._dispatch as d
    ...
    def test_scan_note_counts_failed_reads(self, tmp_path, monkeypatch):
        """AC7-ERR (x-713b): one failed read of two keeps its place in the
        cut note: scanned=1 of 2 read_failed=1."""
        import fno.pr_watch._dispatch as d
        from fno.graph._reconcile import ReconcileError
        ...
        self._tick(tmp_path, deps, store_path, listing=self._listing(set()))
