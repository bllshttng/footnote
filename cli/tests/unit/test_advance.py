...
def test_spawn_worker_attaches_gate_exit_and_last_gate_line(monkeypatch):
    ...
    with pytest.raises(adv.SpawnError) as ei:
        adv._spawn_worker("ab-2222aaaa", None, node=_spawn_node())
    exc = ei.value
...
def test_spawn_worker_argv_with_cwd(monkeypatch):
    ...
    monkeypatch.setattr(adv.subprocess, "run", fake_run)
    sid = adv._spawn_worker("ab-2222aaaa", "/work/dir", node=_spawn_node())
...
def test_spawn_worker_threads_model_and_provider(monkeypatch):
    ...
    monkeypatch.setattr(adv.subprocess, "run", fake_run)
    adv._spawn_worker("ab-2222aaaa", "/w", model="glm-4.7", provider="codex", node=_spawn_node())
    cmd = captured["cmd"]
...
def test_spawn_worker_default_provider_claude(monkeypatch):
    ...
    monkeypatch.setattr(adv.subprocess, "run", fake_run)
    adv._spawn_worker("ab-2222aaaa", "/w", node=_spawn_node())
    cmd = captured["cmd"]
...
def test_spawn_worker_argv_fresh_when_no_cwd(monkeypatch):
    ...
    monkeypatch.setattr(adv.subprocess, "run", fake_run)
    sid = adv._spawn_worker("ab-2222aaaa", None, node=_spawn_node())
...
def test_spawn_worker_verb_routes_command_and_brief_env(monkeypatch):
    ...
    monkeypatch.setattr(adv.subprocess, "run", fake_run)
    adv._spawn_worker(
        "ab-2222aaaa", "/w", verb="/think", brief="explore the retry design", node=_spawn_node()
    )
    cmd = captured["cmd"]
...
def test_spawn_worker_verb_normalizes_codex_thread(monkeypatch):
    ...
    monkeypatch.setattr(adv.subprocess, "run", fake_run)
    adv._spawn_worker(
        "ab-2222aaaa", "/w", harness="codex", provider="codex-acct", verb="/think", node=_spawn_node()
    )
    cmd = captured["cmd"]
    assert cmd[cmd.index("--substrate") + 1] == "thread"
...
def test_spawn_worker_thread_without_session_id_refuses(monkeypatch):
    ...
    with pytest.raises(adv.SpawnError, match="no launch-identity receipt"):
        adv._spawn_worker(
            "ab-2222aaaa", "/w", harness="codex", provider="codex", verb="/target", node=_spawn_node()
        )
...
def test_spawn_worker_bg_still_requires_short_id(monkeypatch):
    ...
    with pytest.raises(adv.SpawnError):
        adv._spawn_worker("ab-2222aaaa", "/w", node=_spawn_node())  # default -> claude/bg
...
def test_spawn_worker_extra_env_reaches_subprocess(monkeypatch):
    ...
    monkeypatch.setattr(adv.subprocess, "run", fake_run)
    adv._spawn_worker("ab-2222aaaa", "/w", extra_env={"CLAUDE_CONFIG_DIR": "/acct/x"}, node=_spawn_node())
...
def test_spawn_worker_auto_merge_drops_no_merge(monkeypatch):
    ...
    monkeypatch.setattr(adv.subprocess, "run", fake_run)
    adv._spawn_worker("ab-2222aaaa", None, node=_spawn_node())  # node_cwd=None -> load_settings()
...
def test_spawn_worker_unknown_harness_raises_resolve_error(monkeypatch):
    ...
    with pytest.raises(DispatchResolveError):
        adv._spawn_worker("ab-2222aaaa", "/w", harness="bogus", node=_spawn_node())
...
def test_spawn_worker_declared_verb_lands_in_name(monkeypatch):
    ...
    monkeypatch.setattr(
        adv, "_grid_lane_for", lambda node, **kw: (None, None, None, None, None)
    )
    adv._spawn_worker(
        "x-7aa8abc1",
        None,
        "daily-pass",
        verb="/think",
        # declare the verb so the low difficulty does not derive /target over it
        node=_spawn_node(dispatch_verb="/think"),
    )
    name = captured["cmd"][-2]
...
def test_spawn_worker_name_includes_slug(monkeypatch):
    ...
    monkeypatch.setattr(adv.subprocess, "run", fake_run)
    adv._spawn_worker("ab-2222aaaa", None, "cargo-bootstrapper", node=_spawn_node())
...
def test_spawn_worker_refuses_source_reconcile_impossible_pair():
    ...
    with pytest.raises(adv.SpawnError, match="impossible pair"):
        adv._spawn_worker(
            ...
            source="ac", reconcile_manifest="/tmp/manifest.json", node=_spawn_node(),
        )
...
def test_spawn_worker_unknown_verb_word_refuses(monkeypatch):
    ...
    with pytest.raises(Exception, match="unknown dispatch verb"):
        adv._spawn_worker(
        "x-1", None, "s", verb="/impeccable", node=_spawn_node(dispatch_verb="/impeccable")
    )
...
def test_spawn_worker_skips_noise_line_mentioning_short_id(monkeypatch):
    ...
    assert adv._spawn_worker("ab-2222aaaa", None, node=_spawn_node()) == "abc12345"
...
def test_spawn_worker_accepts_codex_thread_full_id_receipt(monkeypatch):
    ...
    identity = adv._spawn_worker("ab-2222aaaa", None, harness="codex", node=_spawn_node())
...
def test_spawn_worker_refuses_codex_head8_launch_identity(monkeypatch):
    ...
    with pytest.raises(adv.SpawnError) as excinfo:
        adv._spawn_worker("ab-2222aaaa", None, harness="codex", node=_spawn_node())
...
def test_spawn_worker_fills_receipt_out_param(monkeypatch, tmp_path):
    ...
    receipt: dict = {}
    sid = adv._spawn_worker("ab-2222aaaa", None, events_path=ev, receipt=receipt, node=_spawn_node())
...
def test_spawn_worker_receipt_stays_empty_on_spawn_error(monkeypatch):
    ...
    with pytest.raises(adv.SpawnError):
        adv._spawn_worker("ab-2222aaaa", None, receipt=receipt, node=_spawn_node())
    assert receipt == {}
...
def test_cmd_advance_resolves_closed_project_from_graph(monkeypatch):
    ...
    monkeypatch.setattr(
        gcli, "wire_rows",
        lambda path=None, **k: [{"id": "ab-A", "project": "fno"}],
    )
...
def test_direct_dependents_admits_ready_and_cold_idea(monkeypatch):
    ...
    monkeypatch.setattr("fno.graph.api.wire_rows", lambda path=None: entries)
...
def test_spawn_worker_passes_substrate_bg(monkeypatch):
    ...
    sid = adv._spawn_worker("ab-2222aaaa", "/tmp/x", "some-slug", node=_spawn_node())
...
def test_spawn_worker_reconcile_keeps_substrate_bg(monkeypatch):
    ...
    captured = _capture_spawn_argv(monkeypatch)
    sid = adv._spawn_worker(
        "ab-2222aaaa", "/tmp/x", "some-slug", reconcile_manifest="/tmp/m.md", node=_spawn_node()
    )
    assert sid == "abc12345"
    cmd = captured["cmd"]
    i = cmd.index("--harness")
...
def test_spawn_worker_error_contract_unchanged(monkeypatch):
    ...
    with pytest.raises(adv.SpawnAlreadyRunning):
        adv._spawn_worker("ab-2222aaaa", "/tmp/x", "slug", node=_spawn_node())
...
def test_spawn_worker_auto_merge_true_omits_no_merge(monkeypatch):
    ...
    adv._spawn_worker("ab-2222aaaa", "/work/dir", node=_spawn_node())
...
def test_spawn_worker_auto_merge_reconcile_variant(monkeypatch):
    ...
    captured = _capture_spawn_argv(monkeypatch)
    monkeypatch.setattr(_config, "load_settings_for_repo", lambda _p: _settings_ns(auto_merge=True))
    adv._spawn_worker("ab-2222aaaa", "/work/dir", reconcile_manifest="/tmp/m.md", node=_spawn_node())
...
def test_spawn_worker_auto_merge_reads_dependent_cwd(monkeypatch):
    ...
    adv._spawn_worker("ab-2222aaaa", "/dependent/repo", node=_spawn_node())
...
def _spawn_argv(monkeypatch, *, provider, perm_config, permission_mode=None, substrate="bg", harness=None):
    ...
    adv._spawn_worker("ab-2222aaaa", None, "next", provider=provider, permission_mode=permission_mode, node=_spawn_node())
    return captured["cmd"]
...
def test_long_configured_node_id_and_slug_still_spawn_one_valid_worker(monkeypatch):
    ...
    sid = adv._spawn_worker(
        node_id, "/w", slug, source="ab", verb="/blueprint", node=_spawn_node(difficulty="medium")
    )
...
def test_cutover_account_rides_argv_not_the_wrapper_env(monkeypatch):
    ...
    adv._spawn_worker(
        ...
        provider="codex",
        harness="codex",
        dispatch_account="zai-cutover-1", node=_spawn_node(),
    )
...
def test_spawn_worker_refuses_a_state_root_key_in_extra_env(monkeypatch):
    ...
    with pytest.raises(adv.SpawnError) as exc:
        adv._spawn_worker(
            "ab-2222aaaa", "/w", extra_env={"HOME": "/accounts/zai-1/home"}, node=_spawn_node()
        )
...
def test_spawn_worker_still_accepts_a_non_state_root_extra_env(monkeypatch):
    ...
    monkeypatch.setattr(adv.subprocess, "run", fake_run)
    adv._spawn_worker(
        "ab-2222aaaa", "/w", extra_env={"ANTHROPIC_BASE_URL": "https://x"}, node=_spawn_node()
    )
...
def test_spawn_worker_harvests_seam_receipt_from_stderr(monkeypatch):
    ...
    sid = adv._spawn_worker("ab-2222aaaa", "/w", receipt=receipt, node=_spawn_node())
...
def test_spawn_worker_note_harvest_caps_at_twenty(monkeypatch):
    ...
    adv._spawn_worker("ab-2222aaaa", "/w", receipt=receipt, node=_spawn_node())
