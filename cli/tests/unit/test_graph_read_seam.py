...
represent is skipped at import and stays in the seed file only.
...
def test_load_graph_applies_the_status_value_rename(graph: Path) -> None:
    ...
    spelling does not: it is not a modeled field, so that row imports with the
    default status and its claimed value is gone.
    """
    ...
    assert _by_id(load_graph(graph))["x-0001"]["status"] == "idea"
...
def test_unhashable_field_values_do_not_crash_the_readers(tmp_path: Path) -> None:
    ...
    never-fatal. No reader raises now; the store side additionally SKIPS the
    unrepresentable row at import while the scoreboard reader still answers it,
    values untouched, because a display signal must not editorialize.
    """
    p = _write(tmp_path, [
    ...
    for reader in (read_graph_strict, load_graph):
        ...
        assert set(rows) == {"x-0007"}, f"{reader.__name__} kept a row it cannot represent"
