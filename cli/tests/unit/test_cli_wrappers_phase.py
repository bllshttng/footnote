...
Note: fno do phase verify tests removed in Task 3.2 (control-plane collapse,
ab-d0337fbc) -- phase-verifier.sh deleted, fno do phase verify subcommand removed.
"""
...
def test_kill_check_help_renders():
    result = runner.invoke(app, ["do", "phase", "kill-check", "--help"])
...
def test_kill_check_missing_binary_yields_exit_2(monkeypatch):
    ...
    result = runner.invoke(app, ["do", "phase", "kill-check", "/some/plan"])
...
def test_kill_check_invokes_binary_with_plan_path(monkeypatch):
    ...
    result = runner.invoke(app, ["do", "phase", "kill-check", "/some/plan/path"])
