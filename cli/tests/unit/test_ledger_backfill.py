...
# Synthetic node ids (not real backlog nodes) that match the id-shaped token
# regex: an 8-hex legacy form and two 4-hex forms.
NODES = {"ab-11112222", "x-3344", "x-5566"}


def test_resolve_from_title():
    mod = _load()
    row = {"title": "Screen manifest fallback (ab-11112222)", "branch": "main"}
    assert mod.resolve_row(row, NODES) == ("title", "ab-11112222")


def test_resolve_from_branch_when_title_barren():
    mod = _load()
    row = {"title": "no id here", "branch": "feature/x-3344-manifest"}
    assert mod.resolve_row(row, NODES) == ("branch", "x-3344")


def test_ambiguous_title_is_unrecoverable_not_guessed():
    # AC1-EDGE: two distinct existing nodes in one field -> never guess.
    mod = _load()
    row = {"title": "merge of ab-11112222 and x-3344", "branch": "main"}
...
def test_token_absent_from_graph_is_unrecoverable():
    ...
    mod = _load()
    row = {"title": "ab-deadbeef ghost", "branch": "worktree-something"}
    assert mod.resolve_row(row, NODES) == ("unrecoverable", None)
...
def test_backfill_writes_and_is_idempotent(tmp_path):
    ...
    ledger.write_text(json.dumps({"entries": [
        {"title": "ab-11112222 ship", "branch": "main", "pr_number": 1},
        {"title": "no id", "branch": "worktree-x", "pr_url": "http://x/pull/2"},
        {"title": "ab-11112222 non-ship (no pr)", "branch": "main"},
    ]}))

    assert mod.backfill(ledger, NODES, apply=True) == 0
    rows = json.loads(ledger.read_text())["entries"]
    assert rows[0]["graph_node_id"] == "ab-11112222"  # resolved
