...
def test_plan_path_binding_only_binds_the_artifact(tmp_graph, monkeypatch, tmp_path):
    """Plan binding does not claim that the Blueprint phase completed."""
    _set_ambient_claude(monkeypatch)
    _seed(tmp_graph, [{"id": "ab-00000001", "title": "t", "domain": "code", "project": "p"}])
    ...
    assert result.exit_code == 0, result.output
    assert _node(tmp_graph, "ab-00000001")["plan_path"] == str(plan)
    assert _node(tmp_graph, "ab-00000001").get("sessions", []) == []


def test_plan_path_binding_does_not_need_identity(tmp_graph, monkeypatch, tmp_path):
    _clear_ambient(monkeypatch)
    _seed(tmp_graph, [{"id": "ab-00000001", "title": "t", "domain": "code", "project": "p"}])
    plan = tmp_path / "plan.md"
    plan.write_text("---\nstatus: ready\n---\n# plan\n")
...
def test_plan_path_rebind_does_not_write_blueprint_provenance(tmp_graph, monkeypatch, tmp_path):
    _set_ambient_claude(monkeypatch)
    _seed(tmp_graph, [{
        "id": "ab-00000001", "title": "t", "domain": "code", "project": "p",
    ...
    # plan_path is already set; rebinding remains artifact-only.
    result = runner.invoke(app, [
        "backlog", "update", "ab-00000001", "--plan-path", str(plan),
    ])
