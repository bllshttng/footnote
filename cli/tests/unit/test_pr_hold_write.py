...
def _set(graph, **overrides):
    ...
    return _hold.hold_set(**args)


def _release(graph, **overrides):
    args = dict(node="t-0001", evidence="condition held", graph_path=graph, notify=False)
    args.update(overrides)
    return _hold.hold_release(**args)
...
def test_set_writes_a_block_the_merge_gate_reads(node_plan):
    ...
    assert receipt["notified"] == "skipped"
    assert receipt["hold"]["reason"] == "condition R"
    assert dispatch_hold({"plan_path": str(plan), "cwd": str(plan.parent)}).state is DispatchHoldState.HELD
...
def test_set_refusals_write_nothing(node_plan):
    ...
    with pytest.raises(_hold.HoldWriteError) as err:
        _set(graph, node="t-nope")
    assert err.value.exit_code == 2
    with pytest.raises(_hold.HoldWriteError) as err:
        _set(graph, reason="  ")
    assert err.value.exit_code == 2
    with pytest.raises(_hold.HoldWriteError) as err:
        _set(graph, review_on="2026-13-99")
    assert err.value.exit_code == 2
...
def test_release_lifts_and_restores_the_original_bytes(node_plan):
    ...
    assert dispatch_hold({"plan_path": str(plan), "cwd": str(plan.parent)}).state is DispatchHoldState.ABSENT
    assert plan.read_text(encoding="utf-8") == PLAN
...
def test_release_names_an_ancestor_that_still_holds(tmp_path):
    ...
    _set(graph, node="t-parent")
    _set(graph, node="t-0001")
    receipt = _hold.hold_release("t-0001", "condition held", graph_path=str(graph), notify=False)
...
def test_cli_hold_set_and_release_round_trip(node_plan, monkeypatch):
    ...
    plan, graph = node_plan
    monkeypatch.setattr(_hold, "_notify_worker", lambda node_id, text: "skipped")
    ...
    result = runner.invoke(
        app,
        ["do", "pr", "hold", "release", "t-0001", "--evidence", "proved"],
    )
    assert result.exit_code == 0, result.output
    assert plan.read_text(encoding="utf-8") == PLAN
