...
from fno.graph.ladder import DispatchHold, DispatchHoldState, dispatch_hold
...
def test_set_writes_a_block_the_merge_gate_reads(node_plan):
    ...
    # The splice is byte-stable: every other frontmatter key and the body
    # survive unchanged.
    fields, _block, rest = parse_frontmatter(plan.read_text(encoding="utf-8"))
    assert fields["claims"] == "t-0001"
    assert fields["priority"] == "p1"
    assert "dispatch_hold" in fields
    assert rest == "\n# A plan\n\nBody.\n"
...
def test_set_refusals_write_nothing(node_plan):
    ...
    with pytest.raises(_hold.HoldWriteError) as err:
        _hold.hold_write("set", "t-nope", reason="r", release_when="w", set_by="s", graph_path=graph)
...
def test_a_failed_readback_restores_the_plan(node_plan, monkeypatch):
    plan, graph = node_plan
    # A reader that never answers HELD simulates a write whose proof missed.
    monkeypatch.setattr(
        "fno.graph.ladder.dispatch_hold",
        lambda entry: DispatchHold(DispatchHoldState.ABSENT),
    )
    with pytest.raises(_hold.HoldWriteError) as err:
        _set(graph)
...
def test_release_names_nothing_when_no_ancestor_holds(node_plan):
    plan, graph = node_plan
    _set(graph)
    receipt = _release(graph)
    assert receipt["hold"] == {"evidence": "condition held"}
