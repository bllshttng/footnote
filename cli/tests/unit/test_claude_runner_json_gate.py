...
class _FakeAdapter:
    """Returns a canned spawn_worker result dict (stdout-only)."""

    def __init__(self, stdout: str) -> None:
        self._stdout = stdout

    def spawn_worker(self, prompt: str) -> dict:
        return {"stdout": self._stdout, "pid": 4242}
...
def test_off_path_uses_legacy_finding_parser() -> None:
    ...
    outcome = run_via_claude_code(
        "code_reviewer", "p", "d", adapter=adapter, json_findings=False
    )
...
def test_on_path_parses_json_and_attributes_claude() -> None:
    ...
    outcome = run_via_claude_code(
        "code_reviewer", "p", "d", adapter=adapter, json_findings=True
    )
    assert outcome.ok is True
