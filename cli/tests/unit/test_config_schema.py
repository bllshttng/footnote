...
def test_logs_convo_signals_max_mb_default(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Default convo_signals_max_mb is 5 when config.logs is absent."""
    monkeypatch.delenv("FNO_CONFIG", raising=False)
    settings_file = _write_settings(tmp_path, "schema_version: 1\n")
    monkeypatch.setenv("FNO_CONFIG", str(settings_file))
    ...
    settings = config_mod.load_settings()
    assert settings.config.logs.convo_signals_max_mb == 5


def test_logs_convo_signals_max_mb_override(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """config.logs.convo_signals_max_mb is read from settings.yaml."""
    monkeypatch.delenv("FNO_CONFIG", raising=False)
    settings_file = _write_settings(
        tmp_path,
        "schema_version: 1\nconfig:\n  logs:\n    convo_signals_max_mb: 12\n",
    )
    monkeypatch.setenv("FNO_CONFIG", str(settings_file))
    ...
    settings = config_mod.load_settings()
    assert settings.config.logs.convo_signals_max_mb == 12


def test_logs_convo_signals_max_mb_rejects_non_positive(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A convo_signals_max_mb < 1 is rejected at load time (cap must be >= 1)."""
    monkeypatch.delenv("FNO_CONFIG", raising=False)
    settings_file = _write_settings(
        tmp_path,
        "schema_version: 1\nconfig:\n  logs:\n    convo_signals_max_mb: 0\n",
    )
    monkeypatch.setenv("FNO_CONFIG", str(settings_file))
    ...
    with pytest.raises(Exception, match=r"convo_signals_max_mb|>= ?1"):
        config_mod.load_settings()


def test_logs_block_malformed_degrades_to_default(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A non-mapping config.logs: degrades to defaults, not a load crash."""
    monkeypatch.delenv("FNO_CONFIG", raising=False)
    settings_file = _write_settings(
        tmp_path, "schema_version: 1\nconfig:\n  logs: 42\n"
    )
