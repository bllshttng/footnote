...
No fno config found yet. Run `fno config setup wizard` (terminal) or `/fno:setup` (in a Claude Code session) to configure. For Codex/Gemini SessionStart context, the wizard offers hook wiring or you can run `fno config setup cli-hooks` directly. Optional - defaults work, so `/fno:target "..."` runs without it.
EOF
