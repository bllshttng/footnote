...
The loop, goal and monitor survive a compact: verify with \`/hooks\` and the loop receipt, and re-arm any that is missing. The one monitor: the fleet settled-PR wake (600s) - a quiet or parked roster row whose node's PR reads settled gets poked with \`fno agents resume <id>\` (\`fno agents list --json\` + \`fno do pr status <n>\`); mail, board, crown liveness, main CI, capacity and \`fno agents king verdict\` are demand reads, not beats. The two self-injected commands: \`/loop <king.checkin_interval> <king.checkin_text>\` and \`/goal <king.goal_text>\`. The check-in beat is one verb: \`fno agents king checkin\` gathers the readings, prints them, diffs the last beat, and journals \`reign_checkin\` itself; then act on the printout. Levers in order: mail the stalled worker, \`fno backlog encounter <id> --evidence\` and \`fno backlog update <id> --priority\` (rank is the operator's pin and refuses you), undefer or supersede, ask the operator. Dispatch only on a red dispatching arm, journaled as \`reign_dispatch_exception\`. Never \`/goal clear\` on NoProgress - escalate-and-park is the stop path."
    fi
fi
