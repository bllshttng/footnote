...
if [[ "$STATE_FILE" == "$DELIVERY_PENDING_STATE" ]]; then
    ...
    # One settle sweep before the checker: a lost review invocation becomes a
    # `lost` attestation row, so the gate's next read answers a named refusal
    # instead of waiting on silence. Best-effort and quiet - a missing fno or
    # a failed sweep must never hold the stop gate, and the doctor's report
