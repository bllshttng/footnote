...
def _contains_enact_action(command: Any) -> bool:
    ...
    try:
        ...
        return bool(
            re.match(r"^\s*fno\s+(inbox\s+)?law\s+enact\b", command)
        )
    if segment:
        segments.append(segment)

    for tokens in segments:
        if tokens[:4] == ["fno", "inbox", "law", "enact"] or tokens[:3] == ["fno", "law", "enact"]:
            return True
...
def _parse_command(command: Any) -> tuple[str, str] | None:
    ...
    try:
        ...
        return None
    # Both spellings for the one release the `fno law` shim lives: a consent
    # binds the command as typed, so the gate must arm either form.
    if len(tokens) == 8 and tokens[0:5] == ["fno", "inbox", "law", "enact", "--proposal"]:
        if tokens[6] != "--hash":
            return None
        proposal_id, content_hash = tokens[5], tokens[7]
    elif len(tokens) == 7 and tokens[0:4] == ["fno", "law", "enact", "--proposal"]:
        if tokens[5] != "--hash":
            return None
