...
cat >&2 <<EOF
Native review returned $total finding(s) for this turn (P1: $p1, P2: $p2). Act on the findings in your current context now. If no PR exists, read 'fno do pr review-hold round --branch <branch> --head <sha>' and compare it with 'fno config get review.max_rounds'. If the read fails, request no review and emit no attestation. If the cap is spent, request no review or attestation. Push with 'fno do pr push --no-preflight', then open the PR through the normal ship flow and merge on green CI. If below the cap, fix actionable findings, run focused tests, commit once, then run 'fno do target request-self-review' with no '--pr'. If a PR exists, read 'fno do pr status <n>'. When it reads rounds_exhausted null, run 'fno-agents review-coverage --cwd . --pr <n>' and read status again. When it reads rounds_exhausted false, fix actionable findings, run focused tests, commit once, push with 'fno do pr push --no-preflight', then run 'fno do target request-self-review --pr <n>' for this PR. When it reads rounds_exhausted true, the review phase is complete. Request no review or attestation. Decline what you leave unfixed with a recorded reason. If you committed a fix, push it with 'fno do pr push --no-preflight'. Merge on green CI. Below the cap, do not promise completion while findings remain.
EOF
exit 2
