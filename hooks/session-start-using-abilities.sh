#!/usr/bin/env bash
# SessionStart hook: inject the using-abilities SKILL.md as additionalContext.
#
# This is the abilities equivalent of the superpowers session-start hook
# (https://github.com/obra/superpowers/blob/main/hooks/session-start).
# It ensures every Claude session opened in an abilities-enabled project
...
SKILL_PATH="${PLUGIN_ROOT}/skills/using-abilities/SKILL.md"
if [[ ! -r "${SKILL_PATH}" ]]; then
    echo "session-start-using-abilities: SKILL.md missing or unreadable at ${SKILL_PATH}" >&2
...
SKILL_BODY="$(cat "${SKILL_PATH}" 2>/dev/null)" || {
    echo "session-start-using-abilities: could not read ${SKILL_PATH}" >&2
    echo '{}'
    exit 0
}
...
CONTEXT="<EXTREMELY_IMPORTANT>\nYou are in an abilities-enabled project.\n\n**Below is the full content of the 'using-abilities' skill - your introduction to the two surfaces (slash-command workflows and the fno CLI). Re-read it when in doubt about which surface to use:**\n\n${SKILL_ESCAPED}\n</EXTREMELY_IMPORTANT>"
