...
///): 1 is the first tab in display order and 0 is always refused. It
...
/// (v44) sets `Refuse` so exact origin placement never silently lands
...
/// Server-authored exact-placement receipt (v44). Carries the
...
pub struct ResolvedPlacement {
    ...
    pub squad: u64,
    pub tab: TabId,
    /// (v51) The landed tab's name and 1-based ordinal, so the
...
pub struct PanePlacement {
    ...
    /// (v31) Open-here: repoint the sender's focused pane at the target session rather than minting a
    ...
    #[serde(default)]
    pub at: Option<u64>,
    /// (v44) Strict-placement policy. Default `NewTab` keeps the legacy
    ...
    /// DEPRECATED by `portal` below, kept as the compatibility
    ...
    /// (v64) Which portal this placement targets. `Some(n)` reaches
    ...
    /// (v64) Open in the NEXT FREE portal, letting the SERVER pick the
    ...
    #[serde(default)]
    pub portal_new: bool,
    /// (v80) Server-chosen tab: the server picks the first tab in
...
impl PanePlacement {
    /// The portal this placement targets, folding the deprecated
    ...
    /// Does this placement ask for a portal at all, by any of the
