...
mod liveness;
use self::liveness::derive_liveness;
use crate::served_liveness::served_liveness;
// The capability table lives once, in fno-agents; this file's TOML is a
// build-generated byte copy of it (fno-agents' build.rs writes it, the same
// way it writes the Python-tree copy the package loads). The crates stay
// copy-fed rather than dep-fed on purpose: they publish to crates.io
// independently, so a dep would tie this crate's publishability to a
// registry state that lags the repo.
...
// Default exists for test fixtures: a literal over twenty Option fields
// made every new field touch every helper in every consumer's test mod.
    ...
    for path in config_toml_candidates() {
        ...
        for (name, over) in harnesses {
            ...
            if rows.contains_key(name) {
                // A bundled row corrects per field. The gate here is the
                // bundled vocabulary: every key the override carries must
                // already exist on the bundled row (or be a shallow lane
                // key), so a typo isolates to its own harness - unknown
                // keys are refused, the bundled row kept, the refusal named
                // (AC1-ERR). The SEMANTIC contract gate is the Python
                // reader's (dispatch refuses on load); the crates publish
                // independently, so this reader cannot call the fno-agents
                // validator, and the resolved-row parity test pins the two
                // readers' agreement on the accepted class.
                let mut candidate = rows.get(name).cloned().unwrap();
                if let Err(error) = override_keys_are_bundled_vocabulary(&normalized, &candidate) {
                    if let Ok(mut w) = warnings_slot().lock() {
                        w.push(format!(
...
fn override_keys_are_bundled_vocabulary(
    ...
    const SHALLOW_LANE_KEYS: [&str; 2] = ["attach", "resume"];
    let bundled_keys: std::collections::HashSet<&str> = bundled
        .as_table()
    ...
    let unknown: Vec<&str> = normalized
        .keys()
        .map(String::as_str)
        .filter(|key| !bundled_keys.contains(key) && !SHALLOW_LANE_KEYS.contains(key))
        .collect();
    if unknown.is_empty() {
        Ok(())
    } else {
...
pub(crate) fn isolated_account_dirs() -> Vec<(String, PathBuf)> {
...
tokens = []"#,
    ...
    /// Python reader answers the same stage for the accepted class; the
    /// cross-check lives in cli/tests/agents/test_harness_capability_parity.py.
    #[test]
...
[harness.claude]
ready_marker_typo = "live_prompt_box"
"#,
        )
        .unwrap();
...
"#,
        ...
        // CONFLICTING candidates; gemini was not overridden project-locally).
        assert_eq!(rows["gemini"]["route_on_pane"].as_bool(), Some(true));

        // The unknown roster name is refused, never advertised.
        assert!(!rows.contains_key("brandnewharness"));

        // An unknown key is outside the bundled vocabulary: the override is
        // refused, so claude keeps its bundled row whole.
        assert!(rows["claude"].get("ready_marker_typo").is_none());
        assert_eq!(
            rows["claude"].get("resume").and_then(|v| v.as_str()),
            Some("native-session")
        );

        // A nested table over bundled keys merges per field: the override's
        // kind lands, and the bundled status pair survives the deep merge.
        // The Python reader's semantic validator may refuse this same stage;
        // the mux reads lane forms from these rows, so the divergence is
        // inert here, and the parity test pins the accepted class.
        assert_eq!(
            rows["codex"]["model_switch_strategy"]["kind"].as_str(),
            Some("direct")
        );
        assert_eq!(
            rows["codex"]["model_switch_strategy"]["status_command"].as_str(),
            Some("/status"),
            "the deep merge keeps bundled keys the override never named"
        );
        let warnings = override_warnings();
        assert!(
            warnings.iter().any(|w| w.contains("claude")),
            "the unknown-key rejection is named: {warnings:?}"
        );
        assert!(
