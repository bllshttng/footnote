//! The persisted slot family : what one pane of a stored layout
//! binds to. Extracted from `proto` under the file-budget gate; re-exported
//! there, so every `crate::proto::` path keeps working.

use serde::{Deserialize, Serialize};

/// What a layout slot binds to (v44). Exactly one slot binds `Anchor`
...
/// A persisted portal seat: the portal index and the row key it
...
pub struct LayoutSlot {
    ...
    /// (v68) pane's cwd at capture; `None` pre-v68.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub cwd: Option<String>,
    /// When this slot is a portal seat: the portal index and the row
