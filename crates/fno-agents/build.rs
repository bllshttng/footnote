...
/// PRODUCE the fno crate's event-store copy from this crate's implementation.
///
/// The two crates publish independently, so fno must compile a checked-in copy
/// instead of depending on fno-agents. The generated-copies CI step checks the
/// copy against this owner after every build.
fn sync_event_store() {
    println!("cargo:rerun-if-changed=src/event_store.rs");
    let canonical = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/event_store.rs");
    ...
    let body = text
        .strip_suffix("\n#[cfg(test)]\nmod tests;\n")
        .expect("src/event_store.rs must end with its test module declaration; update sync_event_store");
