...
/// PRODUCE the in-crate copy of the capability table from the canonical.
///
/// x-244c (operator ruling 2026-09-02): the Python-tree table
/// (`cli/src/fno/agents/harness_capabilities.toml`) is the ONE file a human
/// edits; this crate's `src/harness_capabilities.toml` is a generated byte
/// copy that `harness_capabilities.rs` `include_str!`s. Generation replaces
/// the old byte-parity gate: an edit to the canonical lands here on the next
/// build, and a hand edit of the copy is silently discarded rather than
/// flagged. The copy stays tracked because the crates.io tarball must compile
/// standalone, where no `cli/` exists (the events_limits.toml precedent).
///
/// No-op when `cli/` is absent: that is the `cargo package` / crates.io tarball
/// case, where the committed copy is the value (`publish = true`).
