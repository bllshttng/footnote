...
| `events.rs` | Operator-facing `events.jsonl` emitter; `O_APPEND` open-write-close; schema cap (`limits.max_data_bytes`, 65536) -> `event_payload_too_large` meta-event (never silent); rotation ingests the store first. |
| `events_store.rs` | One SQLite store (`events.db`) beside each durable journal; ingests the rotated generation and then the live file (inode cursor + line-hash dedupe), WAL + busy-timeout, 30-day horizon, `(scope, type, ts_ms)` index for history reads. |
