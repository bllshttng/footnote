    ...
    match state.backend() {
        Backend::Json => {
            ...
            let Some(pre_ident) = pre_ident else {
                // No identity, no trust: a stat failure invalidates and the
                // read answers fresh, exactly as an unreadable stat read
                // today.
            ...
            // Digest first, parse second: with the gate held, a keeper-side
            // write cannot interleave, and a FOREIGN one (gc_sweep on the
            // file) moves the post-parse identity, which the fill check
            // below refuses.
            let version = graph_store::file_content_version(&state.graph);
            ...
            Ok(GraphRead::Fresh(entries, version))
        }
        Backend::Sqlite => {
...
pub(crate) fn handle_request(state: &StoreState, payload: &[u8]) -> Value {
    ...
    let result = match method {
        ...
        "plan_docs" => handle_plan_docs(state, &params),
...
/// The `plan_docs` method: `{op: "project"|"stamp"|"graduate"|"set_expected"|
/// "waves", ...}`. `project` reads the graph this keeper owns and rewrites
/// each named node's linked plan doc; `stamp`/`graduate`/`set_expected`
/// write plan frontmatter with the same exit codes the Python module
/// returned; `waves` derives an epic's wave strata. Plan-doc events ride the
/// keeper's own `--events` journal when one is configured.
fn handle_plan_docs(state: &StoreState, params: &Value) -> Result<Value, StoreError> {
    use crate::plan_doc;
    ...
    match op {
        "project" => {
            ...
            let (rewritten, warnings) = plan_doc::project::project_graph_nodes(
                &cached,
                &ids,
                root.as_deref(),
        ...
        "stamp" => {
            ...
            let urls: Vec<String> = params
                ...
                .unwrap_or_default();
            let result = plan_doc::stamp::cmd_stamp(
                Path::new(plan_path),
                session_id,
                &urls,
                params
        ...
        "graduate" => {
            ...
            let result = plan_doc::stamp::cmd_graduate(
                Path::new(plan_path),
                params
                    .get("dry_run")
                    .and_then(Value::as_bool)
        ...
        "set_expected" => {
            ...
            let result = plan_doc::stamp::cmd_set_expected(
                Path::new(plan_path),
                count,
                params
                    .get("dry_run")
        ...
        "waves" => {
            ...
            let cached = cached_entries(state, false, false)?;
            let (wave_by_id, max_wave) = plan_doc::rollup::compute_waves(epic_id, &cached);
            let wave_by_id: serde_json::Map<String, Value> = wave_by_id
                .into_iter()
