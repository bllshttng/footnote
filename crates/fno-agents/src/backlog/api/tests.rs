...
struct ClaimsFixture {
    ...
    record: crate::claims::ClaimRecord,
}
...
impl ClaimsFixture {
    fn new() -> Self {
        ...
        let record = match crate::claims::acquire(
            "node:ab-one",
            "target-session:s-1",
...
fn both_stores() -> (ClaimsFixture, TempDir, TempDir, Store, Store) {
    ...
    (claims, json_dir, sqlite_dir, json_store, sqlite_store)
}
...
fn api_node_finds_one_row() {
    ...
    let expected_locked_at =
        chrono::DateTime::<chrono::Utc>::from_timestamp_millis(_claims.record.acquired_at)
            .unwrap()
    ...
    for store in [&json_store, &sqlite_store] {
        ...
        assert_eq!(one.claim.harness.as_deref(), Some("claude"));
        assert_eq!(one.claim.harness_session.as_deref(), Some("s-1"));
        assert_eq!(
            one.claim.locked_at.as_deref(),
            Some(expected_locked_at.as_str())
        );
...
#[test]
fn api_node_fails_closed_when_claims_path_is_unreadable() {
    let claims = ClaimsFixture::new();
    let claims_dir = claims._root.path().join(".fno").join("claims");
    std::fs::remove_dir_all(&claims_dir).unwrap();
    std::fs::write(&claims_dir, b"not a directory").unwrap();
...
fn api_node_update_status_moves_through_the_patch_door() {
    ...
    let (_claims, _d1, _d2, json_store, _sqlite_store) = both_stores();
