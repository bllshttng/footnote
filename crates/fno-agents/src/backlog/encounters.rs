...
fn build(parts: RowParts) -> Encounter {
    let (
        ts,
        evidence,
        session_id,
...
/// Every node's encounters, grouped by node id, in list order within each
/// node. One full scan instead of one query per node.
pub(crate) fn load_all(
    connection: &Connection,
) -> Result<std::collections::HashMap<String, Vec<Encounter>>, String> {
    let mut statement = connection
        .prepare_cached(
            "SELECT ts, evidence, session_id, voter_key, voter_kind, harness, fno_id, effort,
                    model, extras, node_id
             FROM encounters ORDER BY node_id, seq",
        )
        .map_err(|error| error.to_string())?;
    let rows = statement
        .query_map([], |row| Ok((row.get::<_, String>(10)?, map_row(row)?)))
        .map_err(|error| error.to_string())?;
    let mut out: std::collections::HashMap<String, Vec<Encounter>> =
