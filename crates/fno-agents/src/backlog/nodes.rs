...
/// store at read time, and ensure_table drops a v4 store's mirror on open.
const TABLES: [&str; 5] = [
    "nodes",
    "nodes_raw",
    "node_dispatch",
...
/// Schema 4. nodes keeps a wire `created_at` and gains only updated_at; the
/// other tables get both.
pub fn ddl() -> String {
    let nodes_iso = [
        "created_at",
        "touched_at",
