...
// ---------------------------------------------------------------------------
// capabilities / target-family (x-3873 change 2)
// ---------------------------------------------------------------------------
...
/// to `fno agents dispatch capabilities`, which the dispatch-verb retirement
/// removed (x-3873): the JSON shape (map_version, harness, then the harness's
/// table) matches what the Python leaf printed. An unknown harness exits 2
/// naming the harness and the declared list, with nothing on stdout.
...
pub fn run_target_family(args: &[String]) -> i32 {
    ...
    let verdict = if crate::merge_posture::is_target_family(&message) {
        "family"
    } else {
        "other"
    };
...
mod capability_leaves {
    ...
    fn target_family_answers_family_then_other_and_always_exits_zero() {
        ...
        assert_eq!(other, 0);
        assert!(crate::merge_posture::is_target_family("/fno:target x-1"));
        assert!(!crate::merge_posture::is_target_family("hello"));
    }
