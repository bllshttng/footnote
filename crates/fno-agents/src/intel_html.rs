...
pub(crate) fn render_report(md_path: &Path) -> Result<RenderOutcome, String> {
    ...
    let page = template()
        .replacen("__TITLE__", &html_escape_quote(&title), 1)
        .replacen("__META__", &meta, 1)
        .replacen("__BODY__", &body, 1)
        .replacen("__FOOTER__", &html_escape_quote(&footer), 1);
...
fn category_bars(fold: &Value, notes: &mut Vec<String>) -> Option<String> {
    ...
    for item in items {
        ...
        let share = item.get(KEY_SHARE).and_then(Value::as_f64).unwrap_or(0.0);
