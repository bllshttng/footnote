...
pub(crate) fn read_pr_gates(
    ...
    match crate::claim_store::list_db(Some("merge-slot:"), false, None) {
        Ok(rows_json) => {
            let holders: Vec<u64> = rows_json
                .as_array()
                .map(|list| {
                    list.iter()
                        .filter_map(|r| r.get("holder").and_then(Value::as_str))
                        .filter_map(crate::authorized_merge::parse_slot_holder)
                        .collect()
                })
                .unwrap_or_default();
            for holder in &holders {
                ...
                let named = rows.iter().any(|row| {
                    row.get("number").and_then(Value::as_u64) == Some(*holder)
                });
                if !named {
                    rows.push(json!({
                        "number": holder,
                    ...
                    warnings.push(format!(
                        "mergeable_pr: merge slot holder PR {holder} has no gate row in this                          read; carried as a slot row so the queue names what orders it"
                    ));
                }
            }
        }
