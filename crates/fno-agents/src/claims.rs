...
mod tests {
    ...
    // ---- an expired reservation resolves through its pid (x-41f7) ---------

    fn reservation(pid: i32, expires_at: Option<i64>, host: &str) -> ClaimRecord {
        let mut rec = record(pid, now_ms(), expires_at, host);
        rec.key = "dispatch:x-t".into();
        rec.holder = format!("spawn-cli:{pid}");
        rec.session_id = Some("s-dispatcher".into());
    ...
    fn expired_reservation_resolves_through_the_pid_it_names() {
        ...
        let (provably_dead, _) =
            classify_for_sweep(&dead, Some(now), &probe_pid, None, Some(witness));
    ...
    fn offhost_or_pid_unavailable_reservation_keeps_todayss_verdict() {
        ...
        let mut offhost = reservation(-1, Some(now - 1), "elsewhere.example");
        offhost.session_id = Some("s-dispatcher".into());
        let (state, cause) = classify_with_basis_and_exclusivity(
            &offhost,
            Some(now),
            &probe_pid,
