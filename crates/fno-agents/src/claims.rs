...
mod tests {
    ...
    fn renew_extends_an_expired_bg_job_claim_whose_witness_says_live() {
        ...
        // keyed by the bg-job session id. Pre-x-b445 this refused (expired =>
        // Ok(false)) and the watching tag was rejected 32 times with the
        // transient "could not be renewed" text. This pins the bg-job shape:
        // expired + a LIVE verdict through a session id => renew extends and
        // re-anchors to the row pid.
        let _guard = test_env_lock().lock().unwrap_or_else(|e| e.into_inner());
        let td = TempDir::new().unwrap();
        let home = td.path().join("agents-home");
        std::fs::create_dir_all(&home).unwrap();
        ...
        o.ttl_ms = Some(60_000);
        o.pid = Some(dead_pid());
        let _ = acquire("node:x-3227-bglease", "target-session:me", o);
        let mut rec = read_claim(&td, "node:x-3227-bglease");
        rec.session_id = Some("t-3227-bgjob-session".into());
        rec.expires_at = Some(now_ms() - 1);
        atomic_replace(
            &lockfile(&td, "node:x-3227-bglease"),
            &serialize_claim(&rec).unwrap(),
        )
        .unwrap();
        let (state, basis) =
            crate::claim_verbs::status_verdict(&read_claim(&td, "node:x-3227-bglease"));
        assert_eq!(
            state,
            ClaimState::Live,
            "fixture must read LIVE through the bg-job witness or AC proves nothing"
        );
        assert_eq!(basis, basis::REGISTRY_SESSION_LIVE, "{basis}");
        ...
        let after = read_claim(&td, "node:x-3227-bglease");
        assert_eq!(
            after.pid,
            Some(live as i32),
            "the anchor must MOVE to the live bg-job row pid"
        );
        assert_eq!(classify(&after, None), ClaimState::Live);
    }
