...
/// (x-7b5e) The `resume-argv` reply for one harness: the declared
/// `interactive_resume` form as tokens, or the specific reason the table
/// declares none. Pure over the contract so every branch is assertable
/// without a process. The `{session_id}` placeholder travels INTACT - the
/// caller fills it - so a form naming any other placeholder is an honest
/// `available: false` here rather than a rendered guess.
fn resume_argv_reply(contract: &HarnessContract, harness: &str) -> serde_json::Value {
    let unusable = |reason: String| {
        serde_json::json!({ "harness": harness, "available": false, "reason": reason })
    };
    let caps = match contract.capabilities(harness) {
        Ok(caps) => caps,
        Err(e) => return unusable(e.to_string()),
    };
    let Some(form) = caps.resume_strategy.forms.get("interactive_resume") else {
        return unusable(format!("{harness} declares no interactive_resume form"));
    };
    if form.kind == "unsupported" {
        return unusable(format!("{harness} declares interactive_resume unsupported"));
    }
    if form.tokens.is_empty() {
        return unusable(format!("{harness} declares an empty interactive_resume"));
    }
    for token in &form.tokens {
        let placeholder = token.starts_with('{') && token.ends_with('}');
        if placeholder && token != "{session_id}" {
            return unusable(format!(
                "{harness} resume form names {token}; only {{session_id}} can be filled"
            ));
        }
    }
    serde_json::json!({ "harness": harness, "available": true, "argv": form.tokens })
}

/// (x-7b5e) `fno-agents resume-argv <harness>`: print one harness's declared
/// `interactive_resume` form as a one-line JSON object on stdout. The mux
/// server shells this once per harness and fills the session id itself, so a
/// harness the table declares is resumable with no Rust change (AC3-HP) and
/// one declaring no usable form is named, never silently skipped (AC5-ERR).
///
/// Exit 0 with `available: false` is an ANSWER about the table, not a
/// failure; exit 2 is a usage error or an unreadable packaged contract.
pub fn run_resume_argv(args: &[String]) -> i32 {
    if args.len() != 1 || args[0].starts_with('-') {
        eprintln!("usage: fno-agents resume-argv <harness>");
        return 2;
    }
    let contract = match HarnessContract::packaged() {
        Ok(c) => c,
        Err(e) => {
            eprintln!("fno-agents: resume-argv: {e}");
            return 2;
        }
    };
    println!("{}", resume_argv_reply(&contract, &args[0]));
    0
}
...
mod tests {
    ...
    /// (x-7b5e) Every harness the table declares gets an `available: true`
    /// reply carrying exactly its own declared tokens. Read from the SAME
    /// contract object the reply fn reads, so this asserts the plumbing, and
    /// the fno-side pinning test asserts the tokens against the TOML text.
    #[test]
    fn resume_argv_reply_is_available_for_every_declared_harness() {
        let contract = HarnessContract::packaged().unwrap();
        for name in contract.harness.keys() {
            let reply = resume_argv_reply(&contract, name);
            assert_eq!(reply["harness"], **name, "reply names its harness");
            assert_eq!(reply["available"], serde_json::json!(true), "{name}");
            let argv = reply["argv"].as_array().expect("argv array");
            assert!(!argv.is_empty(), "{name} form is non-empty");
            let declared = &contract
                ...
                .expect("declared form")
                .tokens;
            let got: Vec<String> = argv
                .iter()
                .map(|t| t.as_str().expect("string token").to_string())
                .collect();
            assert_eq!(&got, declared, "{name} reply carries the declared tokens");
            assert!(
                declared.iter().any(|t| t == "{session_id}"),
                "{name} resume form fills the session id"
            );
        }
    }

    /// (x-7b5e) The honest negative arms. An unknown harness is named, and a
    /// table whose interactive_resume went unsupported answers
    /// `available: false` with the reason instead of a rendered argv.
    #[test]
    fn resume_argv_reply_names_a_harness_with_no_usable_form() {
        let contract = HarnessContract::packaged().unwrap();
        let reply = resume_argv_reply(&contract, "iambad");
        assert_eq!(reply["available"], serde_json::json!(false));
        let reason = reply["reason"].as_str().expect("reason string");
        assert!(reason.contains("iambad"), "names the harness: {reason}");
        assert!(reply.get("argv").is_none(), "no argv on a refusal");

        // An unsupported form parses only with empty tokens (the validator
        // refuses `unsupported` carrying any), so the mutation empties both.
        let unsupported = CAPABILITY_TOML.replacen(
            "[harness.codex.resume_strategy.forms.interactive_resume]\nkind = \"subcommand\"\ntokens = [\"codex\", \"resume\", \"{session_id}\"]",
            "[harness.codex.resume_strategy.forms.interactive_resume]\nkind = \"unsupported\"\ntokens = []",
            1,
        );
        let contract = HarnessContract::parse(&unsupported).unwrap();
        let reply = resume_argv_reply(&contract, "codex");
        assert_eq!(reply["available"], serde_json::json!(false));
        let reason = reply["reason"].as_str().expect("reason string");
        assert!(reason.contains("codex"), "names the harness: {reason}");
        assert!(reason.contains("unsupported"), "{reason}");
    }

    /// (x-7b5e) A form naming a placeholder the caller cannot fill refuses
    /// instead of rendering, per AC5-ERR. The parser already refuses such a
    /// lane in TOML (a resume form must carry `{session_id}`), so this arms
    /// the in-memory check that would catch a future contract-shape change.
    #[test]
    fn resume_argv_reply_refuses_an_unfillable_placeholder() {
        let mut contract = HarnessContract::packaged().unwrap();
        contract
            .harness
            .get_mut("pi")
            .expect("pi is declared")
            .resume_strategy
            .forms
            .get_mut("interactive_resume")
            .expect("pi declares the form")
            .tokens = vec!["pi".into(), "--join".into(), "{short_id}".into()];
        let reply = resume_argv_reply(&contract, "pi");
        assert_eq!(reply["available"], serde_json::json!(false));
        let reason = reply["reason"].as_str().expect("reason string");
        assert!(reason.contains("pi"), "names the harness: {reason}");
        assert!(reason.contains("short_id"), "names the placeholder: {reason}");
    }
