...
pub(crate) fn release_redeemed_reservation(name: &str, root: Option<&Path>) {
    ...
    match crate::claim_store::force_release(&key, &format!("redeemed by spawn {name}"), root) {
        Ok(_) => eprintln!("spawn-gate: redeemed reservation {key} (reserved by {by}: {why})"),
        Err(e) => eprintln!("{NOTE} reservation {key} could not be released before acquire: {e}"),
    }
}
...
MemAvailable:    8000000 kB\n";
    ...
    /// Fixture for the reservation tests: config with zai capped at 2, one
    /// live zai registry row, one zai-tagged reservation claim held live.
    fn reservation_fixture(dir: &Path) -> (std::path::PathBuf, std::path::PathBuf) {
        let _ = std::fs::remove_dir_all(dir);
        let root = dir.join("claims-root");
        std::fs::create_dir_all(&root).unwrap();
        std::env::set_var("FNO_CLAIMS_ROOT", &root);
    ...
    fn a_reserved_name_admits_at_cap_and_releases_the_reservation() {
        ...
        assert!(!matches!(rec.as_ref(), Some(r) if r.metadata.get("reserved_by").is_some()));
        assert!(
