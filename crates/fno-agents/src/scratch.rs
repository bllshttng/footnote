...
/// The rotated sibling, then the live file: one 8 MB generation alone
/// forgets observed pairs within about a day.
fn read_journal(path: &Path) -> Vec<serde_json::Value> {
    let mut events = read_events(&crate::events::rotated_path(path));
...
/// the sweep never mutates the graph (node birth goes through
/// `fno backlog idea`, the one seam crossing).
...
mod tests {
    ...
    fn sweep_reads_observed_pairs_from_the_rotated_journal() {
        ...
        let paths = sweep_paths(&tmp);
        seed_journal(
            &crate::events::rotated_path(&paths.journal),
            &JOBS3
                .iter()
    ...
    fn report_reads_the_rotated_journal_too() {
        ...
        let journal = tmp.join("events.jsonl");
        seed_journal(
            &crate::events::rotated_path(&journal),
            &[observed_row("ci_probe", "j1", now_ts())],
        );
