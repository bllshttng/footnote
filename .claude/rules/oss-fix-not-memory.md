...
footnote is open source: users inherit the code, docs, and runtime behavior - never your private agent memory. A load-bearing fact kept only in memory is a fix you mailed to yourself. It ships to nobody.
...
2. Self-teaching runtime text - a refusal message, receipt line, or `--help` string. It cannot drift from behavior. It drifts from POLICY freely. A ruling retiring a command leaves every string still teaching it, and `scripts/ci/check-retired-command-strings.sh` closes that gap.
...
5. A filed node whose details name the concrete fix path (`fno backlog idea` - the verb, file, gate).
