...
- **DIRTY** - never auto-touched. Report only. Except setup's own symlinks into canonical: discounted and named (`reason=setup-links`).
...
- **`harness-native`** - with both keys unset, the harness's own location: claude lands at `<repo>/.claude/worktrees/<name>`. Codex Desktop uses `/worktree` or **Hand off -> Worktree**. A harness with no native transition degrades to `~/.fno/worktrees` with no `worktrees_base` inheritance. Ensure needs `--harness` and never guesses. An explicitly set `paths.worktrees_base` degrades `harness-native` to `external` (x-f96e). The key alone relocates every creation path, so "set the base but forgot the policy" is no longer a silent no-op.
- **`external`** - fno-managed at `<worktrees_base>/<repo>/<name>`.

The per-project policy outranks the global `worktree.policy` key ("conductor" is a base value, not a policy value). An explicitly set `worktrees_base` degrades a resolved `harness-native` to `external`, so the key alone relocates. A parse error or out-of-enum value REFUSES creation (fail closed): ensure exits non-zero with empty stdout, so the caller holds.
...
- `~/.cursor/worktrees/...` (cursor-agent's native worktree). fno allocates the worktree and never passes cursor-agent a native worktree flag.
- `<repo>/worktrees/` or any non-`.claude` path inside the checkout. Enforced since 2026-09-03: `setup-worktree.sh` refuses a `WORKTREE` that does not exist.
