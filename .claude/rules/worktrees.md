...
- `<repo>/worktrees/` or any non-`.claude` path inside the checkout. Enforced since 2026-09-03: `setup-worktree.sh` refuses a `WORKTREE` that does not exist, so nothing can be conjured at a bad location. Nine directories reached this one before that, built by the real script off a test mock's path.
