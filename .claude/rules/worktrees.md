...
The refusal SHAPE is load-bearing and counter-intuitive. Per the hook contract, a **non-zero exit falls back to Claude Code's default worktree flow**, so exiting non-zero creates the very worktree you meant to block. The supported abort is **exit 0 with nothing on stdout**, which CC reports as "no successful output". Pre-creation is shape-dependent, not universal: a **path-present** create (CC sends `.path`) has CC pre-create the worktree and pass its path, so the refusal reaps that directory or the stray survives; a **name-only** create (no `.path`, e.g. the EnterWorktree tool) does NOT pre-create - `test -d` was absent at hook fire in the captured payload - so there is nothing to reap and that path defers (non-zero) instead of aborting. The gate runs before the hook's own `cd`, because an absent path would fail there first and take the fallback branch. It fails open on anything but an affirmative `never`, since a stale `fno` must not break interactive `claude --worktree`. A `claude --worktree` spawned from inside an existing session is a child (`CLAUDE_CODE_CHILD_SESSION`) and does NOT fire WorktreeCreate at all - only a top-level invocation or the in-process EnterWorktree tool fires it, so probe with one of those.
