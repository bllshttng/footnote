...
- **unborn** - a branch with no commit of its own, inside the setup window, is never a merged branch whatever the merge-base says. The gate refuses it (`reason=unborn`, row `kept (unborn)`) so a fresh dispatch is not eaten mid-setup. Predicate and rationale: [worktree-mechanics](../../docs/architecture/worktree-mechanics.md).
