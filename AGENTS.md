...
- **Pane keeper** - a worker pane's pty master lives in a keeper process, not the server. A killed server is re-adopted by the next one, which binds the SAME child pid instead of respawning. Plain panes still die with the server. The shutdown sweep never kills keeper-hosted panes. `fno mux pane keeper list` reads surviving panes with the server dead. [pane-keeper](docs/architecture/pane-keeper.md).
