...
**Board order == work order (x-d1fe).** The lane key and *selection* share one rank definition. What the walker / active-backlog daemon works next comes from `fno backlog next` -> `make_selection_sort_key`, which prepends the **same** `_rank_band` term the board uses (`rank band -> epics-first -> priority -> created_at`). So `fno backlog rank <id> --top` floats a card on the board AND makes it run next, and an explicit rank overrides the epics-first heuristic. With no rank set, selection is byte-for-byte the prior epics-first -> priority -> created_at order, so `fno backlog reprioritize <id> p0` is still the lever for unranked work. The shared helper lives in `cli/src/fno/graph/_constants.py` so board and selection can never drift. See the "Board order == work order" section of [docs/architecture/backlog-board-ordering.md](docs/architecture/backlog-board-ordering.md).
