...
- **File budget:** a source file over 5,000 lines is shrink-only. No new Python in `cli/src/fno`: new code lands in `crates/`, and a Python edit is a port or a deletion. `scripts/ci/check-file-budget.sh` is the push-time backstop.
