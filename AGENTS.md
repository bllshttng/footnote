...
- **Demand signal:** `fno backlog encounter <id> --evidence "..."` records ONE encounter per session. Evidence is required and capped. `fno backlog demand` ranks them by divergence from operator priority. It never re-ranks the board. [demand signal](docs/backlog-usage.md#demand-signal-what-the-agents-keep-hitting).
