...
**Cross-session (the unified loop).** One Rust runtime (`fno-agents loop run`, step 5) drives both loop altitudes; drivers differ only in their Queue/Dispatcher impls:
...
Driver skills (`/target`, `/megawalk`) are self-contained at the skill-folder level so each is portable to any markdown-aware runtime (Codex, Gemini, openclaw, hermes). The only external dependency is `fno`, declared in skill frontmatter as a binary dependency.
