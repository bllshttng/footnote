...
- **Test value:** every new or changed test answers the four-question authoring gate in [skills/test-audit/SKILL.md](skills/test-audit/SKILL.md); footnote-only mechanics (runners, campaign shape) in [.claude/rules/test-value-gate.md](.claude/rules/test-value-gate.md). The review lane's Test value angle enforces it on diffs.
