...
4. **Surgical changes, but fix what you find.** Touch what the task requires and match existing style. But a pre-existing bug, flaky test, lint violation, or dead code you hit while in the code gets FIXED here, as its own atomic commit - the warm window is the only window a deferred find reliably gets (in practice we clear ~20-30% of the backlog). The narrow exception is a genuinely large effort that needs its own design: carve it out (`fno carveout add`) or file a node whose details name the concrete fix path. When in doubt, fix it now.
