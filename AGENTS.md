...
**Role-based model routing.** A spawn's *role* selects its model at spawn time: cheap coordination roles (`coordinate|tidy|orient|consolidate`) route the worker to z.ai GLM via env (`ANTHROPIC_BASE_URL`/`AUTH_TOKEN`/`MODEL`), while production roles (`implement|review-verdict`) and the default (no role) stay on the Anthropic model, byte-for-byte unchanged. Pass `--role` to `fno agents spawn`. A hard guard refuses to route a protected role cheap even via config override; with no z.ai key configured a cheap role fails safe to the default model with a one-line notice. Opt-in/config under `config.model_routing` (the secret lives in an env var or `.env` file, never in settings.yaml). See [docs/architecture/role-based-model-routing.md](docs/architecture/role-based-model-routing.md).
