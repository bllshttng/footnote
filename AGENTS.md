...
6. **Comments earn their place.** Match the surrounding file's comment density and idiom; add one only for a non-obvious invariant, race, or why-not-the-obvious. Never ticket/PR/node IDs. No gate catches this for you in code: `scripts/ci/check-no-internal-refs.sh` matches node IDs and session URLs in prose only, and it does not scan `cli/`, `crates/`, `scripts/`, `hooks/` or `skills/` at all.
