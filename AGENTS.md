...
- **Pane keeper** - a worker pane's pty master lives in a keeper process, not the server. A fresh server re-adopts the SAME child pid. Plain panes still die with the server. `fno mux pane keeper list` reads survivors. [pane-keeper](docs/architecture/pane-keeper.md).
...
Platform & ops: [harnesses](docs/HARNESSES.md) · [multi-CLI hooks](docs/architecture/multi-cli-hooks.md) · [path config](docs/path-config.md) · [workspace restore](docs/architecture/workspace-restore.md) · [disposable deletes](docs/architecture/disposable-deletes.md)
