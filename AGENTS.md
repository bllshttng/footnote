...
**Board order is NOT work order (today).** This lane key orders the *board*; it is a separate code path from *selection*. What the walker / active-backlog daemon works next comes from `fno backlog next` -> `make_selection_sort_key` (epics-first -> priority -> created_at), which does **not** consider `rank`. So `fno backlog rank --top` reorders the board but does not make a node run next - use `fno backlog reprioritize <id> p0` for that. Unifying the two (rank drives selection) is tracked as `x-d1fe`. See the "Board order vs work order" section of [docs/architecture/backlog-board-ordering.md](docs/architecture/backlog-board-ordering.md).
