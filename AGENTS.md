...
- **Spawn substrate axis** - `fno agents spawn --substrate <pane|thread|headless>`: `pane` (default), `thread` (claude `--bg`, opencode serve lane), `headless` (one-shot `-p`/`--exec`). `bg` remains a deprecated input alias for `thread` for one release. `-p` is reachable only via explicit `headless`; never default to it.
