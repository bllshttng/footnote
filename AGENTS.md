...
- **Mux backlog lane** - the `~ backlog` cards (id-first labels, the scope reason under the header) and the Enter-on-card node detail: sessions with a state-aware launch per row, the king reverse lookup, plan, notes, and the card menu's Plan spawn. [mux-backlog-lane](docs/architecture/mux-backlog-lane.md).
