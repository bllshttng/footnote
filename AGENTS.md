...
- **`fno mail` - king-mediated native review.** A worker self-invokes the native review verb (claude `/code-review`, codex `/review`) via the Skill tool first; when refused, `fno mail send <worker> --raw '/<verb>'` fires it at the prompt line (a wrapped reply does not - it relies on the worker's own invocation). Keep the reviewer off the author where a lane allows; the aim, not a gate - `self_attested` counts and merges. No live king -> advisory self-review. [review lanes](docs/architecture/review-lanes.md).
- **Self-review is enforced in the codebase, not just this prose.** A code payload floors the harness-resolved self-review reviewer onto the required set at the stop gate (`crates/fno-agents/src/loopcheck.rs`) and at `fno pr merge`, so a `/target` that ships code with no head-pinned `review_attestation` is held; the held reason and the `fno target init` receipt name this harness's verb and the `fno mail send '<verb>' --to-self --raw` fallback. Opt out with `config.review.self_review_required = false`. The obligation lives here because a direct CLI call or a non-Claude worker skips the skill layer.
