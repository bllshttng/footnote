...
- **`fno agents feed`** - one projection joining questions.jsonl + graph.json into an ordered activity feed; rows carry the node/session id the mux `prefix+e` overlay deep-links through the sideline's own attach path. [activity-feed](docs/architecture/activity-feed.md).
