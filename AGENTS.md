...
- **Operator asks:** file what the OPERATOR asked for with `fno backlog idea "..." --source-kind operator_request` (or `capture promote --source-kind operator_request`). Then `fno inbox operator status|list|ack` is the derived queue + receipt for operator turns.
