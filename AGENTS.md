...
- **Pane keeper** - a worker pane's pty master lives in a keeper process, not the server. A fresh server re-adopts the SAME pid. Plain panes still die with the server. `fno mux pane keeper list` reads survivors. [pane-keeper](docs/architecture/pane-keeper.md).
