...
- **`fno agents watchdog`** - external fleet sweep from transcript truth: wake / reroute / reap / ghost per row. Dry run by default; `--apply` (wake) and `--apply=all` (reap + reroute) act. Tick cadence behind `config.recovery.watchdog`. [fleet-watchdog](docs/architecture/fleet-watchdog.md)
