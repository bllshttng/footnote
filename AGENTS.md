...
- Signal distress without stopping: `<help reason="..." evidence="...">...</help>`. Cancel a target with `touch .fno/.target-cancelled` or `TARGET_CANCEL=1`; cancel a crowned king with `fno agents king cancel --scope <scope>`. Subprocess agents return `RESULT: BLOCKED` on stdout.
