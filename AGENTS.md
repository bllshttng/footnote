...
- **`fno mail` - king-mediated native review.** A worker self-invokes the native review verb (claude `/code-review`, codex `/review`) via the Skill tool first; when that is refused, the reliable path is `fno mail send <worker> --raw '/<review-verb>'` - the raw payload fires at the worker's prompt line (a wrapped reply does NOT fire it; it relies on the worker pulling its own trigger, which is the unreliable half). The reviewer must not have authored the diff - a king reviewing its own diff is self-review even through --raw. No live king -> advisory self-review. [review lanes](docs/architecture/review-lanes.md).
