...
- **Demand signal:** `fno backlog encounter <id> --evidence "<what it cost>"` records ONE encounter per session, evidence required and capped; `fno backlog demand` reads them ranked by divergence from operator priority. A read that never re-ranks the board. [demand signal](docs/backlog-usage.md#demand-signal-what-the-agents-keep-hitting).
