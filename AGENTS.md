...
- **In-N-Out CLI menu (`fno --help` is curated)** — `fno --help` advertises a small menu (~9 top-level verbs); most commands are **hidden but fully invocable**. To see everything, run **`fno help --all`** (the full-surface door, rendered from the registry, so it never breaks on a bad sub-app). A hidden verb's own `fno <verb> --help` still works — hiding changed display, never dispatch. **New verbs default to hidden**; promoting one to the menu is deliberate and lint-gated: `fno lint menu-caps` fails CI if the advertised surface exceeds 10 top-level / 12 per sub-app (raise the named cap constant in a one-line diff, or mark the verb hidden). The display-surface counterpart of the LOC ratchet.
